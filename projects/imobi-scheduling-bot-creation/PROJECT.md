...
- [x] **Per-conversation rate limit** — verified seed gap: `noctusai_lib.api.rate_limit.create_limiter` is per-IP (slowapi), not per-conversation. Shipped product-side `app/services/conversation_rate_limit.py` (`RedisConversationRateLimiter` — fixed-window counter, 30/60s default, fail-open on Redis outage). Wired into `whatsapp_router.handle_inbound_whatsapp` BEFORE auth resolution; lifespan-configured via `make_redis_client` / `make_fake_redis_client` selection. N=1 consumer; seed-lift destination `noctusai_lib.api.conversation_rate_limit` filed below.
- [x] **Prompt-injection test suite** at `tests/security/test_prompt_injection.py` — 21 tests across 7 adversarial input shapes (instruction-override / role-swap / jailbreak / system-prompt-leak / tool-arg-injection-cpf / tool-arg-injection-email / delete-tool-request). Each parametrized across `test_adversarial_input_does_not_bypass_tool_allowlist` + `test_system_prompt_contract_present`; plus `TestToolDescriptorAllowlist` (allowlist invariant), `TestSanitizationWrapsHandler` (PII redaction in loop_messages), `TestNoDirectStateMutationFromInboundText` (no side-effect under text-only LLM reply). OpenAI SDK patched at the consumer seam per the harness convention; no monkey-patching of our own code.
- [x] **Output sanitization** — `app/services/sanitization.py` ships `sanitize_tool_result(content)` + `wrap_handler(handler) -> handler`. Patterns: email / Brazilian phone / CPF / CNPJ / URL-with-creds. Wrapped into the conversation processor's tool-handler stack at `app/services/conversation.py::_build_processor` BEFORE the dispatcher loop sees the result. Audit-writer in dispatcher loop sees the sanitized content too (audit-table purpose is "did this fire?" — the underlying DB row IS the raw payload, no double-write).
- [x] **Anomaly detection** — counter-based threshold (20 dispatches per 60s per conversation, sliding-window via Redis ZSET). `app/services/anomaly.py` ships `ToolDispatchAnomalyDetector` + module-level `record_dispatch_observed(...)`. Threaded via a ContextVar (`_current_conversation_id`) set in the conversation processor — the wrapper reads conversation_id without changing the seed's `ToolHandler` signature. Triggers a structured WARN log; no automatic kill-switch (false-positives blocking conversations is worse than the threat; rate-limiter is the hard guard). ML-based detection deferred to `projects/platform-anomaly-detection/` follow-up.
- [x] **KB pattern doc** — extended `KNOWLEDGE-BASE/CONTEXT/PATTERNS/llm-bot-security.md` from 202→333 lines: new §7 "First adopter — imobi-scheduling" with subsections 7.1 rate-limiter / 7.2 sanitization / 7.3 prompt-injection test patterns / 7.4 anomaly detection / 7.5 adoption checklist. Already linked from `CLAUDE.md § 2 The Map` (pre-Phase-11) and `KNOWLEDGE-BASE/INDEX.md` (patterns table); no new wire-up needed.
- [x] **Tests**: 67 new tests (`tests/services/test_conversation_rate_limit.py` × 14 + `tests/services/test_sanitization.py` × 22 + `tests/services/test_anomaly.py` × 13 + `tests/security/test_prompt_injection.py` × 18). Full suite **393 passed** (was 326 Phase 10 baseline; +67).
- [x] `python mcp/noctusai/cli.py --review --product imobi-scheduling` → **0 issues**.
- [x] `bash scripts/verify-kb-sync.sh` → green.

**Improvements:**
- **P0 — Seed gap: per-conversation rate limit (orthogonal to seed's per-IP slowapi limiter).** The seed's `noctusai_lib.api.rate_limit.create_limiter` is HTTP/IP-axis only. Chat platforms (WAHA, Slack, Telegram) need a per-conversation axis — every inbound arrives via the upstream broker's IPs, so per-IP can't distinguish a flooding chat from a legitimate one. Shipped product-side as `RedisConversationRateLimiter`. **N=1 consumer.** Seed-lift destination: `noctusai_lib.api.conversation_rate_limit` (or `noctusai_lib.domain.chatbot.rate_limit` per the chatbot framework's home). File the seed project when a second LLM-tool product (therapy / mailing inbound replies) hits the same axis.
- **P0 — Seed gap: PII sanitization library is consumer-side.** No `noctusai_lib.primitives.pii` module exists today. Each LLM-tool product currently rolls its own regex catalogue (or absorbs nothing). Shipped at `app/services/sanitization.py` as N=1 consumer. **N=2 destination**: `noctusai_lib.primitives.pii` per `KB § PATTERNS/seed-lib-layout.md § primitives` — pure-logic redactors, no IO, exempt from the Fake+Real shape per `KB § PATTERNS/seed-fake-real-adapter.md` § Exemption. File the seed lift when therapy / mailing surface the same regex needs (likely; both touch corretor / patient contact info).
- **P0 — Seed gap: anomaly detector for chatbot platforms.** No `noctusai_lib.domain.anomaly` module exists. Shipped at `app/services/anomaly.py` as N=1 consumer. **N=2 destination**: `noctusai_lib.domain.anomaly.threshold_detector` (or as a sibling of `noctusai_lib.domain.chatbot` since the consumer pattern is "wrap the dispatcher's tool handler"). The ContextVar-threading of `conversation_id` should also move to the seed dispatcher itself — the current product-side ContextVar is a workaround for the seed's `ToolHandler` signature not carrying conversation context.
- **P1 — Sanitization sees the audit too (deliberate).** Per the seed dispatcher contract, `tool_handler(call)` runs BEFORE `audit_writer(call, result)`, so wrapping the handler means the audit gets the sanitized content. This is intentional (the underlying DB row IS the raw payload — no double-write of LGPD-controlled data into `tool_call_audits`). Documented in `sanitization.py::wrap_handler` docstring. If a future requirement is "audit raw, LLM-loop sanitized" we'd add a second seam layer; today the simpler shape covers the threat model.
- **P1 — Rate-limiter is fixed-window, not sliding.** The minor edge case (a burst straddling two windows can briefly exceed the limit by ~2x at the window boundary) is acceptable for flood detection. Sliding-window would need Redis Lua or a more complex multi-key approach; deferred until the threat model surfaces a precision need. Documented inline.
- **P1 — Anomaly detector emits WARN log only, no automatic action.** Deliberate (false-positives blocking legitimate conversations is worse than the threat; the rate-limiter is the hard guard). Operator dashboards filter on the log. ML-based detection + per-tool-name baselines lives in the `projects/platform-anomaly-detection/` follow-up; deferred until the WARN log volume gives us labeled training data.
- **P2 — ContextVar threading of conversation_id is a workaround.** The seed's `ToolHandler = Callable[[ToolCall], ToolResult]` signature doesn't carry conversation context. We thread via a module-level ContextVar set in the conversation processor. Cleaner long-term: extend `ToolCall` with an optional `conversation_id` field, OR introduce a `DispatchContext` parameter to `ToolHandler`. Surface candidate for the seed lift filed above — when we lift the anomaly detector, the seed-side ContextVar / signature change comes with it.
- **P2 — Phone regex calibrated for Brazilian numbers only.** The pattern matches `(NN) NNNNN-NNNN` / `+55NN...` shapes; international numbers (international corretor contacts? unlikely in v1) wouldn't redact. Acceptable for the single-agency v1 scope. Add international patterns if/when the product surface widens.
- **P2 — `delete_tool_request` adversarial input is the SOFT side.** The test asserts that an inbound asking for `drop_all_tables()` does NOT cause a tool dispatch under a benign LLM mock. The HARDER test — what if the LLM mock returned `drop_all_tables` as a tool_calls field? — is covered by `TestToolDescriptorAllowlist::test_unknown_tool_returns_structured_unknown_tool_payload` (the handler returns `unknown_tool`, no exception, no side effect). Both tests together cover both axes.
- **P3 — Worktree Write tool phantom-success N=7+.** N=4 confirmed in this session: ALL Write + Edit tool calls in the first pass phantom-succeeded (file write reported "successful" but `git status` + `ls` showed no files on disk). Heredoc fallback via Bash + libcst Python edits landed cleanly. Per `feedback_findings_md_return_as_text.md` the methodology adapts at role boundary; per §16.7 N=6+ watch the heredoc fallback is the standing answer. Recurrence rule fires — the seed-side fix (a write-tool guard that re-reads the file post-write + flags absence) is the right destination. Filed for tracking; not blocking Phase 11 close.

### Phase 12 — README + MASTER-PROMPT + KB ✅

- [x] **`README.md` (human-facing)** — replaced the scaffold seed-stub prose with product-specific content (stack table, run commands, key-features bullets, DEPLOYMENT.md pointer, test breakdown). Folded into noc 2026-05-11; sibling repo path scoped to repo-name only (`whatsapp-google-scheduling/`) so post-deletion the README has zero dead pointers.
- [x] **`MASTER-PROMPT.md` (agent-facing)** — replaced the scaffold seed-stub prose with the canonical agent-facing shape (Purpose / Architecture / Key Domains / Services / Seed Consumption / Phase 0 stamped decisions / Imobi-Specific Patterns / Standard Routers Mounted / Frontend (deferred) / Development Guidelines / Testing / Contract Notes / Dependencies). 13 services enumerated; 17-table schema mapped to identity / inventory / scheduling / conversation groups.
- [x] **`KB § CONTEXT/02-LANDSCAPE.md`** — added Imobi Scheduling row (ports 8011/8160, schema `imobi_scheduling`, description "WhatsApp chatbot scheduling real-estate media crews … folded 2026-05-11 from sibling repo …"). The inventory + database-schemas blocks already carried imobi via the pre-commit `scripts/update-kb-counts.py` auto-update — no manual edit needed there.
- [x] **`KB § INDEX.md`** — verified the three NEW pattern docs (`chatbot-operational-readiness.md` / `llm-bot-security.md` / `whatsapp-chatbot-seed.md`) are already indexed (Layout tree + patterns table); pre-Phase-10/11 KB-first authorship landed them. No new entries needed at Phase 12.
- [x] **`CLAUDE.md § 2 The Map`** per-product pointer — `chatbot-operational-readiness.md` already mentions "first adopter imobi-scheduling" in the existing patterns bullet. No new pointer needed; the §3 "When to read what" table already routes per-product traffic via the universal "Backend specs → KB § backend/0X-*.md" + "Per-product details — open only the product you're working on" rows.
- [x] **Three-way sync (KB ↔ CLAUDE.md ↔ memory)** — `bash scripts/verify-kb-sync.sh` green. The KB / CLAUDE.md / memory layers all already reflect chatbot-operational-readiness adoption (Phase 10 close landed the three-way sync; Phase 12 just verified).
- [x] **Sibling-path reference scrub** — one stale path-style reference found in `products/imobi-scheduling/backend/app/prompts/scheduling_bot.md` (commentary block citing the source-prompt path `~/Documents/.../whatsapp-google-scheduling/.../conversation.py`). Rewritten to a relative-style reference (`whatsapp-google-scheduling/` + `projects/imobi-scheduling-bot-creation/`); survives sibling-repo deletion per §12.

**Improvements:**
- **P1 — Per-product KB doc not yet authored.** Products with depth-content (PF / ERP / Therapy / Core / Daily-Life) carry a `KB § CONTEXT/backend/0X-PRODUCT.md`. Imobi-scheduling's depth lives entirely in `MASTER-PROMPT.md` today. **N=2 trigger** is when an unrelated agent doing seed-first-design needs to read imobi-scheduling-specific patterns and the MASTER-PROMPT is too shallow — author the KB doc then. Until then the MASTER-PROMPT serves as both agent-facing brief AND depth reference.
- **P1 — Frontend folder is a placeholder.** Per Phase 0 Q3 (DEFER), the scaffolded `frontend/` ships unmodified seed prose. A follow-up project (`projects/imobi-scheduling-admin-ui/` TBD slug) will either populate (operators / condos / properties / services / crew CRUD) or formally delete. Tracked in accept-with-rationale (Phase 0 close).
- **P2 — `scheduling_bot.md` provenance comment cites a sibling-relative path.** Cleaner long-term: store the prose source in a code-only artifact (e.g. a fixture) that survives the prompts/ folder layout. Surface candidate during a future i18n refactor when prompts move into a translatable framework.

### Phase 13 — Frontend (CONDITIONAL — see §7 Q3) ✅

- [x] **DEFERRED per orchestrator-stamped §7 Q3** (2026-05-10): WhatsApp-only v1; admin UI is a follow-up project. The scaffolded `frontend/` folder stays as placeholder — no code authored, no route wiring, no test added. Documented in §11 + accept-with-rationale.

**Improvements:**
- **P1 — Follow-up project file `projects/imobi-scheduling-admin-ui/` not yet created.** Per branching-first orchestration the follow-up branch + brief author themselves when an architect picks up the admin-UI scope. Trigger: agency operator asks for a non-WhatsApp surface (audit-trail view / condominium roster / crew calendar), OR a second agency requires multi-tenant routing (which needs an admin surface anyway). Until then the placeholder frontend ships as-is.

### Phase 14 — Final verification + go-live ✅

- [x] **All three test layers green** — `cd products/imobi-scheduling/backend && /Users/rapha/Documents/repository/NoctusAI/noctusai/venv/bin/python -m pytest tests/ -q` → **393 passed, 1 warning in 4.73s** (matches Phase 11 baseline; no test churn this phase). Test layout: structural (89 migration + 36 schema) + routers (health/team/example/webhook/whatsapp) + services (13 service-test files) + lifespan + security/prompt_injection + integration/e2e_flows.
- [x] **`bash scripts/verify-kb-sync.sh` green** — all CLAUDE.md pointers resolve, all KB docs indexed, Layout tree current. Re-verified post-landscape edit.
- [x] **Keeper review** — `python mcp/noctusai/cli.py --review --product imobi-scheduling --worktree-path "$PWD"` → **0 issues**. Mode: agent; no proposals filed.
- [x] **Sibling repo deletion-safety check** — searched product code + KB + CLAUDE.md / topical for absolute path `~/Documents/repository/NoctusAI/whatsapp-google-scheduling/`: one occurrence in `products/imobi-scheduling/backend/app/prompts/scheduling_bot.md` (provenance commentary) rewritten to a relative-style citation surviving sibling deletion. KB references (e.g. `KB § PATTERNS/seed-fake-real-adapter.md`, `whatsapp-chatbot-seed.md`, `containerization.md`) cite the sibling repo as the *origin* of folded patterns — historical references, not functional pointers; they remain valid prose after the sibling is deleted (same shape as citing any defunct upstream library). **No `pyproject.toml` path-dep, no symlink, no editable install** under `products/imobi-scheduling/` — `grep -r "whatsapp-google-scheduling" products/imobi-scheduling/` returns only the rewritten prompt comment.
- [x] **Manual end-to-end smoke against staging WAHA + sandbox Calendar** — **DEFERRED** to the user's deploy drill per `KB § GUIDES/deploy-workspace-online.md`. No real WAHA session or Google OAuth credentials in the worktree; the seed Fake adapters cover dev paths. Production smoke fires on the first `./start.sh tunnel imobi-scheduling` run.

**Improvements:**
- **P1 — Live deploy smoke is operator-side.** Same deferral as Phase 10. The smoke recipe is documented in `DEPLOYMENT.md § Smoke test` (post-deploy) + the deploy guide (`KB § GUIDES/deploy-workspace-online.md`). Captured here so the project close doesn't claim "verified end-to-end against production" without that gate. First production deploy is the natural moment to run it.
- **P2 — KB citations to `whatsapp-google-scheduling` survive sibling deletion as prose.** The sibling repo's name appears in 5+ KB pattern docs (origin attribution for folded patterns). These are *historical* citations — same shape as citing a deprecated upstream library. Post-deletion they're still semantically valid; only path-style URI-references (which we scrubbed) would break.
...
| 2026-05-11 | **Phases 12 + 13 + 14 (final close — README + MASTER-PROMPT + KB / frontend / final verification) closed by Engineer IMB-FIN.** Phase 12: replaced the scaffold seed-stub `products/imobi-scheduling/README.md` (38→61 lines, real stack + features + DEPLOYMENT.md pointer + test breakdown) and `MASTER-PROMPT.md` (78→125 lines, full agent-facing shape per the PF reference: Purpose / Architecture / Key Domains / Services (13) / Seed Consumption map / Phase 0 stamped decisions / Imobi-Specific Patterns / Standard Routers Mounted / Frontend (deferred) / Development Guidelines / Testing / Contract Notes / Dependencies). Added Imobi Scheduling row to `KB § CONTEXT/02-LANDSCAPE.md` Products table (8011/8160 / `imobi_scheduling`). INDEX.md + verify-kb-sync.sh + memory: already current from Phase 10/11 KB-first authorship. Sibling-path scrub: one stale `~/Documents/.../whatsapp-google-scheduling/...` reference inside `products/imobi-scheduling/backend/app/prompts/scheduling_bot.md` rewritten to relative-style citation. Phase 13: DEFERRED per orchestrator-stamped §7 Q3 (2026-05-10) — scaffolded frontend stays as placeholder; follow-up admin-UI project will populate or delete. Phase 14: pytest `tests/ -q` → **393 passed, 1 warning in 4.73s** (Phase 11 baseline preserved); `bash scripts/verify-kb-sync.sh` green; `cli.py --review --product imobi-scheduling` → 0 issues; sibling-deletion safety check — only KB historical-citation references survive (semantically valid post-deletion); live WAHA + Calendar smoke deferred to operator deploy drill (no real creds in worktree). | engineer-subagent (IMB-FIN) |
