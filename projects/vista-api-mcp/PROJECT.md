...
- **Created:** 2026-05-02
- **Last updated:** 2026-05-03 (Phase 1 shipped + post-Phase-1 seed-lib formalization; documentation track active in KB)
- **Status:** ✅ **Phase 1 closed 2026-05-03** — `mcp/vista/` MCP server live with 10 tools registered + per-tenant calibration + JSON-escape detector bug fix; Vista client formalized into `noctusai_lib.integrations.vista` (canonical home for ERP + MCP consumers per recurrence-rule absorption); 12/12 smoke tests green. Documentation source-of-truth lives at `KNOWLEDGE-BASE/CONTEXT/INTEGRATIONS/vista.md`. **Phases 2-5 deferred** per §7 Q6 reactivation triggers (keeper detector for guide↔adapter drift, full integration tests against live tenant, nested-sub-field proactive discovery, per-endpoint probe sentinels). Documentation track stays open as long-tail upkeep on the KB doc.
