...
- [x] **3.2** `integrations/storage/` (Supabase Storage + Local + Fake) — Engineer H, branch `sh-yt-storage`. Canonical Protocol+Fake+Local+Supabase+factory. 7 source files + 52 new tests covering put/get/delete/list/signed_url/exists, 404→None translation, factory routing.
- [x] **3.3** `integrations/quota/` (Redis + Fake) — Engineer I, branch `sh-yt-quota`. `QuotaConfig` + `QuotaCheck` + `QuotaTracker` Protocol (`runtime_checkable`), `InMemoryQuotaTracker` (deque-per-key sliding window + asyncio.Lock), `RedisQuotaTracker` (ZSET sliding window via WATCH/MULTI/EXEC — switched from Lua-EVAL when fakeredis lacked Lua), `make_quota_tracker` factory. 30 new tests with YouTube-quota examples + atomicity under `asyncio.gather`.
