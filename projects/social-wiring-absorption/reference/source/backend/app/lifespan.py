"""FastAPI lifespan hooks for YouTube Crawler.
...
youtube-crawler-specific wiring, mirroring the shape that
...
async def on_startup() -> None:
    ...
    logger.info(
        "YouTube Crawler lifespan startup: ConversationModule ready (org_id=%s).",
        org_id,
    )
...
async def on_shutdown() -> None:
    ...
    await stop_worker()
    logger.info("YouTube Crawler lifespan shutdown complete.")
