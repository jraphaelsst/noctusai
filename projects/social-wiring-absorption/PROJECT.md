...
- [ ] Wave-2 **full-suite-green** gated on **TEST-ISO** (in flight) — pre-existing N=2 module-conftest global-state leak (consent registry / DatabaseModule patch / `ai_consent_features` sys.modules cache / APScheduler not restored per-test) causes ~55 cascade ONLY in the full directory run; **proven NOT integration-caused** (W2.1 dir-run = stated baseline with splices in). Root-fix = shared `tests/modules/conftest.py` autouse reset. Wave 2 ✅ + the irreversible Wave 4 hard-gate both wait on this.
