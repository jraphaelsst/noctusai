...
### Wave 3 status ⏳ — all 3 pilots confirm ZERO reconcile-induced breakage; blocked-clean on DEP-A (pre-existing seed-infra), then re-baseline.
...
- [ ] DEP-A (pause-on-dependency, in flight) — structural `purge_shadowing_editable_finders` per-package-root fix → eliminates the N=12 conftest workaround. On merge: re-baseline therapy (expect green, no PYTHONPATH crutch); ERP/core call-site workarounds become harmless-redundant (accept-with-rationale).
...
- [ ] W5.8 **Pilot-products-first refactor cadence** — three-way sync the NEW platform rule: KB (new pattern or §PATTERNS/project-execution addition) + CLAUDE.md §1 bullet + memory `feedback_pilot_products_first` (done, s2) + MEMORY.md index (done). Codification candidate: a `noctus.dev` advisory that warns when a ripple-wave dispatches >pilots+core. Pilots = erp-imobiliario · therapy-platform · social-wiring (+core control-plane). Add the Wave-3b "extend after pilots green" step to the absorption playbook (W5.1).
- [ ] W5.9 W3-surfaced: (a) `check_hardcoded_product_slug_set` keeper — seed tests must derive product sets from `parse_products_registry()`, never hardcode (N=2: cors_registry+sentinel) — s4; (b) tool-code-coherence: the `seed/{lib,framework}` axis-swap moved package roots without updating `purge_shadowing_editable_finders` — add to the doc-code-coherence rule's discovery (a tool that moves a package root must update its guards same-change); (c) `@supabase/supabase-js` version-lockstep alignment (seed-frontend + all product frontends) — separate cross-cutting follow-up project, surfaced by W3.1.
