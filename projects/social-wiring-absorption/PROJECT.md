...
- **Status:** ✅ **Done** — all 7 waves complete (2026-05-16); pushed at close
...
- [x] W2.2 ✅ Absorb `mailing` → `app/modules/email_marketing/` (`a559509`, integrated `a774841`) — 33 tests; consumes seed digest/llm/ai/scheduler/webhook_signatures; NEW subtree; held deltas spliced.
- [x] W2.3 ✅ Absorb `imobi-scheduling` → `app/modules/scheduling/` (`a559509`, integrated `a774841`) — 58 tests (30+28 router); consumes seed scheduling/chatbot/retry_policy; NEW subtree; held deltas spliced.
...
- [x] ~~W2.5/W2.5b~~ → **RE-SCOPED OUT of Wave 2 (Option A, architect decision)** ✅-as-decided. W2.5 AND W2.5b both correctly blocked (zero edits, surfaced) — the workspace↔seed contract gap is **4 axes** (credential-read [bridgeable] · OAuth credential-WRITE path · `isinstance` adapter-type labeling · **Meta method-set capability gap**: Wave-1.E4 dropped `me()`/`get_page()`), and the 69-test oracle tests workspace-package internals (delete = destroy oracle). **Decision:** social-wiring KEEPS its validated `services/{calendar,meta,routing,drive_api}`+`credential_store.py` product-local (validated + self-oracled; product is real-world-functional). Seed-convergence for these 4 deferred to follow-up project **`seed-adapter-convergence`** (`projects/seed-adapter-convergence/`), gated on the seed FIRST shipping: Meta `me()`/`get_page()` capability + `credential_store=` convenience + OAuth-credential-WRITE seam. Catalogued in `KB § PATTERNS/accept-with-rationale.md` (N=1 product-local, deferred convergence, named destination). W5.7(a) extended to the **4th shape**: verify-seed-ships-it must assert the seed ships the full consumer-coupling surface (method-set + write-path), not just a same-name/same-signature factory.
- [x] Wave-2 integration ✅ **wiring merge-ready** (`a774841`) — both `001` SQL blocks spliced verbatim (collection clean, 0 MockSchemaError, 353+ collected), MODULES=`[media_wiring, email_marketing, scheduling]`, lifespan scheduler splice (N=1), requirements (resend/APScheduler/sqlalchemy), +28 status-pinned scheduling router tests. App boots 137 routes, 3 modules mount. **Every suite green in isolation** (W2.1 45/45 · em 33/33 · sched 58/58 · services 209 + W2.5 local-adapter oracle 52/52); W2.1 baseline preserved → splices don't regress.
- [x] Wave-2 **full-suite-green ✅** (`09b7165` TEST-ISO) — root cause sharper than diagnosed: the 2 child module conftests re-ran `purge_shadowing_editable_finders` AFTER the parent's single canonical purge + seed-singleton population → `sys.modules` swap of `noctusai_lib`/`noctusai_seed` under cached `app.*`. Root-fix: shared `tests/modules/conftest.py` (no re-purge + autouse guard) + removed the redundant purge from the 2 conftests. Full dir run (default + `no:randomly` + 2 seeds): **1 failed / 364 passed** = the 1 known-accepted `[A]` TestTeamFlow; **cascade ~81 → 0**; isolation suites unweakened (em 33 · sched 58 · routers 45). **Wave 2 FULLY GREEN.** Seed N≥3 helper (`noctusai_lib.testing.seed_singleton_guard` — 2nd product-side reinvention of seed-singleton test hygiene, cf `bind_consent_module_to_mock`) → W5 codification route.
- Hygiene done: dropped the W2-INTEGRATE engineer's slip `stash@{0}` (it used `git stash` for counterfactual isolation under a ZERO-git brief; working tree verified intact first; 10 unrelated stashes untouched).
...
### Wave 4 — Teardown ✅ (gate satisfied; executed irreversibly + verified)
**DONE** — commits `8012f7e` (a: 3-dir delete + sentinel-derive atomic), `5523bde` (b: migration 033 + live Supabase mirror), `dffe387` (c: CLAUDE/platform twin + LGPD resolved + ledger). W4.0 preservation FIRST (snapshots at `.integration-holding/W4.0-preservation/` + ledger +1; LGPD imobi entry **resolved** — social-wiring uses Fernet `CredentialStore`, vuln did not carry forward). 4 product dirs gone (`media-scheduling` was untracked debris). Verification: 8-cmd set green; **zero RAW-dangling refs** (every residual doomed-slug mention is the correct dated-retirement form — "Retired 2026-05-16 → social-wiring"; accept-with-rationale 44 hits = sanctioned durable register, preserved). KB-scrub completeness independently re-verified by the architect (false-alarm from hook auto-stage into `8012f7e` cleared — no-silent-error both ways). **3 pre-existing W0.3 social-wiring scaffold defects surfaced (Wave-4-independent, NOT worked around) → routed:** (1) `app/config.py:22` hand-enumerates CORS as the *retired youtube-crawler ports* 8010/8150 instead of `@registry:own:social-wiring`; (2) `docker-compose.override.yml` missing → `docker compose config` invalid (override-registration drift); (3) scaffold readme/master-prompt slug-substitution. → fixed in the SW-SCAFFOLD-FIX chunk (gates Wave 6 docker-validate + product correctness).
...
- [x] W4.0 ✅ RISK-A preservation FIRST — snapshots + ledger +1; LGPD imobi entry resolved (Fernet, vuln didn't carry forward).
- [x] W4.1–W4.4 ✅ 4 dirs deleted · compose/start.sh/ports/init-db sanitized (regenerated) · migration 033 + live Supabase mirror (013/028 immutable) · KB/CLAUDE-twin/sentinel-registry-derive(same-commit)/accept-with-rationale/ledger scrub — verified zero RAW-dangling (all dated-retirement form).
...
### Wave 5 — Methodology codification (three-way sync) ✅
> Every item is either LANDED or discharged via a filed named-destination follow-up (the methodology's valid completion form — no silent deferral).
- [x] W5.1 ✅ Absorption playbook (`18e5a41`) — `KB § GUIDES/absorb-seed-workspace.md` (10 gates, self-contained, written from the proven flow) + CLAUDE.md §2 Map + §3 routing + INDEX. verify-kb-sync green.
- [x] W5.2 ✅ Promotion-map automation (`17e2fc2`) — `scripts/gen-promotions-index.py` (scans `.promotions/*.md`, reuses canonical `parse_manifest`, `--check` drift-gate); `bootstrap-seed-workspace.sh` ships the `.promotions/` convention + MANIFEST-TEMPLATE + generates the index; seed-workspace pre-commit Rule 3 refuses drift. KB § PATTERNS/seed-workspace.md + CLAUDE.md §1 + memory `feedback_promotions_index_derived` + MEMORY.md (s3 three-way). 9/9 tests; closes W0.2 stale-index slip. **Routed follow-up (no-silent-error):** canonical `parse_manifest` (`mcp/noctusai/tools/noctus/dev/promotion.py`) doesn't parse YAML block-list `origin:`/`intended_noc_destination:` → ~5/14 manifests render empty destination; N≥2 consumers (this generator + `list_promotions` + `promote_from_seed_workspace`) → fix-at-root follow-up project `promotion-manifest-parser-blocklist` (NOT W5.2 scope).
- [x] W5.3 ✅ Divergent-arch → house-container rule (`18e5a41`) — KB § PATTERNS/containerization.md § 12a + CLAUDE.md §1 bullet + memory `feedback_divergent_arch_house_container` + MEMORY.md (s3 three-way).
- [x] W5.4 ✅ Full-reconcile-to-validated-source precedent + SEED-NEEDS fixes — documented in the playbook gate-5 + `KB § PATTERNS/accept-with-rationale.md` (social-wiring Wave-2 entry) + findings; SEED-NEEDS dev-auth shipped flag-gated (W1.E7), sqlite-backend → filed stub `projects/seed-sqlite-dev-backend/`.
- [x] W5.5 ✅ (canonicalized) — the worktree-base / master-tree-parallel / no-collision discipline is now canonically homed in the playbook (gates 1 & 5) + already correctly stated across CLAUDE.md §1 + the existing memory entries; the substantive rule (uncommitted/branch-only inputs invisible to `isolation:worktree` → commit-to-base OR master-tree OR inline) is captured. Residual memory-entry de-dup is benign cosmetic housekeeping (all 3 layers state the rule correctly; the overlap is non-misleading) — explicitly NOT blocking; left as optional tidy.
- [x] W5.6 ✅ Improvement-queue routed — Wave-1 bundle proposal filed in `proposals/`; #1 stale-PROMOTIONS→W5.2 ✅, #2 scaffold-override→SW-SCAFFOLD-FIX ✅; #3 `safely_run` (N=6) + #4 `dependencies.py` boilerplate (N=7-8) → Wave-3b/post-pilot (named destination, tracked).
...
- [x] W5.9-rest ✅ routed — `check_hardcoded_fleet_size_literal` keeper LANDED (`18e5a41`, 32/32, the tool-code-coherence/frozen-fleet-literal class); supabase-js version-lockstep → routed in `seed-adapter-convergence` follow-up (W3.1 surfaced).
- [x] W5.7-rest ✅ routed — (b) `check_dockerfile_vite_supabase_args` = Stage-3 deferred-with-rationale (deterministic but lower-priority; noted, not silent); (c) `seed-sqlite-dev-backend` stub filed; (d) `VistaClientProtocol`+4th-shape → `seed-adapter-convergence` §1a; (e) `compute_content_stats` N=2 → accept-with-rationale (minor in-seed dup; Wave-2-integration noted); (f) imobi N=2 seed-lift → `seed-singleton-guard` stub + the seed-adapter-convergence routing. All discharged via named destinations.

### Wave 6 — Containerization resume ✅ (the original task that started this project)
- [x] W6.0 ✅ (`513293c`) — fixed TWO unsubstituted-seed defects (compose service/container/image/tunnel/depends_on AND backend/Dockerfile was a verbatim seed copy, EXPOSE 8004) + root-fixed `sync-seed-template.sh`+templates+propagate so future scaffolds substitute. N≥2 latent `seed-tunnel:` service-key drift across 7 products fixed at propagate root. No other product had the service/container defect.
- [x] W6.1 ✅ (`513293c`) — both compose projects `config -q` exit 0 (9 products, external noctus-net); `propagate-{dockerfiles,composes} --check` in-sync; init-db verified (Wave-4-regenerated, provenance-comment-only retired refs).
- [x] W6.2 ✅ **social-wiring builds in the house single-container model** — `ghcr.io/jraphaelsst/noctus-social-wiring:dev` (1.51GB, `runtime-watch` target, frontend `npm install` 345 pkgs OK → the 3-dep fix worked, `FROM noctus-seed-*-base` seam, no `noctus-seed` collision). **The absorption's containerization deliverable is PROVEN.** Two cold full-fleet `./start.sh full` attempts each hit a NON-absorption obstacle, both correctly diagnosed + handled: (1) Docker-VM disk-full → storage-hygiene reclaim ~30GB (env, not code); (2) `erp-imobiliario` pre-existing `pycairo`/`xhtml2pdf` C-extension build-dep gap (this branch never touched erp's requirements; surfaced-not-caused) → routed to follow-up `projects/fleet-build-product-deps/` (named destination). **Scope discipline:** this project delivers the absorbed product + the correct consolidated refactor (✅); whole-fleet-`./start.sh full`-green is gated on that separate pre-existing-erp follow-up, NOT on this project.
- [x] W6.3 ✅ (`513293c`) — `TestScaffoldRegistersInRootCompose` (5 status-pinned; test_scaffold 57) pins the house single-line include shape (would have caught the override-drift at author time).

### Wave 7 — Completeness sign-off + project close ✅
- [x] W7.1 ✅ Completeness re-check vs W0.2 **VERIFIED** (concrete, not assumed): 14 manifests + workspace docs in-home; 195-file validated SOURCE in-home; W4.0 preservation snapshots in-home; mailing+imobi domains live in `social-wiring/app/modules/{email_marketing,scheduling}`; LGPD imobi entry resolved; 4 dirs gone + registry-retired (migration 033 + live mirror); social-wiring image built (house model); **authoritative grep = ZERO dangling deleted-product refs/paths** (W4-CLEANUP `c042b37` closed the W7.1-verify-caught N≥24 provenance-pointer gap). ⇒ nothing useful remains only in the workspace.
- [x] W7.2 ✅ Sign-off delivered to the user (below). User deletes the workspace manually — we never do.
- [x] W7.3 ✅ Project close: status → Done; playbook amended with the verified teardown-scope gap; archive + FF `main` + single `git push` (literal last step).
...
| 2026-05-16 | **Wave 2 ✅ + parallel acceleration** — W2.1`1808b99`/W2.2+W2.3`a559509`/W2.4/W2.5→OptionA/integration`a774841`/TEST-ISO`09b7165` (full dir 1-fail[A]/364, cascade 81→0). W2.5b 4-axis block → `seed-adapter-convergence` follow-up + accept-with-rationale. Parallel Wave-5 drained: W5.8 pilot-first, W5.7a/W5.9a keepers, W5.2 promotion-map (`17e2fc2`, closes W0.2). Wave 4 PRE-COMPUTED (`019111a` manifest + RISK-A W4.0 sub-gate). Wave-4 hard-gate now satisfied. | Claude Opus 4.7 |
| 2026-05-16 | **Wave 4 ✅** (`8012f7e`/`5523bde`/`dffe387`) — 4 products consolidated into social-wiring, preservation-first, verified zero raw-dangling. SW-SCAFFOLD-FIX `5370b6e` (3 W0.3 defects root-fixed; surfaced + routed the container-name collision → W6.0). **Wave 5 ✅** (`18e5a41`) — absorption playbook + divergent-arch rule + fleet-size keeper + 5 follow-up stubs filed; all "-rest" items discharged via named destinations. Only Wave 6 (the originally-paused docker resume) + Wave 7 sign-off remain. 46 commits local. | Claude Opus 4.7 |
| 2026-05-16 | **Wave 6 ✅** — W6.0/1/3 `513293c` (TWO unsubstituted-seed root-fixes + scaffold-template + N≥2 tunnel-key + house-include test); W2.4-residual 3 frontend deps `7c22b50`; **social-wiring image built `ghcr.io/jraphaelsst/noctus-social-wiring:dev` (1.51GB, house model)** — absorption containerization deliverable PROVEN. 2 cold-fleet obstacles both NON-absorption + handled: disk-full→storage-reclaim ~30GB; pre-existing erp pycairo build-dep→routed `fleet-build-product-deps`. Whole-fleet-green gated on that follow-up, NOT this project. | Claude Opus 4.7 |
...
| 2026-05-16 | **Wave 7 ✅ — PROJECT DONE.** W4-CLEANUP `c042b37` closed the W7.1-verify-caught N≥24 dangling-provenance-pointer gap (authoritative grep EMPTY). Completeness VERIFIED concretely. Playbook amended with the teardown-scope gap + `check_dangling_deleted_product_path` keeper candidate. All 7 waves ✅; sign-off delivered; archive + FF main + single push = literal last step. 55+ commits, one push. | Claude Opus 4.7 |
