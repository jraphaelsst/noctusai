...
- **No collisions / preserve others' work** — no other session is active. All engineer teams work in **isolated git worktrees off `origin/main`**, file-disjoint, never the same worktree. *(User: "remember not to overwrite the job being done by others. No conflicts wanted.")*
...
4. **File-disjoint parallelism.** Wave-based dispatch; each engineer owns a disjoint package path in its own worktree; Wave N+1 gates on Wave N FF-merge.
...
### Wave 3 — Adapt remaining consumers to reconciled seed (depends W1; parallels W2)
- [ ] W3.1 therapy-platform · W3.2 personal-finance · W3.3 daily-life · W3.4 erp-imobiliario · W3.5 core — mechanical signature adaptation; pytest+vite green.
