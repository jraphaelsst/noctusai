...
### Wave 7 — Completeness sign-off + project close (NOT a deletion step) — pre-staged, fires on W6.2 green
- [ ] W7.1 Final completeness re-check vs W0.2 (assemble; mostly already true): all 14 promotion manifests + workspace docs in-home (`reference/`, Wave-0); validated SOURCE in-home (`reference/source/`, W0.5, 195 files); W4.0 preservation snapshots in-home (unabsorbed mailing/yt-crawler PROJECTs + 32KB proposals); mailing+imobi domains live in `social-wiring/app/modules/{email_marketing,scheduling}`; LGPD imobi entry resolved; zero surviving import of a doomed product (W4-RECON verified). ⇒ **nothing useful left only in the workspace.**
- [ ] W7.2 Deliver the explicit **"safe to delete the `noctusai-youtube-crawler` workspace"** sign-off to the user (user deletes manually — we never do).
- [ ] W7.3 Project close: flip PROJECT.md status → Done; `noctus.dev.archive` the project (git-mv to `archive/`); FF `feat/social-wiring-absorption` → local `main`; **`git push`** — the literal last step, the ONLY push of the whole run (per methodology: push only at project close).
