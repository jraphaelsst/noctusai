...
- [ ] **Phase enrichment loop pre-flight** (per `KB § PATTERNS/project-execution.md § 2.11`, landed by `phase-detector-and-enrichment-loop` Phase 5). Query unconsumed learnings for THIS project: `python -c "import sys; sys.path.insert(0, 'mcp/noctusai'); from tools.noctus.dev.phase_learnings import query_learnings; rows = query_learnings('repo-state-consolidation-wave-2', unconsumed_only=True); print(rows)"` — fold relevant entries into this phase's plan. Initial pickup: 0 unconsumed (this is the first execution); the loop becomes load-bearing from Phase 1 onward.
- [ ] **Drive-by-aware §11 prose convention.** When writing §11 entries that REFERENCE another file's `Phase N ✅` (e.g. citing the parallel-batched master at `products-wiring-rollout` from your own §11), wrap the reference in backticks per markdown convention. The phase-state detector (`mcp/noctusai/tools/noctus/dev/compliance.py § _strip_code_spans`, landed by `phase-detector-and-enrichment-loop` Phase 1) strips inline code spans before self-claim matching, so backticked references no longer trigger Rule-1 false positives. Unbacked "Phase 0 ✅" in your §11 = self-claim about THIS file.
...
- [ ] Run final verification suite:
  - `cd mcp/noctusai && python -m pytest tests/ -q 2>&1 | tail -3` (must be green; was 564 passed at audit time + 25 new tests from `phase-detector-and-enrichment-loop`, expect 589+)
...
- [ ] **Phase enrichment loop close-out** (per `KB § PATTERNS/project-execution.md § 2.11`). Each of Phases 1-5 logged ≥1 learning to the SQLite tracker (`mcp/noctusai/data/phase_learnings.db`); Phase 6 runs a final query to verify they're all consumed by the appropriate phase or marked `consumed_by_phase=6` (this close phase). Zero unconsumed at close — anything left points at a follow-up project. Format:
  ```bash
  python -c "import sys; sys.path.insert(0, 'mcp/noctusai'); from tools.noctus.dev.phase_learnings import query_learnings; rows = query_learnings('repo-state-consolidation-wave-2', unconsumed_only=True); print(f'unconsumed at close: {len(rows)}'); print(rows)"
  ```
...
| 2026-05-03 | **Enriched with learnings from `phase-detector-and-enrichment-loop`** (commit `2214c75` for the methodology rule, prior commits for tooling). The follow-up project flagged in the row above SHIPPED — both the detector regex precision fix (Phase 1) and the master-tree cross-reference Improvements convention (Phase 2) landed in main. Plus: the new SQLite phase-learnings tracker + 3 MCP tools (`noctus.dev.phase_learning_log` / `_query` / `_consume`) and the `Phase enrichment loop` methodology rule (`KB § PATTERNS/project-execution.md § 2.11`) are now load-bearing for THIS project's execution. **§6 enrichments applied:** Phase 0 gained two pre-flight sub-tasks (query unconsumed learnings; backtick cross-file refs in §11 prose); Phase 6 gained a close-out sub-task (verify zero unconsumed learnings at close). Full v1 learning bundle (8 entries logged + 7 consumed by v1 Phase 6 + 1 dogfood entry) lives in `mcp/noctusai/data/phase_learnings.db` (gitignored; rebuilt locally per machine). | claude-opus-4-7 |
