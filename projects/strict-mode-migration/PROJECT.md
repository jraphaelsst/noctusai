...
### Phase 3 — Framework: add tsconfig + strict from day 1 ✅ (executed 2026-05-10)

Goal: `seed/framework/frontend/tsconfig.json` exists with `strict: true` and zero errors.

- [x] Create `seed/framework/frontend/tsconfig.json`. Mirror lib's config except start with `strict: true`:
  ```json
  {
    "compilerOptions": {
      "target": "ES2020",
  ...
  ```
- [x] Install any missing peer-dep types as devDeps (the framework's 5 files import less than the lib; survey via TS2307s and install accordingly).
- [x] Run `tsc --noEmit -p .` — fix errors.
- [x] Add `"check": "tsc --noEmit"` script to `package.json`.

**Improvements / findings:**
- **Pre-tsc error count (after `npm install` + tsconfig only):** 74 errors. Almost all were TS2307 from lib's path-mapped source files referencing `lucide-react`, `@tanstack/react-query`, `sonner`, `clsx`, `tailwind-merge`, `zustand`, `@radix-ui/react-collapsible`, `@radix-ui/react-hover-card`, `@supabase/supabase-js` — none of which had ever been resolved cross-realm before because lib + framework were never tsc-checked together. **Pre-strict-fix error count (after peer-dep installs):** 4 distinct errors (1 QueryClient cross-realm + 1 `@/lib/utils` stale path alias + 2 csstype cross-realm in lib's AIFeedbackButtons.tsx). **Post-fix error count:** 0.
- **`./infra` exports entry applied (Phase 1 finding closed).** Added `"./infra": "./src/infra.tsx"` to `seed/framework/frontend/package.json#exports`. This closes the Phase 1 finding: lib's `design-system/ai/*` imports `@noctusai/seed/infra` from the framework but the package didn't declare the subpath. The runtime resolver was permissive; a strict consumer (modern bundlers using export-conditions strictly; TS without paths-mapping) would have broken. Architectural surface now matches consumer imports.
- **Peer-dep parity rule fired across the worktree boundary** — framework needed `clsx`, `tailwind-merge`, `zustand`, `@radix-ui/react-hover-card` (the four lib peers framework was missing) installed with `^` pinned to the same majors lib uses (`^2.1.1`, `^3.5.0`, `^5.0.11`, `^1.1.15` respectively). The first three came in as devDeps cleanly; the radix one rolled in at `^1.1.15`. This is the **worktree-cross peer-dep pinning rule** from the dispatch brief.
- **csstype cross-realm collision required `@types/react` install in lib** — lib's `AIFeedbackButtons.tsx` (and any React component using inline `style={...}`) types from `@types/react` walk up to `/Users/rapha/node_modules/csstype@3.1.3`, while framework's types come from local `node_modules/csstype@3.2.3`. The `CSSProperties.alignmentBaseline` enum widened between versions and won't unify cross-realm. **Fix:** added `@types/react@^18.3.23` + `@types/react-dom@^18.3.7` (matching framework's) to `seed/lib/frontend/devDependencies` so lib's type-resolution stops at its own realm. Phase 1 oversight surfaced; closed inline.
- **@tanstack/react-query cross-realm collision required exact version pinning across lib + framework.** Two installs of `@tanstack/query-core` (lib@5.90.20 vs framework@5.99.2 originally) have unique `#private` class fields and won't unify even with `^5.0.0` peerDep range. **Fix:** pinned both to `@tanstack/react-query@5.100.9` so they share the exact same `query-core` package. Without this, lib's `createQueryClient(): QueryClient` returned a different `QueryClient` class than framework's `QueryClientProvider` expected. **Methodology learning:** for class-with-private-fields cross-workspace SDKs (anything that uses ES `#private` for nominal typing — QueryClient, Pinia, MobX), `^` range pinning is insufficient. Sibling workspace packages must agree on the *exact resolved version* via deduplication or pinned exact pins. This is a stronger constraint than the worktree-cross peer-dep pinning rule and deserves its own KB pattern entry: **"private-fields nominal typing breaks workspace-cross unification."** Logged as a deferred memory entry below.
- **`@/lib/utils` stale path alias in `src/components/ui/textarea.tsx`** — only file in the framework that used the alias; every other ui file uses `@noctusai/lib/utils` (the canonical seed import). Fixed via ts-morph codemod in `/tmp/strict-mode/fix-textarea-import.ts`. No sed/regex on TS. Inline-applied; no follow-up project needed.
- **No `!` non-null assertions added.** No `any` added. No `@ts-ignore` or `@ts-expect-error` added.
- **Path mapping added for `@noctusai/seed/infra`** — even though the package's own `exports` field now declares it, the framework's tsconfig also lists `"@noctusai/seed/infra": ["./src/infra.tsx"]` as a defensive path map. Reasoning: a package importing itself via its own public name (`@noctusai/seed`) only works reliably when the package is npm-linked. The path-mapping is the deterministic fallback for `tsc --noEmit` running from the framework's own root.
- **No source code in `src/` was modified except the textarea import alias** — the strict flip's value is to surface contract bugs in framework code, and only one TS2307 fired at the framework boundary (the textarea import). The remaining errors all surfaced from path-mapped lib code, which Phase 2 already handles. Framework gets strict-from-day-one with effectively zero refactor cost.
