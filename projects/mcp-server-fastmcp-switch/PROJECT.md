...
- **Created:** 2026-05-03
- **Last updated:** 2026-05-03 (Phase 3 ✅ + Commit B landed `270a0ab` + Phase 4 re-scoped Path 2)
- **Status:** ⏳ **EXECUTING** — Phase 0 ✅, Phases 1+2 ✅ (Commit A landed `eb15d0b` 2026-05-03 — FastMCP switch + per-file register() + tools/__init__.py::register_all + _validate_one absorbed into compliance.py + server.py 475→63 lines + 550 tests pass + 60 tools registered with structurally-equivalent schemas), **Phase 3 ✅** (Commit B landed `270a0ab` 2026-05-03 — libcst-based import rewriter relocated 25 files of imports + 18 `parents[3]→[5]` depth bumps + 24 tool files `git mv`'d into `tools/noctus/dev/` + hierarchical 3-tier `register_all` + 4 hard-coded test paths fixed + 550/551 pytest pass + CLI smoke green + tool inventory diff: 60 == 60 byte-for-byte), **Phase 4 RE-SCOPED Path 2** (gate cleared by Tier 1 substrate ship; user direction 2026-05-03 keeps dev-agent vendor branches `google.calendar.*` + `google.maps.travel_estimate` + `openai.{audio,vision}.*` and drops `noctus.business.*` since substrate is consumed by products via direct Python import of `noctusai_lib`, not via MCP — re-wrapping = duplication; recurrence trigger documented if 2+ dev-agent uses surface). Phase 5 awaits Phase 4 ports. Predecessor `projects/mcp-server-expansion/` closes with Phases 4+5 deferred here. **Phase 3 close also triggers `projects/mcp-tool-name-deprecation/` Phase 1** — dotted-name aliases for the remaining 43 flat-only tools are now a one-line-per-file change (just add a second `server.tool(name="noctus.dev.<x>")(handler)` line in each `register()`).
...
GATE CLEARED 2026-05-03: `whatsapp-seed-absorption` shipped (8cf1a23) + `scheduling-engine-seed` shipped (64f48f9). Substrate now lives in `noctusai_lib.{integrations.{google_calendar,google_maps,whatsapp},domain.{scheduling,conversation}}` — not in the sibling repo. **Re-scope decision** (user 2026-05-03 FastMCP-switch session): keep the **dev-agent-useful** vendor branches (Claude Code + future dev bots benefit from direct calendar / maps / vision / transcription access during ad-hoc work). Drop the `noctus.business.*` ports — that substrate is consumed by products via direct Python import of `noctusai_lib`, not via MCP, and re-wrapping it would be duplication.

**In scope (Path 2):**

- [ ] Create `mcp/noctusai/context.py` — lazy `NoctusContext` cm holding adapter handles (Google Calendar API client, Google Maps client, OpenAI client). Sibling pattern; the `__aenter__`/`__aexit__` shape from `~/Documents/repository/NoctusAI/whatsapp-google-scheduling/mcp_server/context.py` already absorbed into `KB § PATTERNS/mcp-tool-conventions.md § 4. Lazy NoctusContext` — port from KB, not from sibling.
- [ ] Port `google.calendar.{create_event, get_event, list_events, delete_event}` (4 tools) — thin wrappers over `noctusai_lib.integrations.google_calendar.*` adapters.
- [ ] Port `google.maps.travel_estimate` (1 tool) — thin wrapper over `noctusai_lib.integrations.google_maps.travel_estimate`.
- [ ] Port `openai.audio.transcribe` + `openai.vision.identify` (2 tools) — direct OpenAI SDK wrappers; no seed-lib substrate yet (these are vendor leaves, not domain primitives, so they live in MCP without a `noctusai_lib` middle layer; if a second consumer surfaces, formalize per the recurrence rule).
- [ ] Each ported tool: Pydantic In/Out + dual-callable `_impl` + `register(server)`. New umbrellas land in `mcp/noctusai/tools/google/{calendar,maps}/<action>.py` + `mcp/noctusai/tools/openai/{audio,vision}/<action>.py` with hierarchical `register_all` mirroring the dev branch's pattern.
- [ ] Tests for each ported tool — Pydantic schema introspection + happy-path call against mock adapter.
- [ ] No sibling-path leaks (`grep -rIn "whatsapp-google-scheduling" mcp/noctusai/` returns 0).

**Out of scope (deferred per Path 2):**

- `noctus.business.{scheduling.suggest_slots, appointments.intervals_for_date, users.lookup_by_phone, whatsapp.send_text}` — substrate already in `noctusai_lib.domain.{scheduling,conversation}` + `noctusai_lib.integrations.whatsapp`. Products consume via direct Python import; MCP wrapper is duplication unless a dev-side / agent-side use case emerges. Logged in `KB § PATTERNS/accept-with-rationale.md` at re-scope time. Recurrence trigger: if 2+ dev-agent / Claude-Code workflows need to reach the underlying business primitive, flip to formalize and port.

### Phase 5 — Final verification + handoff

- [ ] `python mcp/noctusai/cli.py --validate` green.
- [ ] `pytest mcp/noctusai/tests/` green (Phase 4 ports add new test files; baseline + new should all pass).
- [ ] `bash scripts/verify-kb-sync.sh` green.
- [ ] `python mcp/noctusai/cli.py --review` triage.
- [ ] No sibling-path leak audit: `grep -rIn "whatsapp-google-scheduling" mcp/noctusai/` returns 0.
- [ ] Trigger `projects/mcp-tool-name-deprecation/` Phase 1 (Phase 0 already shipped 2026-05-03 in `d063feb`; dotted aliases for remaining 43 flat-only tools now trivial via per-file `register()`).
