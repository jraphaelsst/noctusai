...
- **Status:** **ALL 6 phases ✅ + cutover live.** 1 (safety-net doc) · 2 (de-track) · 3 (prod branch — **VPS repointed to `origin/prod` 2026-05-22**) · 4 (predeploy_check) · 5 (DB decision + **One Permutas RLS FIXED**) · 6 (Supabase MCP, live). `main` pushed (`dfaf6dd4`). Remaining = lower-priority: VPS `config.yml` Phase-2 migration (when prod is promoted past `a16eac85`) · C1/C2 corrective nets at next real deploy · PAT rotation · optional `.mcp.json` supabase registration · local `supabase/` cleanup.
...
- [x] **C1 (backup-ref) + C2 (atomic image rollback) scoped OUT of Phase 4 → VPS-runtime corrective nets, not local MCP tools.** They run *on the VPS during a deploy* (`git tag backup/predeploy-<utc>` + tar deploy-local; keep image `:previous` + swap on health-fail). Documented as ⏳ nets C1/C2 in `KB § GUIDES/production-deploy.md`; implemented at the next real VPS deploy (which is user-gated anyway). Named destination = the GUIDE C1/C2 nets + the deploy-time follow-up. *(Defer-with-destination, not silent skip — out-of-safe-scope: needs VPS deploy execution.)*
