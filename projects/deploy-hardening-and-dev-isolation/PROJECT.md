...
- **Status:** **ALL 6 phases ✅ + close-out deploy live (2026-05-22).** 1 (safety-net doc) · 2 (de-track — **VPS migration DONE**) · 3 (prod branch — VPS on `origin/prod`) · 4 (predeploy_check — **P5 interpreter false-block FIXED**) · 5 (DB decision + One Permutas RLS FIXED) · 6 (Supabase MCP, live). `main`/`prod`/VPS all at `79f19ccb`. **Close-out deploy:** promoted `79f19ccb` → `prod` → VPS via the §2a drill (C1 backup ref+tar; config.yml+creds migrated → working tree now clean; fleet healthy, no rebuild). **Safety matrix asserted P1–P5 / D1–D4 / C1 / C3 all live** (C2 awaits a product-code/image-rebuild deploy). Remaining = user-dependent: PAT rotation (pasted in chat) · optional `.mcp.json` supabase registration · local `supabase/` cleanup (decision pending) · **product-subdomain DNS** (social-wiring/erp/PF/therapy don't resolve — pre-existing rollout gap, surfaced) · C2 net at first image-rebuild deploy.
