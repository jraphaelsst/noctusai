...
- [ ] **DEPLOY (user-gated):** needs the one-permutas prod secrets (`SUPABASE_DB_URL` + keys, `CELERY_BROKER_URL`/`RESULT_BACKEND`) + the `legacy` A-record. NOT urgent — Permutas is still live on Replit; legacy is the future home for the apex→core cutover. THEN retire the Replit deployment.
