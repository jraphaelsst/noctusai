...
- **First-batch absorptions** identified by the scans (each is its own follow-up project):
  ...
  - `noctusai_lib.integrations.sql.<rls_template>` — absorbs `SET search_path` (176 occurrences across 5 products)
  - `noctusai_lib.integrations.sql.audit_trigger` / `updated_at_trigger` — absorbs trigger templates
