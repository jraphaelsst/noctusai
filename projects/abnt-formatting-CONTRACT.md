...
File scope: `products/social-wiring/frontend/src/pages/Matriculas.tsx`, `pages/Certidoes.tsx`, `components/CertidoesPartePanel.tsx`, `components/GeradorContratoContainer.tsx` (+ contract-version labels in `components/card/ContratosPanel.tsx` if they say docx), `hooks/useMatriculas.ts`, `hooks/useCertidoes.ts`, their tests, and ONE shared helper in the seed lib frontend: `copyRichText(html: string, text: string): Promise<{ rich: boolean }>` at `@noctusai/lib/clipboard` (ClipboardItem with `text/html` + `text/plain`; falls back to `writeText(text)` only when `ClipboardItem` is unavailable and then resolves `{ rich: false }`, so the caller can say so — never silently). *Amended 2026-09-14 from `Promise<void>`: the original signature could not express the required non-silent fallback (surfaced by S5).*
