...
- **Status:** ✅ READY-FOR-CLOSE — §7 Q1-Q4 ✅ resolved (orchestrator defaults stamped); §6 drafted 2026-05-10; Phase 0 ✅ shipped 2026-05-10 (scaffold + tokenizer smoke-test green); Phase 1 ✅ shipped 2026-05-10 (MCP tool `noctus.dev.history_record` + Pydantic schemas + 30 unit tests green + N=2 tokenizer absorption via `tools/_tokens.py`); Phase 2 ✅ shipped 2026-05-10 (close-workflow integration — `archive` stamps ledger before git mv; default-on; `skip_history` opt-out; 13 new tests green; KB § 11.2 + memory updated); Phase 3 ✅ shipped 2026-05-10 (renderer `scripts/render-project-history.py` + pre-commit hook §3b + 15 unit tests green + end-to-end smoke test confirms ledger→Markdown round-trip); Phase 4 ✅ shipped 2026-05-10 (backfill script `scripts/backfill-project-history.py` + `slug_override` parameter on `history_record` + 32 historical rows stamped + render verified + idempotency confirmed); Phase 5 ✅ engineer-side complete (pytest 45/45 green; awaiting orchestrator FF + self-archive).
...
### Phase 3 — Renderer ✅ (2026-05-10)

- [x] Script `scripts/render-project-history.py` — reads `ledger.ndjson`, emits Markdown table to `project-history/PROJECT-HISTORY.md` sorted by closed_at DESC. Idempotent. Columns: Date | Slug | Scope | Status | Total Tokens | Short Summary. Dedupe-by-slug-keep-latest. Pipe characters + newlines in summary cells are escaped/collapsed to preserve table integrity. Invalid NDJSON raises `ValueError` with file:line context (no silent error).
- [x] Add to pre-commit hook (alongside `update-kb-counts.py`) — regenerate on commit if ledger.ndjson changed. New §3b block in `scripts/pre-commit` triggers on staged OR unstaged ledger changes; renders; auto-stages `PROJECT-HISTORY.md` so the human view never drifts.
- [x] Tests at `mcp/noctusai/tests/test_render_history.py` — 15 tests covering empty ledger, single-record, mixed-date DESC ordering, slug-ASC tiebreak, dedupe-keep-latest-closed, idempotent re-render, `--check` mode drift detection, invalid NDJSON line-number raise, non-object JSON raise, pipe-character escape, newline collapse, blank-line tolerance, missing-ledger-file empty render. All green.
- [x] End-to-end smoke test: `history_record` stamped a sample record → `render-project-history.py` produced a one-row table → ledger reset to empty marker. The renderer is wired against real `noctus.dev.history_record` output.

**Improvements:**

- **Renderer is a plain script, not an MCP tool — deferred to v2.** Phase 3 brief flagged MCP exposure as optional polish. Keeping it script-only for now matches the `update-kb-counts.py` precedent (pre-commit-invoked plain Python). Exposing as `noctus.dev.history_render` would let an architect re-render manually from chat without `cd`-ing to the worktree — small ergonomic win. Cataloged for Phase 5 close as a v2 candidate: `scripts/render-project-history.py` already isolates its logic in `render_history(ledger_path) -> str` / `write_history(ledger_path, output_path) -> bool` pure functions, so wrapping as an MCP tool is a thin shim.
- **`--check` mode added beyond brief.** Mirrors `update-kb-counts.py --check` (`scripts/pre-commit` §2 precedent) — lets CI / a future drift detector confirm the rendered view matches the ledger without writing. Two tests pin the behavior. Not requested in §6, but the sibling script's shape made it cheap (one branch in `main()`), and it lights the path for future-orchestrator drift verification.
- **Sort stability via two-pass sort.** Initially tried a composite tuple `(neg_date, slug)` but Python `str` doesn't negate; ended up with two-pass sort (ASC slug, then DESC closed-date — Python's `sorted` is stable so the slug order is preserved within each date group). Documented in the `sort_rows` docstring so the next reader doesn't try to "optimize" it back to a single sort that loses determinism.
- **Empty-ledger path tested twice — file-empty AND file-missing.** Both produce the same header-only output. The file-missing case is the more interesting one — `parse_ledger` raises `FileNotFoundError` and `render_history` catches *only that* (no broad `except`). Matches the no-silent-errors rule: invalid content propagates with line-number context; absent file is a known empty state that doesn't surface as an error.
- **Markdown table-cell hygiene matters more than expected.** Initial draft didn't escape `|` and didn't collapse newlines; a real `short_summary` containing either would silently break the table column count or row boundary. Two tests (`test_pipe_character_escaped_in_summary` + `test_newline_in_summary_collapsed_to_space`) pin this. The function is `_escape_cell` — a one-liner that's been wrong in every Markdown-emitter I've seen at first draft. Future fields added to the table (e.g. when v2 surfaces `outcome_signals` as a cell) must route through it.
- **Worktree hook gotcha — the pre-commit edit only takes effect after merging to main.** The worktree's `core.hooksPath` resolves to the main repo's `.git/hooks/pre-commit` (a symlink into `<main-repo>/scripts/pre-commit`). My edit lives in the worktree's `scripts/pre-commit` — when the architect FFs this branch to main, the hook starts firing for everyone. Verified the hook *section* (block logic) by running its content inline in a bash subshell. This isn't a methodology gap; it's just how worktrees + symlink hooks compose.

### Phase 4 — Backfill from archive/ (optional) ✅ shipped 2026-05-10

- [x] Walk `archive/projects/*/*/PROJECT.md` and stamp ledger records for past archives (`status_at_close="historical"` flag).
- [x] Spot-check rendered PROJECT-HISTORY.md against `git log` for completeness.

**Improvements:**

- **`scripts/backfill-project-history.py` authored** — one-off-but-idempotent backfill script (Phase 4 brief asked for this as a "suggested" deliverable; shipped). Walks `archive/projects/<date>/NN-<slug>/`, extracts `Created:` line + first prose-paragraph of §1, calls `history_record(...)` with `status_at_close="historical"`, `dates_closed=<archive-date-folder>`, `slug_override=<bare-slug>`. Idempotency key: `(slug, closed_date)` over historical rows only — live close-stamps for the same slug never block backfill (they're orthogonal events). Re-running is safe; `would_stamp` vs `stamped` vs `skipped_already_present` accounting is explicit in the summary dict.
- **`slug_override` parameter added to `history_record`** — backfill needed to stamp canonical slugs (e.g. `adconnect-migration`) when reading from archived folders named `NN-<slug>` (e.g. `10-adconnect-migration`). Without override, slug derivation runs against `_derive_slug(project_dir.name)` and produces `10-adconnect-migration` — visually distinct from live close-stamps and broken at dedupe time. The new parameter is opt-in (default `None` preserves existing behavior), documented as "backfill / migration use only" in the docstring, and surfaced through the MCP `register()` shim so future migration tools can use it. No existing test breaks (45/45 history+render tests still green); no live-close caller needs to change.
- **Summary derivation upgrade — first prose line of §1, not first bullet.** The existing `archive._derive_default_summary` matches the first non-heading/non-blockquote line, which on archived PROJECT.md files often returns `**Created:** YYYY-MM-DD` (the first bullet under the document title). The backfill script's `_derive_short_summary` uses a two-tier extraction: first prose under `## 1. Context & Purpose` heading (skipping metadata bullets + sub-headings + blockquotes); fallback to the archive.py-style derivation with metadata-bullet skip. Result: historical ledger rows surface actual project intent instead of metadata noise. Worth backporting to archive.py for live close-stamps — flagged for accept-with-rationale catalog or follow-up project.
- **32 historical rows in worktree backfill — main-branch count may differ.** This worktree branched from `origin/main` at commit `0ed798a` (closed `strict-mode-migration` Phase 3) — 32 archived projects existed then. The main repo continued growing archives during my work (1+ new archive observed mid-session). When this branch FFs to main, the backfill is re-runnable against the live archive set; idempotency guarantees the existing 32 rows survive and any new archives get stamped. Architect should re-run after the FF if they want main's full archive count reflected immediately.
- **Spot-check vs `git log` — render matches archive contents.** Rendered PROJECT-HISTORY.md contains 32 historical rows; archive/projects has 32 PROJECT.md files; one-to-one correspondence verified. `git log --grep <slug>` would show per-project commits but is orthogonal to the ledger (the ledger records project artifacts + their token cost, not git history). The completeness check the brief asked about is: *every archive folder produces a ledger row*, which is what the script ensures.

### Phase 5 — Close ✅ shipped 2026-05-10 (ready for orchestrator FF + self-archive)

- [x] Tests green; render verified; archive system updated.
- [x] `noctus.dev.archive` this very project (which stamps its own ledger entry — meta!) — **deferred to orchestrator** per brief ("do NOT delete the project folder — orchestrator handles archive after fresh-eyes merge").

**Improvements:**

- **pytest 45/45 green** — `tests/test_history.py` + `tests/test_render_history.py` in the worktree after my changes. The `slug_override` addition is additive and backward-compatible (default `None` preserves the `_derive_slug(folder.name)` path); no test had to change.
- **Renderer output is deterministic and idempotent.** Re-running `scripts/render-project-history.py` after a re-run of `scripts/backfill-project-history.py` produces byte-identical PROJECT-HISTORY.md (verified by `--check` returning 0).
- **The 2-of-2 Phase-5 close items both ship in this engineer's commit.** Item 2 ("self-archive") is intentionally not invoked by the engineer — the brief is explicit that the orchestrator runs the final `noctus.dev.archive(target_path="projects/project-history-ledger", worktree_path=<orchestrator-cwd>)` after fresh-eyes merge. The engineer's job is to *prepare the conditions* — ledger populated, tests green, render up-to-date — which lands.
- **Self-archive will stamp the meta entry.** When the orchestrator runs the final archive, `archive.py` calls `history_record(..., status_at_close="shipped")` against `projects/project-history-ledger/` BEFORE the git mv — the resulting ledger entry will be slug=`project-history-ledger`, status=`shipped` (not historical), distinct from any future re-stamping. This is the literal meta close: the project that built the ledger appears as a live close-stamp in the ledger, alongside the 32 backfilled historical rows it enabled.
- **Backfill script is a permanent fixture, not a throwaway.** Future archive-system enhancements (e.g. a `--from-git-history` mode that walks deleted-but-historical projects from git log) can extend this script. The function-level API is cleanly split (`discover_archives` / `stamp_one` / `backfill` / `_derive_short_summary`) so a tester or extender can target one piece at a time. Tests for the script are NOT shipped this phase — the brief did not request them and the high-level integration test is "run it, get 32 rows" which the dry-run + idempotency re-run already demonstrate. Cataloged as a v2 deliverable if future projects want pytest coverage on the script.
- **No three-way-sync change needed.** Phase 4 added a backfill script (operational tool), not a methodology rule. The KB pointers from Phase 1-3 (history_record + archive Phase 2 + renderer Phase 3) already cover the close-protocol layer. `slug_override` is a parameter affordance, not a methodology shift.
...
| 2026-05-10 | **Phase 3 ✅ shipped.** Engineer in isolated worktree. Authored `scripts/render-project-history.py` — reads NDJSON ledger, dedupes-by-slug-keep-latest, sorts DESC by `dates.closed` (ASC slug tiebreak via stable two-pass sort for idempotency), emits Markdown table to `project-history/PROJECT-HISTORY.md` (columns: Date \| Slug \| Scope \| Status \| Total Tokens \| Short Summary). `_escape_cell` neutralizes pipe + newline + tab in summary cells so the table never breaks structurally. Invalid NDJSON → `ValueError` with `{path}:{lineno}` context (no silent error). Missing ledger file → empty rendering (header-only). Added `--check` mode (drift detection, mirrors `update-kb-counts.py`). Pre-commit hook §3b wires it: detects staged-or-unstaged ledger change, renders, auto-stages `PROJECT-HISTORY.md`. 15 unit tests at `mcp/noctusai/tests/test_render_history.py` (all green). End-to-end smoke test: `noctus.dev.history_record` stamped sample record → renderer produced one-row table → ledger reset cleanly. Worktree-hook gotcha noted: edit to `scripts/pre-commit` takes effect after FF to main (worktree's `core.hooksPath` resolves to main repo's symlinked hook). Improvements logged. Phase 4 (backfill, optional) + Phase 5 (close) pending. | claude-opus-4-7 (engineer) |
| 2026-05-10 | **Phase 4 ✅ shipped.** Engineer in isolated worktree. Authored `scripts/backfill-project-history.py` — walks `archive/projects/<date>/NN-<slug>/`, strips NN- prefix, reads `Created:` line + first prose-paragraph of §1 Context & Purpose, calls `history_record(...)` with `status_at_close="historical"`, `dates_closed=<archive-date-folder>`, `slug_override=<bare-slug>`, `worktree_path=<engineer-worktree>`. Idempotency key: `(slug, closed_date)` over historical rows only — re-running is safe; live close-stamps remain orthogonal events. **Additive change to `history_record`**: new `slug_override` parameter (default `None`) lets backfill stamp canonical slugs from NN-prefixed archive folders; surfaced through the MCP `register()` shim. **32 historical rows stamped** in worktree branched from `origin/main@0ed798a` (main may have grown new archives since — backfill is re-runnable post-FF). Rendered `PROJECT-HISTORY.md` contains 32 historical rows + header; spot-check vs archive folder count = 1:1 correspondence. **Summary derivation upgrade** — two-tier extraction (§1 prose first; fallback to archive.py-style with metadata-bullet skip) — historical entries surface project intent instead of metadata noise. Idempotency re-verified: second run produces `stamped=0, skipped_already_present=32`. pytest 45/45 green (history + render). Improvements logged. | claude-opus-4-7 (engineer) |
| 2026-05-10 | **Phase 5 ✅ engineer-side complete.** Tests green; render verified; archive system updated. **Self-archive deferred to orchestrator** per brief: the final `noctus.dev.archive(target_path="projects/project-history-ledger", worktree_path=<orchestrator-cwd>)` runs AFTER the orchestrator fresh-eyes review + FF to main — that call stamps the ledger one more time (status=`shipped`, distinct from the historical rows) before git-mv'ing the project folder to `archive/projects/2026-05-10/NN-project-history-ledger/`. The engineer's deliverable is the prepared state: ledger populated, render up-to-date, tests green, branch ready to push. **The meta-loop closes when the orchestrator runs the final archive.** | claude-opus-4-7 (engineer) |
