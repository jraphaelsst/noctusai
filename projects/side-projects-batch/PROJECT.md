...
**1.d `mcp-scaffold-sql-templates-integration` — drive to ✅.**
- [ ] Phase 0 audit: read `mcp/noctusai/tools/scaffold.py` end-to-end + its tests; determine current SQL-emission behavior; design integration shape.
- [ ] Apply integration; refactor scaffold-emitted SQL through `noctusai_lib.domain.sql_templates` helpers.
- [ ] Tests asserting helper-emitted output shape; full MCP suite green.
