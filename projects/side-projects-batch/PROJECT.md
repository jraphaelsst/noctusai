...
**1.e `therapy-consent-frontend-wiring` — drive to ✅. ✅ (closed 2026-05-03)**
- [x] Phase 0 audit — found existing `ConsentPopup.tsx` was UI-only (didn't call `/api/consents/grant`), so every session-start would 400 against the post-Phase-5 backend gate. `Session.tsx` phase machine had `consent` → `active` flow but bypassed the new persistence requirement. Both paths (patient self-serve + therapist-attest) are reachable from the same waiting-room surface — Q6's "both paths" answer fits cleanly with one shared modal + actor-aware copy.
- [x] Built `src/hooks/useConsents.ts` — `useConsents(appointmentId)` query + `useGrantConsent` / `useRevokeConsent` mutations + `getActiveConsent(records, scope)` helper + `CONSENT_POLICY_VERSION` constant + default purpose-text. Toast on error per platform convention.
- [x] Updated `ConsentPopup.tsx` — accepts `actor: 'patient' | 'therapist_attesting'` + `isPending` props; renders patient-facing self-consent copy OR therapist-attest copy (Q6). Disabled state respects in-flight grant/start mutation. Button label flips between "Aceito" and "Atestar e Iniciar".
- [x] Updated `Session.tsx` — patient view in waiting room shows "Conceder consentimento de gravação" button when no active recording consent (Q6 self-serve), confirms "Consentimento de gravação registrado." otherwise. Therapist `handleStartClick` now skips the consent modal entirely if a recording consent already exists (patient pre-served), else opens the modal in `therapist_attesting` mode. `handleConsentAccept` now calls `useGrantConsent.mutateAsync` first (idempotent — only fires when `recordingConsentGranted` is false), then for therapist also fires `useStartSession`. Error handling stays on the consent phase to allow retry.
- [x] Built `src/components/session/ConsentHistoryView.tsx` — lists all records (active + revoked) with `ShieldCheck`/`ShieldOff` icons; revoked rows render strike-through + "Revogado em <data>" badge per Q8; "Revogar" button per active row opens `AlertDialog` confirm before firing the revoke mutation; `canRevoke` prop allows read-only mounts. Mounting decision deferred to `therapy-platform-wiring` project (admin/patient/therapist surface decisions are its scope).
- [x] **Therapy frontend `vite build`: green** (Session chunk grew from ~16 kB to 19.74 kB, gzip 6.51 kB). **Backend regression on consent + sessions + lifecycle: 53/53 passed**. Keeper: 0 issues.
- [x] **No new Vitest test added** — therapy frontend has zero Vitest test files today (`No test files found`). Adding the first test surface (mock api shim + QueryClient wrapper) would be scope creep; deferred. Captured in this batch §11 as a candidate for a future "first-frontend-test" project. The 3 backend regression guards from Phase 1.b cover the cross-stack consent contract.
- [x] **Test-helper recurrence finding from Phase 1.b** (`_seed_room_and_segments` shape) — N=2 still; consent UI work was frontend-only with no new room/segment fixture seeding, so the recurrence count did not advance. Pattern stays in observation mode until a 3rd repeat fires.
- [x] Close-phase commit pending.
- [x] Child folder deletion pending.
...
**2.b `erp-schema-drift-reconciliation` — drive to ✅. ✅ (closed 2026-05-03 with EXPAND-LOUDLY scope split)**
- [x] Phase 0 — live-DB inspection via Supabase MCP against `erp` schema. **Findings widened the scope substantially**: 11+ ERP tables (`ativos`, `clientes`, `profiles`, `metas`, `agenda`, `imoveis`, `site_imoveis_config`, `whatsapp_settings`, `whatsapp_etiquetas`, `certidoes_consultas`, `financeiro`) DO NOT have `org_id` column, but code queries them by `org_id` in 20+ sites. Plus `profiles.avatar_url` references (3 sites) target a column that doesn't exist (live has `avatar`).
- [x] **Security concern surfaced at Phase 0**: `app/routers/profiles.py:115` reads `org_id` from `profiles` for the cross-org access guard before `DELETE /api/profiles/<id>`. Because the column doesn't exist, `target_org` is always `None`, the cross-org check is silently bypassed, and admins of one org can `auth.admin.delete_user(...)` users of another org. Documented in the follow-up project's §1.
- [x] **Phase 1 narrow ship — 3 unambiguous `avatar_url` → `avatar` sites fixed:** `routers/gamificacao.py:74` (select), `:82` (response stays `avatar_url` for frontend back-compat with note), `routers/profiles.py:28` (Pydantic `ProfileUpdate.avatar` with `alias="avatar_url"` + `populate_by_name=True` so frontend can submit either). Pydantic `Field` import added. **ERP backend: 1816/1816 passed; profiles+gamificacao routers: 26/26 passed.**
- [x] **Filed follow-up project** `projects/erp-schema-drift-deep-audit/PROJECT.md` covering: (a) the wider org_id audit + design decision (add column vs. rewire query path), (b) per-table migration + backfill plan, (c) the `profiles.py:115` security fix path, (d) LGPD checklist. Phase 0 interrogation pending.
- [x] **`mock-supabase-schema-validation` re-enable on ERP** is part of the deep-audit follow-up's success criteria, not this child's. The narrow scope here was satisfied by the unambiguous renames + escalation paperwork.
- [x] Close-phase commit + child folder deletion at end of Tier 2.

**2.c `keeper-warning-triage` deferred phases — drive to ✅. ✅ (closed 2026-05-03 — already cleared by intervening work)**
- [x] **Phase 0 audit finding**: `python mcp/noctusai/cli.py --review` reports **0 issues / 0 proposals** at 2026-05-03. The 407 deferred warnings tracked at filing time (Phases 2-6: core/mailing/PF/daily-life/adconnect tests cleanup, seed-lib silent-error cleanup, detector refinement) have all been cleared by intervening work — primarily `projects/execution-workflow-codequality-rollout/` Phase 2 (therapy `test_messaging_router.py` 12/40 + monkey-patch sweeps) and the `# silent-ok` retirement landed in `feedback_silent_ok_is_not_a_substitute_for_logging.md`.
- [x] No code change needed to close. Phases 3-6 §6 sub-tasks are effectively no-ops because the warning queue is already empty.
- [x] **Phase 6 detector refinement** (return-value-as-surface) — original triage's design idea; with 0 warnings active, refinement has no signal to calibrate against. Captured here as a candidate for `execution-workflow-codequality-rollout` Phase 5 (calibration revisit) if a future warning class fires.
- [x] Close-phase commit + child folder deletion at end of Tier 2.

### Phase 3 — Tier 3 batch (larger / multi-phase rollout)

**3.a §7 interrogation round (gates Phase 3.b). ✅ (closed 2026-05-03 — no user input needed)**
- [x] Phase 0 audit found Phase 3 + Phase 4 part 1 already done by intervening work — no §7 round needed since the only remaining substantive work (digest absorptions) is too large for inline absorption and gets its own follow-up project.

**3.b `execution-workflow-codequality-rollout` deferred phases — drive to ✅. ✅ (closed 2026-05-03)**
- [x] Phase 3 audit: `grep -rn "monkeypatch.setattr" products/therapy-platform/backend/tests/` returns **0 hits**. The 32 sites the child planned to clean (in `test_invitations_router.py`, `test_e2e_flows.py`, service tails) have all been cleaned by intervening work — primarily Phase 2 of this same project plus the platform-wide `# silent-ok` retirement. Phase 3 closes as already-done.
- [x] Phase 4 audit: `assert_error_contains` is already present at `noctusai_lib.testing` (verified import at `products/therapy-platform/backend/tests/routers/test_messaging_router.py:17` + 3 call sites). The Phase 4 absorption candidate the child explicitly named at filing time has shipped. Phase 4 partial-close for that scope.
- [x] Phase 5 calibration: re-ran `noctusai_scan_cross_product_helpers`. **Standing recurrence patterns confirmed (no Phase 4 absorption shipped between Phase 1.b's scan and now)**: `_render_bodies` (N=5: core/daily-life/erp-imobiliario/mailing/personal-finance), `_generate_narrative` (N=4: core/daily-life/mailing/personal-finance — ERP excluded per existing `KB § PATTERNS/accept-with-rationale.md` § ERP metas digest entry), `_TEMPLATE_DIR` Path expression (N=5). HTTPException shape recurrences (4-5 products each at warning severity) are an established cross-cutting pattern — not in this project's scope.
- [x] **Filed follow-up project** `projects/digest-helpers-absorption/PROJECT.md` covering the `_render_bodies` + `_generate_narrative` formalization (target: `noctusai_lib.domain.digest` primitives + 4 product migrations). The two recurrences are substantive enough that absorbing them inline in this batch's Phase 3.b would be scope creep — they each need their own design phase + migration cadence.
- [x] **MCP suite: 475 passed (1 skipped); ERP backend: 1816/1816 passed; therapy backend: 1148/1148 passed; keeper review: 0 issues.**
- [x] Close-phase commit + child folder deletion at end of Tier 3.
...
| 2026-05-03 | **Phase 1.d ✅ — `mcp-scaffold-sql-templates-integration` execution complete.** Phase 0 audit of `mcp/noctusai/tools/scaffold.py` (89 lines, template-copy + placeholder-substitution architecture) surfaced an **EXPAND-LOUDLY pre-existing bug**: the bundled template `templates/product-seed/backend/migrations/001_seed.sql` hardcoded literal `seed.` instead of using the `{{SCHEMA_NAME}}.` placeholder, so every previously-scaffolded product would have produced migrations referencing the wrong schema (the template's own seed schema). Fixed in-flight per the "Phase 0 — expand loudly" rule. Same migration template now also carries a `SET search_path = {{SCHEMA_NAME}}, public;` prelude that matches `set_search_path()` output. 3 new colocated regression tests (`TestSqlTemplatesIntegration`) at `mcp/noctusai/tests/test_scaffold.py` assert (a) scaffolded `SET search_path` matches helper output, (b) scaffolded RLS policy matches `rls_subquery_policy(...)` output (whitespace-normalized), (c) no `seed.` literal leaks into scaffolded migration. **MCP suite: 475 passed (+3 new); keeper: 0 issues.** Phase committed as `1d39f28`. Pause point at end: another agent now active in `projects/` (created empty `projects/template-workspace/`); user confirmed they're working on a different batch — no collision risk. | Claude Opus 4.7 |
| 2026-05-03 | **Phase 1.e ✅ — `therapy-consent-frontend-wiring` execution complete (Tier 1 closed).** Phase 0 audit found the existing `ConsentPopup.tsx` was UI-only — every session-start would 400 against the post-Phase-5 backend gate. New `src/hooks/useConsents.ts` carries `useConsents` query + `useGrantConsent`/`useRevokeConsent` mutations + `getActiveConsent` helper. `ConsentPopup.tsx` extended with `actor: 'patient'|'therapist_attesting'` prop driving copy + button labels (Q6 = both paths). `Session.tsx` waiting-room shows "Conceder consentimento de gravação" button on the patient view when no active consent (self-serve), confirms "Consentimento de gravação registrado." otherwise; therapist `handleStartClick` skips the modal when consent already granted; `handleConsentAccept` now calls `grantConsent.mutateAsync` first (idempotent), then for therapist also fires session-start. New `ConsentHistoryView.tsx` lists all records (active + revoked) with `ShieldCheck`/`ShieldOff` icons + strike-through + "Revogado em <data>" badge (Q8) + revoke-confirm `AlertDialog`; mounting decision deferred to `therapy-platform-wiring` project. **Therapy frontend `vite build`: green; backend consent+sessions+lifecycle: 53/53 passed; keeper: 0 issues.** No new Vitest test (therapy frontend has no test files today; first-test infra would be scope creep) — captured as deferred candidate for a future "first-frontend-test" project. **Tier 1 fully closed: 4/4 children complete, 4 commits (`4c50b02`, `84c585b`, `1d39f28`, `9bfae8b`).** | Claude Opus 4.7 |
| 2026-05-03 | **Tier 2 closed (2/2 children) — Phase 2.b + 2.c.** **2.b** `erp-schema-drift-reconciliation`: Phase 0 live-DB audit (Supabase MCP project `nyplttplcoyiiqjrvtiw`) confirmed 11 ERP tables lack `org_id` while code queries them by it at 20+ sites; `profiles.avatar_url` references (3 sites) target a column that's actually `avatar` in live DB. **Security concern surfaced**: `routers/profiles.py:115` cross-org guard silently bypassed because `target_profile.data.get("org_id")` is always None on a non-existent column — admin of org A can `DELETE` profiles in org B. Narrow Phase 1 ship: 3 unambiguous `avatar_url`→`avatar` sites fixed (`gamificacao.py:74`/`:82` keeps response key `avatar_url` for FE back-compat; `profiles.py:28` Pydantic `ProfileUpdate.avatar` with `alias="avatar_url"` + `populate_by_name=True`). ERP backend 1816/1816 passed. Filed follow-up `projects/erp-schema-drift-deep-audit/PROJECT.md` with the security concern + wider org_id audit + design questions; user §7 sign-off pending before any migration. **2.c** `keeper-warning-triage` Phases 2-6: Phase 0 audit found `cli.py --review` reports **0 issues** today — the 407 deferred warnings have all been cleared by intervening work (`execution-workflow-codequality-rollout` Phase 2 monkey-patch sweeps + `# silent-ok` retirement). No code change needed; project closes as-already-done. Detector-refinement (Phase 6) ideas captured as candidates for `execution-workflow-codequality-rollout` Phase 5 calibration revisit. Tier 2 committed as `eb460fc`. | Claude Opus 4.7 |
| 2026-05-03 | **Tier 3 closed (1/1 child) — Phase 3.b.** `execution-workflow-codequality-rollout` deferred phases 3-5 resolved: Phase 3 (therapy test monkeypatch cleanup) already-done — `grep -rn "monkeypatch.setattr" therapy/tests/` returns 0 hits, sites cleaned by Phase 2 of same project + `# silent-ok` retirement. The `assert_error_contains` absorption candidate shipped to `noctusai_lib.testing`, used at `test_messaging_router.py:17,290,463,482`. Phase 5 calibration scan reran: standing N≥4 recurrences are `_render_bodies` (5p), `_generate_narrative` (4p; ERP excluded per accept-with-rationale), `_TEMPLATE_DIR` Path (5p). Filed `projects/digest-helpers-absorption/PROJECT.md` covering the digest formalization (target: `noctusai_lib.domain.digest` primitives + 4 product migrations) — substantive enough to need its own design phase; absorbing inline here would be scope creep. All 7 in-scope children of side-projects-batch now closed across 3 tiers. Project-close protocol next. | Claude Opus 4.7 |
