...
These artifacts live in the **live** deploy tree (`deploy/legacy/`), beside
`deploy/fleet`, `deploy/tunnel` and `deploy/services`. They were promoted here on
2026-08-26 from `archive/projects/2026-05-23/01-production-deploy-migration/`,
where they had been archived as project history despite being load-bearing
production infrastructure — a security rebuild required digging them out of an
archived project while the VPS copy had been cleaned down to just `.env`.
...
NOC=/opt/noctus/noctusai/deploy/legacy
