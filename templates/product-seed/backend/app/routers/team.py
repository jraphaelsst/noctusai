...
Seed has only two roles: admin and member.  There is no product-specific
...
def _require_seed_admin(user) -> None:
...
async def listar_membros(authorization: Optional[str] = Header(None)):
    ...
    # Map org_role to Seed role labels for display
...
async def convidar_membro(body: InviteBody, authorization: Optional[str] = Header(None)):
    ...
    role_label = SEED_ROLE_LABELS.get(body.role, body.role.capitalize())
...
async def validar_convite(token: str):
    ...
    return success_response({
        "email": invite["email"],
        "role": SEED_ROLE_LABELS.get(invite["role"], invite["role"]),
        "org_name": org_name,
    })
