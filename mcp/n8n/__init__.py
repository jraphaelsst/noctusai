...
`X-N8N-API-KEY`) through the single `n8n.api.request_json` HTTP seam
(stdlib `urllib` — no extra deps, mirrors how `mcp/github` wraps the
stdlib `subprocess` `gh` seam). The API key is a secret and lives in
the connector's own co-located `mcp/n8n/.env` (gitignored) — NOT the
product/root `.env` — per the "every connector owns its own auth
store" design principle (`KB § INTEGRATIONS/vista.md § 1`).

Writes (`activate` / `deactivate`) follow the confirm-then-execute
gate (status 412, no side-effect). Missing config / API failure is a
typed, never-faked signal (gated-capability honesty, CLAUDE.md §1).
