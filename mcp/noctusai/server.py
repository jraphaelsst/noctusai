...
def _tool(name, desc, props=None, required=None, *, model=None):
    """Build a Tool descriptor.

    When `model` (a Pydantic `BaseModel` subclass) is passed, its
    `model_json_schema()` is the input schema — the documented Phase 2+
    pattern for new and migrated tools. Otherwise we fall back to the
    legacy hand-coded `props` / `required` dict.
    """
...
async def list_tools():
    # Lazy imports keep server.py import-time light; tool modules pull in
    # heavy deps only when list_tools()/call_tool() actually fire.
    from tools.context import AgentContextInput, ProductContextInput
    from tools.compliance import ValidateInput
    from tools.analyzers import AnalyzePatternsInput
    from tools.review import ReviewInput
    from tools.catalog import CatalogInput

    slug_param = {"slug": {"type": "string", "description": "Product slug"}}
    return [
        # Context
        _tool("noctusai_agent_context", "Full platform context for an agent starting fresh. Call FIRST.", model=AgentContextInput),
        _tool("noctus.dev.agent_context", "Dotted alias for noctusai_agent_context. Prefer the dotted form going forward (Phase 3+).", model=AgentContextInput),
        _tool("noctusai_product_context", "Everything needed to work on a product: structure + MASTER-PROMPT + README", model=ProductContextInput),
        _tool("noctus.dev.product_context", "Dotted alias for noctusai_product_context.", model=ProductContextInput),
        ...
        # Compliance
        _tool("noctusai_validate", "Check seed compliance for all products. Returns score 0-100.", model=ValidateInput),
        ...
        _tool("noctusai_analyze_patterns", "Find duplicated functions and inline hooks", model=AnalyzePatternsInput),
        _tool("noctus.dev.analyze_patterns", "Dotted alias for noctusai_analyze_patterns.", model=AnalyzePatternsInput),
        ...
        _tool("noctusai_catalog", "Regenerate the shared-library catalog: every lib symbol, its importers, orphans (zero consumers), and duplication candidates (same name in 2+ products, not in lib). Writes mcp/noctusai/catalog.md.", model=CatalogInput),
        _tool("noctus.dev.catalog", "Dotted alias for noctusai_catalog.", model=CatalogInput),
        ...
        _tool("noctusai_list_promotions", "List promotion manifests in the current workspace, split into pending (`promoted_on=not-yet`) vs promoted (with date). Reads `.promotions/*.md`. See KB § PATTERNS/seed-workspace.md."),
    ]
...
def _dispatch(name, args):
    ...
    dispatch_map = {
        ...
        "noctusai_promote_from_seed_workspace": lambda: promotion.promote_from_seed_workspace(
            slug=args["slug"],
    ...
    # Pattern: `noctus.dev.<action>` dispatches to the same handler as the
    # legacy `noctusai_<action>`. Both names ship until a separate
    # alias-deprecation project retires the flat ones.
    aliases = {
