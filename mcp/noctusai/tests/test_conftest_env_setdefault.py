...
def test_the_incident_verbatim():
    """🔴 THE line. If this stops flagging, the class is live again."""
    found = _check('os.environ.setdefault("ASAAS_WEBHOOK_TOKEN", "token-de-teste")')
    assert len(found) == 1
...
def test_credentials_and_endpoints_are_all_covered():
    for key in (
        "SUPABASE_URL", "SUPABASE_ANON_KEY", "SUPABASE_SERVICE_ROLE_KEY",
        "ENCRYPTION_KEY", "ASAAS_API_KEY", "ASAAS_BASE_URL",
        "P_STUDIO_ORG_ID", "STRIPE_ACCESS_TOKEN",
    ):
        assert _check(f'os.environ.setdefault("{key}", "x")'), key


def test_a_locally_generated_session_key_stays_exempt():
    ...
    assert _check('_os.environ.setdefault("SESSION_ENCRYPTION_KEY", _Fernet.generate_key().decode())') == []
    assert _check('_os.environ.setdefault("REDIS_SESSION_ENCRYPTION_KEY", _k)') == []


def test_an_unrelated_key_is_not_flagged():
    assert _check('os.environ.setdefault("PYTHONHASHSEED", "0")') == []
    assert _check('os.environ.setdefault("TZ", "UTC")') == []


def test_the_prefixed_alias_form_is_caught_too():
    ...
    assert _check('_os.environ.setdefault("SUPABASE_URL", "http://test.local")')


def test_plain_assignment_is_what_we_want_and_is_never_flagged():
    assert _check('os.environ["ASAAS_WEBHOOK_TOKEN"] = "token-de-teste"') == []
    assert _check('own_test_env({"ASAAS_WEBHOOK_TOKEN": "token-de-teste"})') == []


def test_the_line_number_points_at_the_offending_call():
