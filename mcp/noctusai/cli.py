...
def main():
    ...
    parser.add_argument("--check-contract-field-provenance-map", action="store_true", help="social-wiring's F5 contract generator (card_hub/contrato_gerador) emits ~140 distinct dotted placeholder tokens (AST-derived: modelo_texto.py's TEMPLATE string + contexto.montar_contexto's + derivacao.derivar_switches's return-dict keys — no hand-maintained list on the code side); every token must have a provenance row (clause, source document type, extractor code path, confirmed storage, confirm/promote step, status) in the literal § 3 token-inventory table of KNOWLEDGE-BASE/CONTEXT/PRODUCTS/social-wiring/CONTRACT-FIELD-PROVENANCE-MAP.md. Severity high. KB § CONTEXT/PRODUCTS/social-wiring/CONTRACT-FIELD-PROVENANCE-MAP.md § 3.")
    parser.add_argument("--check-contextualize-alignment", action="store_true", help="Keeper: CONTEXTUALIZE.md is the fresh-agent read map and must remain pointer-only (sibling discipline to check_claude_md_router). Enforces (a) file exists at repo root, (b) line cap, (c) every canonical-cores entry is referenced, (d) §4 roster lists every on-disk skill/command/agent and no phantom ones, (e) count claims equal the derived roster; no hand-written MCP tool count. Severity high.")
    ...
    parser.add_argument("--paths", default="", help="Comma-separated repo-relative paths to narrow a keeper's sweep (currently --check-conflict-markers, --check-git-hooks-bypass, --check-git-hook-file-tampering, --check-storage-bucket-public, --check-migration-guard-has-probe, --check-schema-wide-anon-grant, --check-piped-exit-code-pattern). Omitted, the keeper scans its full default scope.")
    ...
    parser.add_argument("--check-stranded-branches", action="store_true", help="Keeper (advisory): remote work branches holding commits NOT on `dev` whose newest commit is older than 7 days. Counts by PATCH-EQUIVALENCE (`git cherry`), never `rev-list` distance — on 2026-09-19 two of four 'unmerged' branches carried nothing at all (content already on dev, byte-identical), and a naive ahead-count reports those as pending work forever, which trains people to ignore the signal. The one branch that DID carry work had an approach superseded the next day by a production incident fix, so merging it on autopilot would have reverted that fix. Advisory by design: a long-running branch is not a defect, and a blocking gate here gets routed around. Severity warning. KB § PATTERNS/common/branching.md.")
    parser.add_argument("--check-mcp-tools-reject-unknown-args", action="store_true", help="Keeper: `build_server()` must call `install_strict_tool_arguments(server)` AFTER `register_all`. FastMCP registers its handler with validate_input=False, so the lowlevel server never validates a call against the advertised inputSchema and pydantic drops undeclared fields — every parameter then falls back to its default. For `noctus.dev.task_branch` that default is the read-only `action='status'`, so a caller typo (`payload=`, `slugg=`) returns ok:true with a full listing and a failed start/integrate/cleanup is indistinguishable from success (2026-09-19 incident). Installing BEFORE register_all is also blocked: the registry would be empty and the gate would pass everything while appearing wired. Severity high. KB § PATTERNS/common/gate-methodology-sync.md.")
    ...
    parser.add_argument("--check-seed-test-root-ci-coverage", action="store_true", help="Keeper: every seed/*/backend with pytest files and every seed/*/frontend with vitest specs must have a workflow step that RUNS that suite. Sibling of --check-ci-test-matrix-coverage, whose domain is products/* and so could never see these. 2026-09-09: seed/lib/backend (3637 tests), seed/framework/backend (203) and seed/lib/frontend (375) had NO job at all — the packages every product imports were the only ones ungated. Matches on the RUN command, not on working-directory: seed-typecheck.yml visited seed/lib/frontend for months running only tsc. Severity high. KB § PATTERNS/devops/product-lockfile-and-slug-drift.md.")
    ...
    parser.add_argument("--check-stand-in-conformance", action="store_true", help="Keeper: a test double (mock/wrapper) must satisfy the same CONTRACT as the real thing it replaces. Built against THREE same-day production failures (2026-09-22) where a stand-in was more permissive than the real thing, so the suite stayed green while production could not work: `imovel_dados` (MockSupabaseClient accepted a `datetime.date` write payload httpx's stdlib JSON encoder refuses — fixed by `_validate_json_serializable`, leg B(iii) pins the call stays wired), `_SchemaPinnedAdminClient` (a `__slots__` wrapper missing `__weakref__` broke every `WeakKeyDictionary`-keyed cache the moment it ran against real dependency resolution — CI green, predeploy_check clean; leg A resolves each product's OWN FastAPI dependency graph against a REAL client object built with dummy JWT-shaped credentials, construction-only, never a live request, and leg B(i)/B(ii) assert the resolved objects stay weak-referenceable and identity-stable for the two accessor names — `get_admin_client`/`get_scoped_admin_client` — whose own docstrings promise it), and `atendimento_contrato_versoes` (a cross-column CHECK `CheckManifest`'s single-column shape cannot express, silently unenforced — leg B(iv) inventories migration-declared cross-column CHECKs against `ConditionalPresenceManifest` coverage; an unlisted one is a finding). NEVER a false green: a product whose dependency graph can't be resolved (missing venv, import failure, timeout) reports `inconclusive`, never `ok` — mirrors `noctus.dev.gate_sweep`'s posture. Scoped to `ativo=true` products (`_active_product_dirs`) for CI speed; legs A/B(i)/B(ii) run one short subprocess PER ACTIVE product in parallel. Leg B(iv) is DELIBERATELY advisory: the fleet-wide migration scan correctly surfaces a substantial number of PRE-EXISTING cross-column CHECK constraints unrelated to this fix that nobody has reviewed yet — an unreviewed backlog, not a broken detector; escape hatch for a reviewed one: `_KNOWN_UNENFORCEABLE_CROSS_COLUMN_CHECKS` in `tools/noctus/dev/stand_in_conformance.py`. KB: see that module's docstring for the full account (no KB doc yet — new detector).")
    parser.add_argument("--check-piped-exit-code-pattern", action="store_true", help="Keeper: verdict-channel-integrity — flags `$?` read off a PIPELINE (reads the LAST stage's exit status, not the judged command's), a pipe into tail/head/grep/sed/awk/cat used directly as an if/&&/|| condition, and either shape with no effective pipefail backing it (severity escalates to high — the verdict is DEFINITELY wrong, not merely fragile; void for a plain bash-shell GitHub Actions run: step, which defaults to pipefail). Scans every .sh file, the extensionless scripts/hooks/ entrypoints, and .github/workflows/*.yml run: steps — excludes .claude/worktrees/** and archive/** (each linked worktree is a full checkout; an unscoped walk over-counted the same 16 real sites ~7x, 2026-09-20). Two real 2026-09-17 incidents: `if git merge ... | tail -1; then` read tail's status on a clean merge; `npx tsc --noEmit | tail -5 && echo \"rc=$?\"` printed tail's $?, not tsc's. `--paths` (staged-files idiom, same as --check-storage-bucket-public) scopes to files touched in this commit; every real finding on this tree today is `warning`, so pre-commit wires this non-blocking. Opt-out: `# noctusai-keeper: allow-piped-exit-code`. KB § PATTERNS/common/methodology-execution-discipline.md § 6.")
    ...
    if args.validate:
        ...
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i['file']} — {i['issue']}")
        sys.exit(1)
    elif args.check_prod_compose_hardening_parity:
        from settings import REPO_ROOT
        from tools.noctus.dev.compliance import check_prod_compose_hardening_parity
        issues = check_prod_compose_hardening_parity(repo_root=REPO_ROOT)
        if not issues:
        ...
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i['file']} — {i['issue']}")
        sys.exit(1)
    elif args.check_contract_field_provenance_map:
        from settings import REPO_ROOT
        from tools.noctus.dev.compliance import check_contract_field_provenance_map
        issues = check_contract_field_provenance_map(repo_root=REPO_ROOT)
        if not issues:
            print(f"  {GREEN}✓ contract-field-provenance-map: every contrato_gerador placeholder has a documented row.{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} contract-field-provenance-map issue(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('product','?')} {i['file']} — {i['issue']}")
        sys.exit(1)
        ...
        if not issues:
            print(f"  {GREEN}✓ CONTEXTUALIZE.md aligned (pointer-only + canonical cores + roster/counts fresh vs disk).{RESET}")
            sys.exit(0)
        ...
        for i in issues:
            print(f"    {YELLOW}[{i['severity']}]{RESET} {i['file']} — {i['issue']}")
        sys.exit(0)
    elif args.check_cache_uses_locking_helper:
        from tools.noctus.dev.compliance import check_cache_uses_locking_helper
        issues = check_cache_uses_locking_helper()
        if not issues:
    ...
    elif args.check_hashlib_usedforsecurity:
        from tools.noctus.dev.compliance import check_hashlib_usedforsecurity
        issues = check_hashlib_usedforsecurity()
        if not issues:
    ...
    elif args.check_primary_checkout_commit:
        from tools.noctus.dev.compliance import check_primary_checkout_commit
        issues = check_primary_checkout_commit()
        if not issues:
            print(f"  {GREEN}✓ primary-checkout-commit: clean (not a work commit on a shared branch in the primary checkout).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ work commit on a shared branch in the PRIMARY checkout:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')}")
            print(f"      {i['issue']}")
        sys.exit(1)

    elif args.check_git_hooks_bypass:
        from tools.noctus.dev.compliance import check_git_hooks_bypass
        # `--paths a,b,c` narrows the sweep (the pre-commit hook passes the
        # staged set); omitted, the whole tracked tree is scanned.
        _paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_git_hooks_bypass(paths=_paths)
        if not issues:
            print(f"  {GREEN}✓ git-hooks-bypass: clean (no tracked file hardcodes a hooks bypass).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} tracked file(s) hardcode a git hooks bypass:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        sys.exit(1)

    elif args.check_git_hook_file_tampering:
        from tools.noctus.dev.compliance import check_git_hook_file_tampering
        # `--paths a,b,c` narrows the sweep (the pre-commit hook passes the
        # staged set); omitted, the whole tracked tree is scanned.
        _paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_git_hook_file_tampering(paths=_paths)
        if not issues:
            print(f"  {GREEN}✓ git-hook-file-tampering: clean (no tracked file disables a hook by filesystem op).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} tracked file(s) tamper with a git hook FILE directly:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        sys.exit(1)

    elif args.check_conftest_env_setdefault:
        from tools.noctus.dev.compliance import check_conftest_env_setdefault
        issues = check_conftest_env_setdefault()
        if not issues:
            print(f"  {GREEN}\u2713 conftest-env: clean (no conftest defers a credential/endpoint key to the ambient env).{RESET}")
            sys.exit(0)
        print(f"  {RED}\u2717 {len(issues)} conftest(s) let the ambient environment decide what the suite talks to:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} \u2014 {i['issue']}")
        sys.exit(1)

    elif args.check_conflict_markers:
        from tools.noctus.dev.compliance import check_conflict_markers
        # `--paths a,b,c` narrows the sweep (the pre-commit hook passes the
        # staged set); omitted, the whole tracked tree is scanned.
        _paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_conflict_markers(paths=_paths)
        if not issues:
            print(f"  {GREEN}✓ conflict-markers: clean (no tracked file carries unresolved markers).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} file(s) committed mid-conflict:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        sys.exit(1)
    ...
    elif args.check_storage_bucket_public:
        from tools.noctus.dev.compliance import check_storage_bucket_public
        # `--paths` (comma-separated, staged-files idiom shared with
        # `--check-conflict-markers`) scopes the scan to THIS commit's
        # staged files — pre-commit's mode, so immutable history (001/011)
        # never re-blocks a commit that never touches it. Omitted, this is
        # the full-tree audit mode — see the function's own docstring for
        # why that duality is diff-scoping, not a suppression mechanism.
        _storage_paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_storage_bucket_public(paths=_storage_paths)
        if not issues:
            print(f"  {GREEN}✓ storage-bucket-public: clean (no public bucket declaration, no get_public_url/getPublicUrl call site, no runtime public bucket creation).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} storage-bucket-public finding(s) — NO override, NO suppression marker, NO accept-with-rationale (owner directive 2026-09-17):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i['file']}:{i['line']} — {i['issue']}")
        # Every leg is `critical` and every leg BLOCKS — there is no lesser
        # severity band to let through. See the keeper's module docstring in
        # compliance.py for why this keeper carries no allowlist.
        sys.exit(1)

    elif args.check_migration_guard_has_probe:
        from tools.noctus.dev.compliance import check_migration_guard_has_probe
        # `--paths` scopes to THIS commit's staged migration files (same
        # diff-scoping idiom as --check-storage-bucket-public, and for the
        # same reason: forward-looking only, never a retroactive sweep of
        # the existing migration corpus).
        _guard_paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_migration_guard_has_probe(paths=_guard_paths)
        if not issues:
            print(f"  {GREEN}✓ migration-guard-has-probe: every guard-shaped DDL object in scope has a registered behaviour probe.{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} migration-guard-has-probe finding(s) — a declared guard with no executable proof it refuses anything:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i['file']} — {i['issue']}")
        sys.exit(1)

    elif args.check_schema_wide_anon_grant:
        from tools.noctus.dev.compliance import check_schema_wide_anon_grant
        # `--paths` (staged-files idiom, same as --check-storage-bucket-public)
        # scopes to THIS commit's staged migration files — diff-scoped, never
        # a full-corpus retroactive sweep: the pre-2026-09-20 `001_*.sql`
        # history stays on disk forever (a migration is a record of what WAS
        # applied, never a mutable snapshot of current intent), so a full-tree
        # sweep would permanently redden this gate. A NEWLY staged
        # schema-wide anon grant blocks unconditionally.
        _anon_grant_paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_schema_wide_anon_grant(paths=_anon_grant_paths)
        if not issues:
            print(f"  {GREEN}✓ schema-wide-anon-grant: clean (no blanket ON ALL TABLES / ALTER DEFAULT PRIVILEGES grant naming anon).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} schema-wide-anon-grant finding(s) — NO override, NO suppression marker:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i['file']}:{i['line']} — {i['issue']}")
        sys.exit(1)
        ...
        issues = check_lying_loading_state()
        if not issues:
            print(f"  {GREEN}✓ lying-loading-state: clean.{RESET}")
            sys.exit(0)
        print(f"  {YELLOW}⚠ {len(issues)} lying-loading-state issue(s) (warnings — observe-first cadence):{RESET}")
        for i in issues:
            print(f"    {YELLOW}[{i['severity']}]{RESET} {i.get('product','?')} {i.get('file','?')} — {i['issue']}")
        # severity=warning only — do NOT block commits (new detector, observe-first).
        sys.exit(0)

    elif args.check_postgrest_schema_qualified_table:
        from tools.noctus.dev.compliance import check_postgrest_schema_qualified_table
        issues = check_postgrest_schema_qualified_table()
        if not issues:
            print(f"  {GREEN}✓ postgrest-schema-qualified-table: clean (all table names are schema-relative).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} schema-qualified PostgREST table name(s) — each is a guaranteed runtime 500:{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        # severity=high — a qualified name NEVER works against real PostgREST.
        sys.exit(1)

    elif args.check_postgrest_unbounded_query:
        from tools.noctus.dev.compliance import check_postgrest_unbounded_query
        issues = check_postgrest_unbounded_query()
        if not issues:
            print(f"  {GREEN}✓ postgrest-unbounded-query: clean.{RESET}")
            sys.exit(0)
        warn = [i for i in issues if i["severity"] == "warning"]
        info = [i for i in issues if i["severity"] != "warning"]
        # `.in_()`-shape (warning) is structurally risky regardless of table
        # cardinality — printed first, own count. Select-shape (info) is
        # audit-worthy but its risk is entirely a function of a row count
        # this keeper cannot see — kept visible, not diluting the warning
        # tier (KB § PATTERNS/backend/postgrest-row-cap.md).
        print(
            f"  {YELLOW}⚠ {len(warn)} .in_()-shape warning(s) + {len(info)} "
            f"select-shape info(s) — observe-first cadence:{RESET}"
        )
        for i in warn:
            print(f"    {YELLOW}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        for i in info:
            print(f"    [{i['severity']}] {i.get('file','?')} — {i['issue']}")
        # Non-blocking regardless of tier — heuristic, no table-cardinality
        # knowledge; do NOT block commits (new detector, observe-first,
        # matches check_lying_loading_state's cadence).
        sys.exit(0)
    ...
    elif args.check_stranded_branches:
        from tools.noctus.dev.compliance import check_stranded_branches
        issues = check_stranded_branches()
        if not issues:
            print(f"  {GREEN}✓ stranded-branches: no work branch is carrying unmerged commits past the age threshold.{RESET}")
            sys.exit(0)
        print(f"  {YELLOW}⚠ {len(issues)} stranded branch(es):{RESET}")
        for i in issues:
            print(f"    {YELLOW}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        sys.exit(0)  # advisory — never blocks

    elif args.check_mcp_tools_reject_unknown_args:
        from tools.noctus.dev.compliance import check_mcp_tools_reject_unknown_args
        issues = check_mcp_tools_reject_unknown_args()
        if not issues:
            print(f"  {GREEN}✓ mcp-strict-tool-args: the strict-argument gate is installed after register_all.{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} strict-argument gate problem(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)

    elif args.check_override_is_range:
        from tools.noctus.dev.compliance import check_override_is_range
        issues = check_override_is_range()
        if not issues:
            print(f"  {GREEN}\u2713 override-is-range: clean (no exact-version override).{RESET}")
            sys.exit(0)
        print(f"  {RED}\u2717 {len(issues)} exact-override freeze(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('product','?')} {i.get('file','?')} — {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)
    ...
    elif args.check_every_test_file_is_gated:
        from tools.noctus.dev.compliance import check_every_test_file_is_gated
        issues = check_every_test_file_is_gated()
        if not issues:
            print(f"  {GREEN}\u2713 every-test-file-is-gated: clean (no tracked test file lives outside a gated area).{RESET}")
            sys.exit(0)
        print(f"  {RED}\u2717 {len(issues)} ungated test area(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} \u2014 {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)

    elif args.check_seed_declared_imports:
        from tools.noctus.dev.compliance import check_seed_declared_imports
        issues = check_seed_declared_imports()
        if not issues:
            print(f"  {GREEN}\u2713 seed-declared-imports: clean (every runtime import names a declared distribution).{RESET}")
            sys.exit(0)
        print(f"  {RED}\u2717 {len(issues)} seed-declared-imports issue(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('product','?')} {i.get('file','?')} \u2014 {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)

    elif args.check_seed_test_root_ci_coverage:
        from tools.noctus.dev.compliance import check_seed_test_root_ci_coverage
        issues = check_seed_test_root_ci_coverage()
        if not issues:
            print(f"  {GREEN}\u2713 seed-test-root-ci-coverage: clean (every seed suite with tests is actually run by a workflow).{RESET}")
            sys.exit(0)
        print(f"  {RED}\u2717 {len(issues)} seed-test-root-ci-coverage issue(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('product','?')} {i.get('file','?')} \u2014 {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)

    elif args.check_upload_route_body_override:
        from tools.noctus.dev.compliance import check_upload_route_body_override
        issues = check_upload_route_body_override()
        if not issues:
            print(f"  {GREEN}✓ upload-route-body-override: clean (every UploadFile route has a max_body_path_overrides entry).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} upload-route-body-override issue(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('product','?')} {i.get('file','?')} — {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)

    elif args.check_stand_in_conformance:
        from tools.noctus.dev.compliance import check_stand_in_conformance
        issues = check_stand_in_conformance()
        if not issues:
            print(f"  {GREEN}✓ stand-in-conformance: clean (every stand-in resolved cleanly against a real object; no unlisted cross-column CHECK).{RESET}")
            sys.exit(0)
        inconclusive = [i for i in issues if i.get("status") == "inconclusive"]
        findings = [i for i in issues if i.get("status") == "finding"]
        # Leg B(iv) is DELIBERATELY advisory — the fleet-wide migration scan
        # surfaces a substantial PRE-EXISTING backlog of unreviewed
        # cross-column CHECK constraints unrelated to any one fix; blocking
        # every commit on that backlog would be a big-bang gate this
        # detector's first landing does not intend to be. Legs A/B(i)/B(ii)/
        # B(iii) block — they pin the three failures this detector was built
        # against and this repo is proven clean on today.
        blocking_findings = [f for f in findings if f.get("leg") != "B(iv)"]
        if inconclusive:
            print(f"  {YELLOW}⚠ {len(inconclusive)} stand-in-conformance INCONCLUSIVE (not measured, not ok):{RESET}")
            for i in inconclusive:
                print(f"    {YELLOW}[{i['leg']}]{RESET} {i.get('product','?')} {i.get('file','?')} — {i['issue']}")
        if findings:
            print(f"  {RED}✗ {len(findings)} stand-in-conformance finding(s):{RESET}")
            for i in findings:
                marker = "(advisory — leg B(iv), pre-existing backlog)" if i.get("leg") == "B(iv)" else ""
                print(f"    {RED}[{i['severity']}/{i['leg']}]{RESET} {i.get('product','?')} {i.get('file','?')} — {i['issue']} {marker}")
        blocking = any(f.get("severity") in ("high", "critical", "error") for f in blocking_findings)
        sys.exit(1 if blocking else 0)

    elif args.check_piped_exit_code_pattern:
        from tools.noctus.dev.compliance import check_piped_exit_code_pattern
        # `--paths` (comma-separated, staged-files idiom shared with
        # `--check-storage-bucket-public`) scopes the scan to THIS commit's
        # staged shell/hook/workflow surface — pre-commit's mode, so an
        # immutable pre-existing offender never re-blocks a commit that
        # never touches it. Omitted, this is the full-tree audit mode.
        _piped_paths = [p for p in (args.paths or "").split(",") if p] or None
        issues = check_piped_exit_code_pattern(paths=_piped_paths)
        if not issues:
            print(f"  {GREEN}✓ piped-exit-code-pattern: clean (no $?/if/&&/|| read off a pipeline's LAST stage).{RESET}")
            sys.exit(0)
        print(f"  {YELLOW}⚠ {len(issues)} piped-exit-code-pattern issue(s) (advisory — every finding measured on this tree is `warning`, see the keeper's docstring):{RESET}")
        for i in issues:
            print(f"    {YELLOW}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)
    ...
    elif args.check_stale_branch_pointers:
        from tools.noctus.dev.compliance import check_stale_branch_pointers
        issues = check_stale_branch_pointers()
        high = [i for i in issues if i["severity"] == "high"]
        if not issues:
            print(f"  {GREEN}✓ stale-branch-pointers: every landed branch's pointer is closed.{RESET}")
            sys.exit(0)
        color = RED if high else YELLOW
        print(f"  {color}{'✗' if high else '⚠'} {len(issues)} stale branch pointer(s) "
              f"({len(high)} owned by this session → blocking):{RESET}")
        for i in issues:
        ...
        issues = check_branch_tree_mirror(branch=branch_arg)
        if not issues:
            label = branch_arg or "all non-terminal branches"
            print(f"  {GREEN}✓ branch-tree-mirror: {label} — pointer intact.{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} branch-tree-mirror issue(s) (HARD-BLOCK):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        sys.exit(1)
    ...
    # — the fresh-subprocess mechanism just happens to be their newest caller.
        ...
        result = _rbs()
        if not result.get("ok"):
            print(f"  {RED}✗ build-scope refresh FAILED: {result.get('error')}{RESET}")
            sys.exit(1)
        slugs = " ".join(result.get("slugs", []))
        excl = ", ".join(result.get("excluded", [])) or "none"
        if result["status"] == "in-sync":
            print(f"  {GREEN}✓ build-scope already in-sync: {slugs}{RESET}")
        else:
        ...
        from tools.noctus.dev.vector_costs import drain_spool as _vc_drain
        result = _vc_drain()
        if not result.get("ok"):
            # Surface, never block: this runs inside pre-commit.
            print(f"  {YELLOW}⚠ cost-spool drain failed: {result.get('error')}{RESET}")
            sys.exit(0)
        if result.get("drained"):
            print(f"  {GREEN}✓ folded {result['drained']} spooled cost row(s) into the ledger{RESET}")
        sys.exit(0)
