...
def main():
    ...
    parser.add_argument("--deploy-verify", action="store_true", help="The INDEPENDENT prod revision-drift witness — read-only, ZERO dependency on --deploy-image having run. For every ACTIVE product (prod compose roster, or --deploy-verify-products): running container revision vs the freshly-resolved prod branch tip + health + startup_hook_error. Exit 0 only when status='verified'. MCP: noctus.dev.deploy_verify.")
    parser.add_argument("--deploy-verify-products", default=None, help="With --deploy-verify: comma-separated product slugs to check (default: the full prod compose roster).")
