...
def main():
    ...
    if args.validate:
        ...
        if not issues:
            ...
            sys.exit(0)
        print(f"  {YELLOW}⚠ {len(issues)} postgrest-unbounded-query issue(s) (warnings — observe-first cadence):{RESET}")
        for i in issues:
            print(f"    {YELLOW}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        # severity=warning only — heuristic, no table-cardinality knowledge;
        # do NOT block commits (new detector, observe-first, matches
        # check_lying_loading_state's cadence).
        sys.exit(0)
