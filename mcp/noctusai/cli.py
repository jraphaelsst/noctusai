...
def main():
    ...
    parser.add_argument("--check-piped-exit-code-pattern", action="store_true", help="Keeper: verdict-channel-integrity — flags `$?` read off a PIPELINE (reads the LAST stage's exit status, not the judged command's), a pipe into tail/head/grep/sed/awk/cat used directly as an if/&&/|| condition, and either shape with no effective pipefail backing it (severity escalates to high — the verdict is DEFINITELY wrong, not merely fragile; void for a plain bash-shell GitHub Actions run: step, which defaults to pipefail). Scans every .sh file, the extensionless scripts/hooks/ entrypoints, and .github/workflows/*.yml run: steps. Two real 2026-09-17 incidents: `if git merge ... | tail -1; then` read tail's status on a clean merge; `npx tsc --noEmit | tail -5 && echo \"rc=$?\"` printed tail's $?, not tsc's. Opt-out: `# noctusai-keeper: allow-piped-exit-code`. KB § PATTERNS/common/methodology-execution-discipline.md § 6.")
    ...
    if args.validate:
        ...
        issues = check_piped_exit_code_pattern()
        if not issues:
            print(f"  {GREEN}✓ piped-exit-code-pattern: clean (no $?/if/&&/|| read off a pipeline's LAST stage).{RESET}")
            sys.exit(0)
        print(f"  {RED}✗ {len(issues)} piped-exit-code-pattern issue(s):{RESET}")
        for i in issues:
            print(f"    {RED}[{i['severity']}]{RESET} {i.get('file','?')} — {i['issue']}")
        blocking = any(i.get("severity") in ("high", "critical", "error") for i in issues)
        sys.exit(1 if blocking else 0)
