...
def register(server) -> None:
    @server.tool(
        ...
        description=(
            ...
            "the incident this tool caused; R4 fresh-subprocess fallback "
            "2026-09-24): a confirm=True call on a MUTATING action (start/"
            "integrate/cleanup) on a STALE MCP server re-runs itself as a "
            "brand-new `python mcp/noctusai/cli.py --task-branch ...` "
            "subprocess against the current on-disk code (status carries "
            "executed_via='fresh_subprocess') instead of refusing outright "
            "— a stale task_branch once silently stopped provisioning "
            "worktrees while still reporting status='started', exit 0; the "
            "fix for that is fresh code, not a manual reconnect. It only "
            "REFUSES (status='refused_stale_toolkit', exit_code=1) when that "
            "fallback itself is impossible. action='status' and any "
            "confirm=False plan call are only warned (toolkit_stale + a "
            "warnings entry), never subprocessed. allow_stale_toolkit=True "
            "is the escape hatch (almost always wrong) that stays fully "
            "in-process. See noctus.dev.toolkit_freshness. "
