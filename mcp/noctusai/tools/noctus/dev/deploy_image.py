...
def register(server) -> None:
    @server.tool(
        ...
        description=(
            ...
            "noctus.dev.deploy_verify. TOOLKIT-STALENESS GUARD (2026-09-18; R4 "
            "fresh-subprocess fallback 2026-09-24): a confirm=True call on a "
            "STALE MCP server re-runs itself as a brand-new `python "
            "mcp/noctusai/cli.py --deploy-image ...` subprocess against the "
            "current on-disk code (status carries executed_via="
            "'fresh_subprocess') instead of refusing outright — swapping a "
            "running container on stale logic is the case that actually "
            "hurts, but the fix is fresh code, not a manual reconnect. It "
            "only REFUSES (status='refused_stale_toolkit', container "
            "untouched) when that fallback itself is impossible. A "
            "confirm=False plan call is only warned, never subprocessed. "
