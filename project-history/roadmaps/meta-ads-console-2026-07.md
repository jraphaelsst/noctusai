...
**Today's status**: T0 ✅ fired · **T1 ✅ FIRED 2026-07-23** (live System User token validated — see below) · T2 ✅ **resolved: N/A** (this account is lead-gen, not purchase — no ROAS to light up; objective-aware KPI design absorbs it) · T3 blocked (DM initiative paused) · T4–T5 not fired · **T6 ✅ FIRED 2026-07-27** (operator added `leads_retrieval` on app `2110125409480735` + regenerated the System-User token; new token vaulted to the shared DB, **live lead RECORDS now render in prod** — verified end-to-end).
...
- **2026-07-27**: **Phase 3.1 — lead PERSISTENCE shipped to prod (data model set).** Operator chose "typed core + JSONB tail" + "full persistence". Migration 033 (`meta_ads_lead_forms` + `meta_ads_leads`): always-present contact fields (name/email/phone, promoted by the owning form question's TYPE, not key) + campaign/form/platform/time as typed indexable columns; the variable per-form questions + hidden fields (e.g. `REF`) in a GIN-indexed `answers jsonb` (jsonb, NOT stringified text — stays queryable). `LeadsSyncService.sync_all` pulls forms+leads → upserts; the `leads_retrieval` gate is surfaced (records_gated), never fatal. Endpoints went DB-first with live fallback (`source=db|live`, `last_synced_at`); `POST /leads/sync` = background job. FE gained "Sincronizar leads" + job-poll + source indicator. Applied migration 033 to the shared DB (also drained pending drift 031/032 media-creation, additive/idempotent), then ran the ingest locally → **298 forms + 941 leads persisted**. Verified live in prod: header "Armazenados no banco · sincronizado", records served from Postgres. Shipped `e010bf43`→`39da5b31`.
  - **DEPLOY NOTE:** `deploy_image` (container swap) completed + verified; `deploy_pull` (VPS git FF + prod pgvector cache-mirror) idle-timed-out on the VPS SSH throttle circuit-breaker (contended — 2 other agents active) and is **deferred** — non-functional (slim image runs from GHCR, not the VPS git checkout; the next `deploy_pull` FFs it forward). The promote FF'd the whole `dev` state, so it also carried forward another agent's already-integrated `2a55f340` (admin Supabase-PAT field) + migrations 031/032 — included, not replaced (whole-repo release model).
- **2026-07-27**: **T6 FIRED — lead RECORDS unlocked + live in prod.** Operator located `leads_retrieval` on app `2110125409480735` ("One Chatbot") — it is a **distinct scope** requiring Advanced Access (a System User is not an app-role person, so Standard Access does not cover it), separate from `ads_read`/`pages_manage_ads` the token already had. Regenerated the System-User token WITH `leads_retrieval` and re-vaulted it: written Fernet-encrypted to the shared `social_wiring.app_integration_config` (`meta_system_user_token`) via `AppConfigStore.put` + updated dev `.env`; kept out of git (gitignored + never committed). Prod resolves the token fresh per request (no cache), so **no redeploy/restart** — records started flowing on the next call. Verified live: `[Senseys] ONE10023` renders 83 real leads with `field_data` (name/phone/email + the 3 qualifying answers) + ad/campaign attribution. **New DB-modeling finding:** a lead's `field_data` can include keys BEYOND the form's visible `questions` (this account carries a hidden `REF` tracking field = the form's short code) — the future `lead_answer` model must accept arbitrary keys, not just the question set. The records path needed zero code change to light up — it was built complete-but-gated on 2026-07-27, exactly the "ready the instant the scope lands" design.
