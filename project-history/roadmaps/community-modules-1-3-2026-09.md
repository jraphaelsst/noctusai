...
| M3 | **community modules 1–2 exposed in prod** — consent recorded, fleet compose + tunnel ingress + build-scope registered, bless → promote → `deploy_image`, then `deploy_verify` + health + `spa_smoke`. | 🚧 in progress 2026-09-17 (this release). Payment keys not yet set, so checkout fails closed until they are; members/tiers/applications are fully functional without them. |
| M4 | **Module 3 (WhatsApp) built** — tier-driven group membership sync as manager-confirmed batches, broadcasts, forward-only ingest feeding engagement points, AI-flag hand-off. Seed reuse survey done: client/group-ops/mappers/webhook-router/dedup/engagement/jobs all CONSUMED; identity resolver PROMOTED out of social-wiring; three social-wiring services explicitly NOT reused (DM-inbox and multi-line domains). | ✅ built 2026-09-17 — FE **83** tests; backend on its branch. NOT in the M3 release by decision: the seed lift widens the build scope fleet-wide, and the module cannot run until M5. |
