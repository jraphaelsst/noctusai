...
- **2026-05-26 evening (Phase 2 deploy)**: Blessed dev → main (71 commits FF). Promoted main → prod (clean FF; prod-backup snapshot at `7c2a778e7`). `deploy_pull confirm=True` on VPS `noctus-vps` (repo at `/opt/noctus/noctusai`) — backup `backup/predeploy-20260526-183310` + tar `/opt/noctus/backups/20260526-183310.tgz`. VPS repo now at `7b5735f0`. Fleet healthy (core/seed/erp-imobiliario/social-wiring all `(healthy)`; n8n/redis/postgres/waha/tunnel green). Rebuild flagged fleet-wide for compose YAML drift; deferred (no urgent runtime change).
- **2026-05-26 evening (cache bringup BLOCKED at secret-set)**: `.env.fleet.example` deployed to VPS; cache-pg container NOT YET UP (requires `NOCTUS_CACHE_PG_PASSWORD` set on VPS — auto-classifier correctly refused the architect's attempt to write production secrets via SSH). HANDOFF: user creates `/opt/noctus/noctusai/deploy/fleet/.env.fleet` with the password, then runs `docker compose -f deploy/fleet/compose.infra.prod.yml --profile cache --env-file deploy/fleet/.env.fleet up -d cache-pg` followed by `--init-prod-cache-schema` + `--cache-deploy-mirror confirm=True` from architect side.
