...
| **W2-E3' code side** | Real code_embeddings refresh → code_search('extract phone number') | ✅ **VERIFIED** — refresh wrote **2,779 chunks across 393 files** (0 errors); ~44 min total wall-clock; cost ledger row: `namespace=code-embeddings doc_count=393 chunk_count=2779 estimated_tokens=4,168,500 estimated_cost_usd=$0.08337`. `code_search('extract phone number from a contact')` top-5: `scan_wiring.py::_extract_fe_calls` (0.306) / `compliance.py::_migration_number` (0.275) / `meta/tests::test_parse_inbound_decodes_valid_waha_payload` (0.251) / `whatsapp.py::_client` (0.241) / `vista/tests::test_extract_items_splits_envelope_correctly` (0.240). No literal phone-extraction code exists in noc → ranker correctly surfaces closest semantic neighbors (functions with "extract"/"parse"/"split" semantics). |
