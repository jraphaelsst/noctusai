...
| 2026-08-18 | **D16 SHIPPED — and measuring it before deploy changed what "180 days" means.** The sweep, the per-org configurable threshold, reactivation-on-new-touch and a manual-restore guard all landed (`058` + `fe1afdd4`, 2123 tests). The design point worth keeping: "silence" is `GREATEST(ultimo_contato_em, reativado_em)` and **never** `updated_at` — `updated_at` moves on any write (a merge, an undo, a name edit) and would shrink the window for reasons unrelated to whether the person is reachable; and without `reativado_em` a human's manual restore would be undone by the very next tick. 🔴 **Impact measured on live data at apply time: at the ratified 180 days, 7 132 of 10 204 clientes (70 %) go inactive on the first tick** — today 0 are. Distribution table in `migrations/APPLIED.md`. Nothing has run: the scheduler lives in the deployed container and this is not deployed, so the threshold can still be changed before first deploy. Surfaced to the user rather than shipped quietly, because D16's "180" was chosen in the abstract and this is what it means against the real base. |
| 2026-08-19 | 🔴 **The card was built, tested and merged — and never reachable, because nothing had been deployed.** The user sent a screenshot of the funil still opening the old read-only field list and said plainly: *"not the same card. You are wrong on what u said."* They were right. The wiring commit was real (`6f506eac`, three tests asserting WHICH dialog opens), but the running social-wiring image was `9e7fc849` — the commit immediately before it. My report described the repository; the user was describing the product. **The rule this earns: a UI claim is only true of a deployed revision.** `deploy_verify` answers "what is actually running" in one call and was never consulted before claiming the card was live. Same failure shape as the original gap (route-exists ≠ wired), one layer further out: merged ≠ shipped. |
| 2026-08-19 | **The funil's null-`cliente_id` fallback, measured rather than assumed.** Clicking a card with no person layer yet opens the old lead modal. Against live data that is **4 of 1 473 atendimentos (0.3 %)**, all created since the last steady-state pass — the 6-hourly `clientes_backfill_job` closes the window by design. Kept as-is deliberately, and recorded here rather than left implicit: a dead click would be worse, and re-keying the whole `card_hub` API from `cliente_id` to `atendimento_id` to erase a 0.3 % window is not proportionate. Revisit if the backfill interval grows or the share does. |
| 2026-08-19 | 🔴 **p-studio and igig could not be logged into at all — and the probe we had was structurally incapable of seeing it.** `Erro no login SSO / Failed to fetch`, with a healthy container, correct bundle and every endpoint 200. Core's allowlist derives from the `PRODUCTS` registry in `start.sh`; **the slim prod image never shipped that file**, so the registry parsed to empty and only products with an explicit `PRODUCT_URL_<SLUG>` got an origin. `PRODUCT_URL_PATTERN` was a no-op in the container. Fixed at the base image (one `COPY`, every product). The lesson generalises past CORS: **a value derived from a file is only derived where that file exists**, and an empty derived list is indistinguishable from "nothing to configure" — which is why it sat unnoticed. `noctus.dev.sso_cors_smoke` now sends the real browser preflight per live product; `sso_smoke` never could, because a server-to-server call sends no `Origin` and passes while every browser is blocked. |
| 2026-08-19 | **A test suite that reported on the developer's shell.** `predeploy_check` blocked p-studio at 15 failing webhook tests that passed 373/373 from a plain terminal — same commit, same interpreter, same command. `conftest.py` used `os.environ.setdefault("ASAAS_WEBHOOK_TOKEN", …)` and the platform `.env` carries that key **empty**, so the default never applied. The flakiness was the lesser half: the same file deferred `SUPABASE_SERVICE_ROLE_KEY` and `PROVEDOR_COBRANCA`, and therapy-platform deferred `SUPABASE_URL` + both keys directly under a comment promising it "never reaches a real Supabase" — export the real values and those suites build a real service-role client and a real payment adapter. `own_test_env` + keeper `check_conftest_env_setdefault`. **`setdefault` in a conftest does not mean "provide a default"; it means "ambient wins".** |
| 2026-08-19 | **The self-branching gate moved from the commit to the write.** The 2026-08-05 keeper refuses a work *commit* on a shared branch in the primary checkout; it caught neither of the two slips on 2026-08-18, because neither reached `git commit` — both were spotted by eye and hand-migrated. A gate at the first *expensive* consequence still lets the work happen in the wrong tree. `primary_write_guard` now denies the tool call itself via a `PreToolUse` hook, parsing `cd` out of the command because `cd <primary> && sed -i …` is the exact shape both slips took and the harness reports only the session cwd. It fired correctly in-session — and over-refused twice on heredocs containing Markdown (`> **Fix:**` read as a redirect) and on `cd "$W"`; both were fixed in the parser rather than by loosening the rule, since **an over-refusing gate is the kind that gets switched off.** Caveat worth keeping: a hook added mid-session does not protect that session. |
