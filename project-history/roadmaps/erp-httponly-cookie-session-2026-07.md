...
| 2 | **ERP backend** — mount the seed router; RLS client per request via the exchanger; ensure the **minted access token** (not the session id) reaches `set_session` (SEC-6); pass `check_auth_session_mutation_on_shared_client`. | ⏳ **NEVER DISPATCHED** — the 2026-07-15 wave dispatched 1b + 3 but skipped 2, so Slice 3 was briefed to build a frontend against endpoints nothing mounts on ERP. Also collides with the org-source-of-truth fan-out in `products/erp-imobiliario/backend/` — must not run beside it. ✅ **SHIPPED 2026-07-29 (`0fed476f`)** — landed the same day it was rescued. `"auth"` in `standard_routers` mounts `/api/auth/{login,me,logout}` + `/api/settings/api-tokens` via the seed's bare `_build_auth_router` (the simpler of the two documented mount shapes: ERP has no real `ApiTokenResolver` to inject and no existing bridge to preserve on these 5 endpoints). Migration renumbered **040 → 043** — erp had reached 042 while the work sat unmerged, so the original could never have applied. **SEC-6 verified**: `get_current_user_org_unified` mints a real access token via `TokenExchanger.access_token_for` before anything reaches `set_session`; the opaque session id is never passed as a token, and a non-user caller 403s rather than being coerced into a user-shaped tuple. `check_auth_session_mutation_on_shared_client` → 0 issues on erp; suite 2150 passed / 34 skipped (+3 new). The 61 pre-existing routers stay on `get_current_user_org`, untouched — nothing consumes the unified dep yet, by design (per-router opt-in is a `Depends(...)` swap with no call-site change). **Both stated blockers were re-checked, not assumed:** the org-SoT collision is gone (its migrations 038/039 already landed on dev; branch absent from the remote), and 1b′ blocks Slice 3's *frontend*, not this backend mount. |
