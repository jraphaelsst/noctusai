...
- **Scoped auto-improvement + consult-before-editing.** Dispatches surface `drift-found:`/`scoped-improvement:`; tech-lead RESOLVES, engineers SURFACE. In `auto-improvement.ndjson` + cache; consult BEFORE editing any doc/agent. Log each `s2-memory` with a `resolve_when` so `auto_improvement_reconcile` self-closes landed drift. Keepers `check_auto_improvement_cache_freshness` + `check_s2_drift_has_resolution_handle`. → `KB § PATTERNS/common/scoped-auto-improvement.md`
