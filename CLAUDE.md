...
- Containerization (multi-layer Docker: per-product `docker-compose.yml` + root `include:` orchestrator + shared `noctus-net` + per-product `<slug>-net` isolation; canonical pattern at `products/seed/`; `./start.sh` Docker-default with `tunnel <slug>` cloudflare quick-tunnel for OAuth/webhook/demo online testing — quick-tunnels MUST pin `--protocol http2`, QUIC dies behind NATs in ~5min and re-registration is impossible while the container stays "Up"; `native` legacy preserved) → `KB § PATTERNS/containerization.md`
