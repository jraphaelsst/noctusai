...
- **`main` is production; `dev` is integration.** Everyday work + pushes → `dev`; `main`/`prod` only by explicit consent; **prod-deploy promotes a dev-fleet-validated build** (deploy+smoke on dev, never first-contact). → `KB § PATTERNS/architect/branching-and-merging.md §0` · skill `noc-ship`
...
- **Lenses-applied commit trailer (optional, observability).** Inline-deved commits SHOULD carry a `Lenses: <name>[, …]` trailer — makes inline-empersonation auditable via `git log --grep "Lenses:"`. → `KB § PATTERNS/common/lenses-applied-trailer.md`
