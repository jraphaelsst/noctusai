...
- **Branching-first orchestration — parallelize by default; serial only when chunks collide.** The dev methodology is branching-first. Orchestrator's default mental model on incoming work: "can this be chunked into parallel branches?" If yes — dispatch subagents on separate branches in a single `Task` tool-use turn (true parallelism). If no (chunks genuinely collide on files/lines OR have hard dependencies) — serial, OR master-tree-parallel-batches for N≥2 same-shape children. **Serial requires justification** (logged as a learning so the recurrence rule can flip it back to parallel later). Anti-patterns: serial-by-default when parallel was feasible; subagents in separate messages (= serial); forcing parallel when chunks genuinely depend; skipping chunk identification (file-overlap analysis + dependency analysis BEFORE dispatch). → `KB § 01-PHILOSOPHY.md § Branching-first orchestration`
