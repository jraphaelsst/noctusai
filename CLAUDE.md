...
- **Absorption-search is a standing duty in product code.** Whenever editing a product's services/routers/hooks/components — *even for an unrelated task* — keep the recurrence rule active and run the MCP scan sextet before walking away, notably `noctus.dev.scan_cross_product_helpers` + `scan_within_product_helpers` + `scan_service_line_recurrence` + `scan_block_patterns`. Use BEFORE writing a new helper/DTO/service shell and AFTER a cleanup pass. **Within-product N=2 needs architect-eyes during Phase 0** (scanners threshold N≥3). → `KB § 06-AGENTS.md § Absorption-search sextet`
- **Code-hygiene trio + the `hound`.** Three orthogonal cleanup scopes: **absorption** (cross-product file-level — `noctus.seed.report`), **fusion** (cross-tool fn-level — `noctus.seed.scan_fusions`), **optimization** (intra-file line-level — `noctus.seed.scan_optimizations`). **`noctus.hound.scan` is the single entry point** — runs all three + emits `next_action`. Default for *"what cleanup is most urgent?"*. Keeper = regulatory; hound = curatorial. → `KB § PATTERNS/seed-absorption.md § noctus.hound.scan` + `KB § 06-AGENTS.md § Code-hygiene trio`
- **Storage-hygiene trio + the `mole`.** keeper guards laws, hound sniffs hygiene, **mole burrows storage waste**. Three scopes: **artifacts** (caches/builds), **environments** (venv/node_modules — *advisory-only*), **worktrees** (stale `.claude/worktrees/agent-*/`). `bash scripts/mole.sh scan` → `next_action`; `sweep --force` destructive (dry-run default). **MCP-exposed**: `noctus.dev.mole` (`mode`,`scope`,`force`; project cleanup is the separate `noctus.dev.archive`). Triggers: pre-dispatch + pre-commit + bootstrap. Never deletes uncommitted work / main / siblings / unmerged / `.env` / migrations. Methodology scripts MUST be bash 3.x compatible. → `KB § PATTERNS/storage-hygiene.md`
...
- **Doc-code coherence — updating a tool means updating its docs in the SAME change.** Extends three-way-sync to **tool-code ↔ doc-prose**. When a tool (script/MCP tool/keeper/CLI) changes behavior (flag/mode/rename/severity), every referencing doc updates in the **same commit**: KB patterns, Situation→Tool maps, CLAUDE.md pointers, INDEX.md, `--help`, product README/MASTER-PROMPT. "Later" is forbidden. Discovery: `grep -rn "<tool-name>" KNOWLEDGE-BASE/ CLAUDE.md CLAUDE/ products/*/README.md`. → `KB § PATTERNS/methodology-codification-pipeline.md § 8 (Situation → tool map)` (the most-stale-prone doc surface)
...
- **Templates cannot modify noc.** A template workspace is a noc sibling consuming 8 noc surfaces via read-only symlinks. Three-layer defense: (1) PRIMARY — template pre-commit refuses commits touching symlinked paths or non-sandbox additions without a `.promotions/<slug>.md`; (2) AGENT — this rule + KB + memory; (3) SYMBOLIC — chmod -h a-w (symbolic). Edits to symlinked surfaces belong in noc; additions go through the promotion manifest + `noctusai_promote_from_template`. `PROMOTIONS.md` is **derived** (`scripts/gen-promotions-index.py`; pre-commit Rule 3 `--check` refuses drift) — never hand-maintain. → `KB § PATTERNS/template-workspace.md` + `KB § PATTERNS/seed-workspace.md § Promotion manifest`
- **Seed workspaces inherit noc whole, not trimmed.** Sibling workspaces consume all 8 noc surfaces via read-only symlinks; *trim none*. Auto-load is already minimal by design (`CLAUDE.md` router; KB on-demand; `MEMORY.md` index). Trimming breaks seed-first analysis, recurrence scans, KB pointer integrity (`verify-kb-sync.sh`), three-way sync. Per-product focus belongs in `products/<slug>/MASTER-PROMPT.md`. The workspace IS `noc + product`, not `subset(noc) + product`. → `KB § PATTERNS/seed-workspace.md § Why the inherited surface is not trimmed`
