...
- **Branching-first orchestration — parallelize by default; serial only when chunks collide. Orchestration STAYS with the orchestrator.** The dev methodology is branching-first. Orchestrator's default mental model on incoming work: "can this be chunked into parallel branches?" If yes — set up `git worktree add` per subagent (`KB § PATTERNS/branching-and-merging.md § 16` — single-worktree contention is real); dispatch subagents on separate branches+worktrees in a single `Task` tool-use turn (true parallelism); maintain findings.md aggregating their slips / errors / lessons / surprises (§ 17). If no — serial, OR master-tree-parallel-batches for N≥2 same-shape children. **Subagents are EXECUTORS of focused chunks; they are never PLANNERS of orchestration** — orchestrator's broad-context advantage IS the planning value. **Serial requires justification** (logged as a learning so the recurrence rule can flip it back to parallel later). Anti-patterns: serial-by-default when parallel was feasible; subagents in separate messages (= serial); forcing parallel when chunks genuinely depend; skipping chunk identification; **delegating the orchestration itself to a subagent**; **dispatching N subagents into the same worktree** (race-prone; use `git worktree add`). → `KB § 01-PHILOSOPHY.md § Branching-first orchestration` + `§ Knowledge tracking — durable findings file`
- **Knowledge tracking — durable findings file for any non-trivial work.** Any non-trivial project / feature / orchestration maintains a `findings.md` at its root capturing slips / errors / mistakes / lessons / interesting findings / discovered knowledge. Five standard categories. Append in-the-moment for surprises (freshness matters); synthesize at close into a curated knowledge artifact. Default-on for projects + master-trees + parallel-orchestration; optional for trivial direct fixes (skip silently is forbidden — log absence to `phase_learnings` SQLite so the absence is explicit). Distinct from `phase_learnings.db` (atomic per-phase learnings) + `live-patterns-log.md` (master-tree per-batch raw log) + §11 Change log (what-we-did): findings.md is what-we-LEARNED, curated. → `KB § 01-PHILOSOPHY.md § Knowledge tracking`
