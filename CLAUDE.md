...
- **Scoped auto-improvement + consult-before-editing.** Every dispatch returns the two-leg footer (`drift-found:` + `scoped-improvement:`); tech-lead RESOLVES on-contact, engineers SURFACE. Surfaces land in `auto-improvement.ndjson` + cache; consult it BEFORE editing any doc/agent. Keeper `check_auto_improvement_cache_freshness`. → `KB § PATTERNS/common/scoped-auto-improvement.md`
