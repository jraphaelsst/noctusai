...
- **Scoped auto-improvement + consult-before-editing.** Dispatches surface `drift-found:`/`scoped-improvement:`; tech-lead RESOLVES, engineers SURFACE. In `auto-improvement.ndjson` + cache; consult BEFORE editing any doc/agent. Log open entries with a `resolve_when` so `auto_improvement_reconcile` self-closes landed drift. Keepers `check_auto_improvement_cache_freshness` + `check_open_drift_has_resolution_handle`. → `KB § PATTERNS/common/scoped-auto-improvement.md`
...
- **Cache auto-freshness — two-tier + heal-on-contact.** Structural caches (keeper/agent-context/auto-improvement/noc-graph) refresh pre-commit AND self-heal on check (`settle_structural_caches`, zero-OpenAI — closes FF/cross-tree staleness gaps); embedding caches warn-only. → `KB § PATTERNS/common/cache-auto-freshness.md`
