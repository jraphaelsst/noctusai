...
- **Agents heal after every change.** After modifying code, run `python -m agents --heal` on the affected product. The agents detect issues, auto-fix deterministic ones, re-run to verify, and repeat until clean. No code ships with known agent violations. This is the local development loop: change → heal → verify → commit.
