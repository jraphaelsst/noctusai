...
- **AST-first — never regex code edits.** Every code change goes through an AST tool: `libcst` for Python, `ts-morph` for TypeScript, `tree-sitter` for cross-language analysis. Regex / sed / awk only for prose, search, and log inspection — never for editing code. **Boundary rule:** *if the file is parsed by a compiler / interpreter / type-checker, use the AST tool. If it's parsed by humans only, regex is fine.* Anti-patterns: sed-rename-trap (hits comments/docstrings/unrelated identifiers), multi-line regex on whitespace-variant code, hand-edit + find-and-replace for multi-file changes (same trap as sed). Companion to seed-first (regex slip in framework code cascades to every product) and no-quick-fixes. → `KB § 01-PHILOSOPHY.md § AST-first` + `KB § PATTERNS/ast.md`
- **MCP-first — agent-exposable capabilities default to MCP.** When you want to expose a capability to agents (Claude Code, future bots, future product agents), the default surface is the MCP server at `mcp/noctusai/`. Dev tooling, business-logic primitives, vendor adapters all converge there as one growing wide-purpose toolkit; the 24-tool dev toolkit is one branch among many, not the whole identity. Naming: 3-segment dotted (`<umbrella>.<service>.<action>`). Pattern: Pydantic in/out schemas, hierarchical registration, lazy `NoctusContext` for business-logic tools. Composition belongs to the consumer (orchestrations don't live in the MCP). Boundary: if a capability has only one co-located consumer, a plain function is fine; MCP-first fires when a plausible second consumer exists. → `KB § 01-PHILOSOPHY.md § MCP-first` + `projects/mcp-server-expansion/PROJECT.md` (active broaden project)
...
- **AST-driven code edits** (libcst / ts-morph / tree-sitter; recipes for rename, find-callers, codemods; anti-patterns; boundary rule) → `KB § PATTERNS/ast.md`
...
| Exposing a new capability to agents (a function any future bot / product agent / Claude Code might call) | `KB § 01-PHILOSOPHY.md § MCP-first` (default surface is `mcp/noctusai/`) |
