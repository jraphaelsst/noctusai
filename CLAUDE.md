...
- **Absorption-search is a standing duty in product code.** Whenever editing a product's services / routers / hooks / components — *even for an unrelated task* — keep the recurrence rule active and run the MCP scans before walking away from the file. Three modes: `noctus.dev.scan_cross_product_helpers`, `noctus.dev.scan_service_line_recurrence`, `noctus.dev.scan_recurrence`. Use BEFORE writing a new helper/DTO/service shell; use AFTER a cleanup pass as the calibration check. → `KB § 06-AGENTS.md § Absorption-search trio`
...
- **Branching-first orchestration — parallelize by default; serial only when chunks collide. Orchestration STAYS with the orchestrator.** The dev methodology is branching-first. Orchestrator's default mental model on incoming work: "can this be chunked into parallel branches?" If yes — dispatch subagents on separate branches in a single `Task` tool-use turn (true parallelism). If no (chunks genuinely collide on files/lines OR have hard dependencies) — serial, OR master-tree-parallel-batches for N≥2 same-shape children. **Subagents are EXECUTORS of focused chunks; they are never PLANNERS of orchestration** — orchestrator's broad-context advantage IS the planning value, and delegating planning to a subagent collapses the head/worker distinction. **Serial requires justification** (logged as a learning so the recurrence rule can flip it back to parallel later). Anti-patterns: serial-by-default when parallel was feasible; subagents in separate messages (= serial); forcing parallel when chunks genuinely depend; skipping chunk identification (file-overlap analysis + dependency analysis BEFORE dispatch); **delegating the orchestration itself to a subagent** (caught 2026-05-03 — read the files yourself, compute batches yourself, THEN dispatch N parallel executors with focused briefs). → `KB § 01-PHILOSOPHY.md § Branching-first orchestration`
