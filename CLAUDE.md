...
- **NEVER `--no-verify` (commit OR push).** Pre-commit hooks fire on commit NOT push — commit-no-verify is the LARGER bypass (skips ALL keepers), not the smaller one. 5 forbidden rationalizations (commit-only-harmless · I'm-fixing-the-tool · false-positive · no-impact · surface-after) ⇒ STOP + `kind="surface"` proposal + return blocked. → `KB § PATTERNS/common/bypass-rationalization-anti-patterns.md`
