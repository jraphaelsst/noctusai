...
- **NEVER `--no-verify` (commit OR push).** Commit-no-verify skips ALL keepers (the larger bypass, not the smaller). 5 rationalization shapes are catalogued and all blocked. → `KB § PATTERNS/common/bypass-rationalization-anti-patterns.md`
...
- **Build-learn-cache mindset (not only during dev).** Body-DRY: every artifact accumulates 8 knowledge categories (facts/errors/drifts/alternatives/manual-validation/integration/e2e/bugs-fixed) AS WE BUILD, cached sidecar ∨ noc-graph node, append-only across refactor/bugfix/integration/deploy. Sibling of code-DRY + skill-DRY. → `KB § PATTERNS/common/build-learn-cache-mindset.md`
...
- **Inline = empersonate the specialist.** Switch lens to the relevant domain specialist (backend/frontend/devops/security) when inline-deving; apply their discipline + owns_kb. → `KB § PATTERNS/architect/parallelization-first-orchestration.md`
...
- **Cache auto-freshness — two-tier cadence, no staleness window.** Structural caches (keeper/agent-context/auto-improvement/noc-graph) refresh pre-commit (fix-on-contact, zero-OpenAI); embedding caches (kb/code/corpus/memory) on pre-push + post-merge/checkout; `check_all_cache_freshness` (in 8-way-sync) surfaces staleness either way. → `KB § PATTERNS/common/cache-auto-freshness.md`
- **Cache-portable architecture — TWO-TIER persistent + machine-portable.** Tier-1 local at `<git-common-dir>/noctusai/cache/*.sqlite` (shared by ALL worktrees of this repo — one physical SQLite, not per-worktree); Tier-2 remote at prod pgvector (mirrored via `cache_deploy_mirror`, pulled via `cache_pull`); auto-pull-on-empty default ON for fresh-clone bootstrap (no OpenAI re-embed). All 11 cache-path resolution sites consolidated through `cache_backend.cache_path()` / `cache_dir()`. → `KB § PATTERNS/common/cache-portable-architecture.md`
- **Vectorize → embed → cache (the unified pipeline).** Any vectorize/embed/cache slice follows the SAME 3-leg pipeline: (1) caching-architecture decision (extend before spawn — N≥3 same-shape ⇒ extend), (2) embed via `noctusai_lib.integrations.llm` (cost-log; never inline), (3) cache with source-sha invariant + 3-leg mirror contract. → `KB § PATTERNS/common/vectorize-embed-cache-framework.md`
