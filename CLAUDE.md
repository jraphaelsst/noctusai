...
- **NEVER `--no-verify` (commit OR push) — and never a silent hooks bypass either** (`-c core.hooksPath=…`, `--config-env`, `GIT_CONFIG_KEY_*`). Commit-no-verify skips ALL keepers; the hooks-bypass shape is worse — it produces no visible flag at all. Rationalization shapes catalogued + blocked; gates: PreToolUse `decide_git_bypass` + keeper `check_git_hooks_bypass`. → `KB § PATTERNS/common/bypass-rationalization-anti-patterns.md`
...
- **`main` is production; `dev` is integration.** Everyday work + pushes → `dev`; `main`/`prod` when the owner asks — the ask IS the permission (2026-09-24), bless FFs to the newest CI-qualifying-green commit `main..dev` (not necessarily the tip — R1); branch model is phased (target `prod`=code-only). → `KB § PATTERNS/architect/git-branch-model.md` · `KB § PATTERNS/devops/ship-consent-riders.md` · skill `noc-ship`
...
- **The product catalog IS the working guide — read it before touching a product.** `ativo`+`deploy_scope`: ativo+live ⇒ work in prod, ativo+dev ⇒ dev only, inativo ⇒ don't touch; inativo is ASLEEP — no gate checks it (`deploy/fleet/active-scope.txt`). → `KB § PATTERNS/architect/product-working-scope.md`
...
- **Hand-maintained lists drift and break the fleet — derive, don't sync by hand; gate pre-push.** Slug sets ∧ lockfile snapshots ∧ coverage lists ∧ import manifests; a closed-world keeper misses the root nobody listed. Keepers `check_hardcoded_product_slug_set` + `check_product_lockfile_dep_sync` + `check_tunnel_ingress_snapshot_sync` + `check_dependabot_product_coverage` + `check_ci_test_matrix_coverage` + `check_seed_test_root_ci_coverage` + `check_seed_declared_imports` + `check_every_test_file_is_gated`. → `KB § PATTERNS/devops/product-lockfile-and-slug-drift.md` · `KB § PATTERNS/devops/tunnel-ingress-source-of-truth.md`
...
- **Verdict-channel integrity — the verdict you read must belong to what you are judging.** A pipe returns `tail`'s, a watcher its own, silence is not a pass; and a red from an invalid harness (no deps/browser/venv, dead dev-server, stale tree) judges the SETUP. `noctus.dev.gate_sweep`: unmeasurable ⇒ `inconclusive`, never `red`. → `KB § PATTERNS/common/methodology-execution-discipline.md`
