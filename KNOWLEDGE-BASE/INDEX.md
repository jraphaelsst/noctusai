...
│   │   ├── seed-absorption.md         ← noctus.seed.* MCP tools (scan_repetition / list_capabilities / audit_drift / absorb_file / specify_capability / report / scan_fusions / scan_optimizations — the absorption+fusion+optimization trio) + 4 absorption strategies (delete dead code / move to seed + re-export / factory / template + runtime substitution) + per-candidate loop + safety rules
...
| Seed absorption methodology + tools — the **absorption + fusion + optimization trio** (cross-product / cross-tool / intra-file scopes): `noctus.seed.scan_repetition` / `list_capabilities` / `audit_drift` / `absorb_file` / `specify_capability` / `report` rollup / `scan_fusions` meta-detector / `scan_optimizations` intra-file detector; four absorption strategies (delete dead code / move-and-re-export / factory / template + runtime substitution); per-candidate loop (scan → evaluate → self-audit → absorb → re-scan → build-verify); safety rules; relation to DRY recurrence rule + delete_product symmetry | `CONTEXT/PATTERNS/seed-absorption.md` |
