...
│   │   ├── containerization-operations.md ← OPERATIONS sibling (architecture in containerization.md): runbook/methodology for working with containers — source-of-truth chain (git→file→manifest→history→inspect-mounts→exec→logs→DD-is-never-truth) + operational primitives (force-recreate after compose-volume edits; build-verify; runtime-verify; disk diagnostics; multi-arch check) + the 16 known bumps codified in this codebase (cold-boot livelock; chown-R-inherited-base; SPA boot race; amd64-only on Apple Silicon; DD-not-source-of-truth; compose-volume-edit silent-stale; base-cascade; pip-vs-base drift; clean-checkout reproducibility; glibc-vs-alpine FE deps; lockfile-pinned-platform-optionals; source-only Python deps; QUIC tunnel dropout; pre-commit foreign-edit absorption FIXED; MCP propagate fixed-CWD; §9a concurrent-agent checkout sharing) + diagnostic flowchart A-G + 8-step safe-change methodology + the meta-arc (every bump = symptom → first-pass → recurrence → root → 3-way sync). Starting point — refines as new bumps surface
...
| Containerization OPERATIONS (runbook + methodology sibling to containerization.md's architecture: source-of-truth chain · operational primitives · 16 known bumps codified · diagnostic flowchart A-G · 8-step safe-change methodology · meta-arc) | `CONTEXT/PATTERNS/containerization-operations.md` |
