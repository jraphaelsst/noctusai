...
│   │   └── pydantic-strict-http.md      ← `noctusai_lib.api.StrictHttpModel` HTTP-boundary base (Pydantic `extra="forbid"` → 422 on unknown keys); defends against silent-drop misroutes (VVV clinic-settings 2026-05-11); migration recipe per product
...
| Pydantic strict-by-default at HTTP boundary (`noctusai_lib.api.StrictHttpModel` — `extra="forbid"` → 422 on unknown keys; ConfigDict MRO-merge semantic; carve-out via explicit `extra="allow"`; migration recipe per product; defends against silent-drop misroutes — VVV clinic-settings 2026-05-11 + memory `feedback_pydantic_silent_drop_kills_writes`) | `CONTEXT/PATTERNS/pydantic-strict-http.md` |
