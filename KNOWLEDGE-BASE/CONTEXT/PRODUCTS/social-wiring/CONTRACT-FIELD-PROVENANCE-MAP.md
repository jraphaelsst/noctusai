...
- 0 of 1 393 `clientes` rows have a confirmed CPF **or** RG **or** endereço. CPF/RG have an extraction+confirm path (migration 097) that has simply never been clicked; endereço has **no extraction path at all** — see row `p.endereco` below.
...
| `imovel.inscricao_municipal` | §2.2 | none (or AI doc-extraction candidate, migration 118) | `imovel_hub/documentos_service.py:581 extrair_estrutura` (when a `guia_iptu`/`carne_condominio` is uploaded) | `imovel_dados.prefeitura_cadastro_imobiliario` | `PATCH /api/imoveis/{codigo}/dados` (`imovel_hub/router.py:71`, body `ImovelDadosPatchBody`, `imovel_hub/schemas.py:26`) | `manual_entry_only` (no `CAMPOS`-style auto-suggest wired to this specific field; document extraction lands on the DOCUMENT row, not auto-applied here) |
| `imovel.matricula_numero` | §2.2 | matrícula document | AI transcription header (not separately parsed — an operator reads it off the extraction) | `imovel_dados.numero_matricula` | `PATCH /api/imoveis/{codigo}/dados` | `manual_entry_only` |
| `imovel.cartorio` | §2.2 | matrícula document | same as above | `imovel_dados.numero_registro_imoveis` | `PATCH /api/imoveis/{codigo}/dados` | `manual_entry_only` |
...
| placeholder | clause | source document type | extractor code path | confirmed storage | confirm/promote step | status |
|---|---|---|---|---|---|---|
| `p.nome` (`clientes.nome_oficial`) | §2.1 | RG/CNH (AI vision) | `identidade_extracao_service.py:209 CAMPOS` (`item_key="nome_oficial"`, `sobrescreve=True` — a newer reading is always offered even when a name is already set) via `:525 extrair_identidade` | `clientes.nome_oficial` | AI suggestion: `POST /api/clientes/{cliente_id}/documentos/{documento_id}/extracao/confirmar` (`card_hub/router.py:968`) → `identidade_extracao_service.py:866 confirmar_sugestao`. OR direct manual entry: `PATCH /api/clientes/{cliente_id}` (`clientes_router.py:706`, `ClientePatchBody`) | `extracted_and_confirmable` |
| `p.nacionalidade_flex` | §2.1 | none | — | `clientes.nacionalidade` | `PATCH /api/clientes/{cliente_id}` | `manual_entry_only` |
| `p.estado_civil_flex` | §2.1 | RG/CNH/certidão de casamento (AI) | `identidade_extracao_service.py` `CAMPOS` `item_key="estado_civil"`, `sobrescreve=False` (a suggestion, never an overwrite — no second column holds the operator's own spelling) | `clientes.estado_civil` (canonicalised by `documento_checklist_service.completude_contratual`) | same two paths as `p.nome` above | `extracted_and_confirmable` |
| `p.profissao` | §2.1 | none | — | `clientes.profissao` | `PATCH /api/clientes/{cliente_id}` | `manual_entry_only` |
| `p.rg` / `p.rg_orgao` | §2.1 | RG (AI vision) | `CAMPOS` `item_key="rg"`, `sobrescreve=False` (migration 097) — **`rg_orgao` (issuing agency) is NOT a `CAMPOS` entry: only the RG NUMBER is extracted, the órgão expedidor is always typed** | `clientes.rg` / `clientes.rg_orgao_expedidor` | `rg`: AI-suggest+confirm OR manual `PATCH`. `rg_orgao_expedidor`: `PATCH /api/clientes/{cliente_id}` only | `rg`: `extracted_and_confirmable`; `rg_orgao`: `manual_entry_only` |
| `p.cpf` | §2.1 | RG/CPF/CNH (AI vision) | `CAMPOS` `item_key="cpf"`, `sobrescreve=False` (migration 097) | `clientes.cpf` | same two paths as `p.rg` | `extracted_and_confirmable` — **0 of 1 393 confirmed** |
| `p.email` | §2.1 | none | — | `clientes.email` (migration 068) | `PATCH /api/clientes/{cliente_id}` | `manual_entry_only` |
| `p.genero` / `p.g()` | §2.1 | RG/CNH (AI vision) | `CAMPOS` `item_key="genero"`, `sobrescreve=False` (migration 073) | `clientes.genero` | AI-suggest+confirm OR manual `PATCH` | `extracted_and_confirmable` |
| `p.endereco` / `n.endereco` | §2.1 | **none — no extractor exists for any address component** | — (`identidade_extracao_service.CAMPOS` has no `endereco*` entry — verified: `logradouro/numero/complemento/bairro/cidade/uf/cep` are absent from the tuple at `identidade_extracao_service.py:209-294`) | `clientes.endereco_logradouro/numero/complemento/bairro/cidade/uf/cep` | `PATCH /api/clientes/{cliente_id}` only | `manual_entry_only` — the ONLY path in this whole map with zero extraction candidate even in principle (a comprovante de residência is uploaded but never structurally read) |
| `n.tipo` (casados/uniao_estavel/individual) | §2.1 | derived | `concordancia.py` (`lado()`; reads `estado_civil` + `conjuge_cliente_id`) | none — pure function of the two fields above | — | `manual_entry_only` (via the two source fields) |
