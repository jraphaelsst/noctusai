...
- **Seed-side engagement-points domain:** `noctusai_lib.domain.engagement` (built 2026-09-16 for `products/community`) is the first data-driven, owner-tunable implementation of this file's § 8 mandate — `PointRule`/`RuleSet`/`evaluate` (pure, caps + idempotency), `BadgeRule`/`evaluate_badges` (no lambda conditions), `leaderboard` (reuses `metas.Period`), and a `PointsLedger` Protocol+Fake+RealSupabase+factory. ERP's `gamificacao_service.py` still hardcodes `PONTOS_POR_ACAO` + lambda badges — re-point deliberately deferred (triage at N=2, not a forced extraction); see the `NOC-REMEDIATE[seed-lift]` marker in that file and `project-history/roadmaps/erp-gamificacao-seed-lift-2026-09.md`. See `KB § CONTEXT/04-SHARED-LIBRARY.md § domain/engagement/` for the full symbol table.
