...
**Adopters:** ERP (`meta_api`, `whatsapp_webhook` call `webhook_endpoint(...)` directly) and Mailing (`webhooks/resend`). ERP's `assinaturas` webhook is a step removed since 2026-09-20 (wiring audit tasks 3+4): it never re-implements D4Sign's form-urlencoded wire shape — `integrations/signature/real.py::D4SignAdapter.validar_webhook` owns BOTH parsing and the `verify_hmac_sha256_hex`-backed `Content-HMAC` check, so `assinaturas` is still an adopter of this module's shared primitive, just via the signature adapter rather than a direct `webhook_endpoint()` call (the previous direct call used the WRONG scheme — `sha256_prefixed`/`X-Hub-Signature-256` — against a secret key nothing ever wrote to). Stripe SDK is the documented carve-out — it ships its own verifier; don't wrap it. → `KB § PATTERNS/security/webhook-signatures.md` for the four-shape catalog + universal rules.
...
| `is_forward_transition(atual, novo, *, terminais)` | Vocabulary-agnostic monotonic-status guard (added 2026-09-20, wiring audit task 4). Once `atual` is a member of the caller-supplied `terminais` set, only re-applying the SAME value is accepted — any different value is a regression and this returns `False` so the caller refuses + logs instead of silently overwriting a finished contract. Shared because a webhook body carries no nonce/timestamp: a captured `concluido`/`cancelado` delivery replayed out of order would otherwise regress the row forever. Two consumers, two vocabularies: `card_hub.assinatura_service.aplicar_evento_webhook` (social-wiring, native `StatusAssinatura`) and erp's `AssinaturaService.processar_webhook` (its own `assinaturas.status` CHECK vocabulary, mapped from `StatusAssinatura` first). |
...
### `photo-editing/` — Edição de Fotos seed organs

Seed-portable AI photo-editing UI, typed to `projects/edicao-fotos/EDICAO-FOTOS-CONTRACT.md`; zero Social-Wiring coupling so it travels unchanged to the owner's phase-2 standalone rebuild. `createEdicaoFotosHooks(api)` (`hooks.ts`, TanStack Query factory in the `llm.ts` style) covers capacidades + review/decisions + the reference pool (multipart pair upload, pool occupancy) + guias de estilo + platform settings — every read hook pre-computes `showSkeleton`/`isRefreshing` per the binding two-signal loading contract (contract §9), and `useRevisao` carries `placeholderData` across a `loteId` key change. `permissions.ts` is a pure, react-query-free set of guards (`podeVerVeredito` etc.) over the `Capacidades` shape from `GET /capacidades` — **never** derived from SSO metadata. `<PhotoReviewGrid/>` — expandable thumbnails, ✓/✗ decision (✗ requires a comment, mirroring the backend's 422), decisions always changeable, `falhou` state with reason + retry, AI verdict gated by the caller-supplied `podeVerVeredito` prop (hidden from corretores). `<BeforeAfterCompare/>` — drag-to-reveal before/after slider, "Imagem gerada com IA" badge for staging output. `<ReferencePairCard/>` — one reference-pool pair (room tag, edit types, note, archive-as-soft-delete; optional `formatComodo`/`formatTipoEdicao` label seams). All exported from `@noctusai/lib`.
