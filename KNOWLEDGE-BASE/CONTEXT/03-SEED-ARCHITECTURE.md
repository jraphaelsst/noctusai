...
3. If the seed only ships the Protocol + a Fake, name the gap explicitly. Two outcomes:
   ...
   - **N=2+ consumers needing the same gap** — DRY-recurrence rule fires (`KB § PATTERNS/architect/project-execution.md § 2.7`) → file the seed real-adapter follow-up project; consumers proceed against Fake until it lands.
...
Returns only the named subset of standard routers. `names` is a sequence of strings drawn from the `_STANDARD_ROUTERS` registry keys (`"health"`, `"notificacoes"`, `"team"`, `"llm"`, `"ai_outputs"`, `"ai_feedback"`, `"scheduler"`, `"status_paginas"`, `"auth"`). Unknown names raise `ValueError` naming every invalid key and listing the valid set. Order is preserved. `create_product_app()` calls this internally based on the `standard_routers=[...]` kwarg — products don't invoke it directly.
...
**Not in this registry (called directly, per-product parameters):** `noctusai_seed.api_keys_router.create_api_keys_router(deps, settings, *, specs, store_factory, get_current_user_org, ...)` and `noctusai_seed.whatsapp_connections_router.create_whatsapp_connections_router(deps, settings, *, store_factory, get_current_user_org, waha_base_url, resolve_webhook_base_url, ...)` — org-scoped managed-API-keys + multi-session WAHA-connection routers, lifted 2026-09-17 (`community uses social-wiring's mechanisms`, Slice A) from `social-wiring`. Both take product-specific parameters (spec list, store factory, testers, auth dependency) that don't fit the fixed `(deps, settings, product_name, version)` `_build_*` builder signature every `_STANDARD_ROUTERS` entry shares — same reason `create_auth_router(...)` documents its own direct-call path above. Backing stores: `noctusai_lib.security.api_keys` (encrypted per-org key store over `noctusai_lib.security.token_store`) and `noctusai_lib.integrations.whatsapp.connection_store` (multi-session, org-scoped WAHA connection store; field-level Fernet encryption via the SAME `require_fernet` check the api-keys store uses). `whatsapp_connections_router` supersedes the removed single-session `whatsapp_admin` standard router (no product ever opted into it — confirmed by a fleet-wide `standard_routers=[...]` grep before removal).
...
| `auth` | POST `/api/auth/login`, GET `/api/auth/me`, POST `/api/auth/logout`, POST/GET `/api/settings/api-tokens`, DELETE `/api/settings/api-tokens/{token_id}` | cookie session (`nai_session`) via `noctusai_lib.api.auth.session.make_get_auth_context` (+ `FakeApiTokenResolver` for `pk_*` bearer + no legacy-JWT bridge BY DEFAULT — a product passes its own `api_token_resolver` / `legacy_jwt_resolver` to `create_auth_router(...)` directly to override either) | `SessionStore` (Redis-backed when `settings.redis_url` set, SEC-3 encrypted-at-rest) + `<schema>.api_tokens` via `deps.get_admin_client()` + trusted-DB `public.noctus_users.org_role` via `deps.get_core_client()` (owner/admin gate — NEVER `user_metadata`) |
...
**`auth`**: `noctusai_seed.auth_router.create_auth_router(deps, settings)` — the httpOnly-cookie-session surface, promoted from the `social-wiring` fork (`erp-httponly-cookie-session-2026-07` roadmap, Slice 1b). `/login` mints a `SessionStore` row + sets `Set-Cookie: nai_session=...` (SEC-1: sign-in runs on a THROWAWAY `create_client`, never the shared admin singleton); `/me` projects the resolved `AuthContext`; `/logout` revokes the session upstream via `SessionRevoker` (SEC-4, best-effort-but-logged — never blocks the local cookie/row cleanup on a GoTrue outage) THEN deletes the row + clears the cookie. `get_session_store(settings)` / `get_session_revoker(settings)` are process-local singletons keyed by `id(settings)` — a product's OWN auth composition (e.g. a legacy-JWT-bridged `app/dependencies.py::get_auth_context`) MUST resolve the session store through the SAME accessor rather than calling `make_session_store(...)` again, or a session minted by this router's `/login` is invisible to a second in-memory `FakeSessionStore` instance (Redis-backed stores don't have this hazard — the shared Redis IS the state). Api-token endpoints gate on `public.noctus_users.org_role` via `deps.get_core_client()` (trusted-DB — NEVER `user_metadata`, the class of spoof the 2026-07-14 role-cascade audit closed elsewhere). `create_auth_router(deps, settings, *, api_token_resolver=None, legacy_jwt_resolver=None)` defaults BOTH kwargs to the "pure new-scheme, cookie-only" shape (`FakeApiTokenResolver()` — always-empty, no `pk_*` bearer resolves; `legacy_jwt_resolver=None` — no bridge) when a product opts into the bare `"auth"` standard-router registry name, since the fixed-signature `_build_auth_router(deps, settings, product_name, version)` builder has no seam to thread per-product overrides through. A product with a REAL DB-backed `ApiTokenResolver` and/or an existing legacy-JWT bridge (e.g. `app/dependencies.py`'s own composition) that needs `/me`/`/logout`/api-tokens to resolve `pk_*`/JWT bearers MUST call `create_auth_router(...)` directly (passing its real adapters) from its own module-registration seam INSTEAD of opting into `standard_routers=["auth"]` — inheriting the Fake/None defaults silently downgrades that product's real token resolution to always-401 on this router's routes specifically (its OTHER routers, composed via its own `get_auth_context`, are unaffected either way). No Real `DBApiTokenResolver` ships in the seed itself (Wave 2, product-schema-specific — each product's `api_tokens` table lives in its own schema).
...
| `standard_routers=[...]` | Opt into bundled seed routers (`health`, `notificacoes`, `team`, `llm`, `ai_outputs`, `ai_feedback`, `scheduler`, `status_paginas`). | Every product (core: `["health"]`; therapy: `["health","notificacoes","llm"]`; etc.). |
...
| Stray `.md` docs at repo root (audit handoffs, session recaps) | Violates `KB § PATTERNS/architect/project-execution.md § 11 Clean-folder principle`. | **Refactor** (absorb into appropriate project folder) OR **accept** (platform-wide file like `CLAUDE.md`). |
...
- Slug: `<product>-seed-wiring` (intent=`wiring` per `PATTERNS/architect/project-execution.md §8`).
