...
> ℹ️ **Provenance (KB-absorbed copy, 2026-06-02).** This is the durable, graph-queryable copy of the orbity absorption diagnostics, landed in `KB § ABSORPTIONS/orbity/` so the knowledge survives the source workspace's eventual deletion (`persistent-files-absorption`). The **knowledge here is self-contained**; the `sistema-orbity/...` source pointers are valid only until the user archives that **transient** external workspace (absorption Gate 9). The methodology this session refined lives in `KB § GUIDES/absorb-seed-workspace.md` ("learning-absorption refinement"). Working notes mirror this set under the source workspace's `_NOC_ABSORPTION/`.
