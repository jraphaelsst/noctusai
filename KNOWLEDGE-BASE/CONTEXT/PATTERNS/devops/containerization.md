...
| `[vite] Rollup failed to resolve "<dep>"` from `seed/.../*.ts` | a seed source import isn't in the product's `package.json` (Vite `dedupe` forces product `node_modules`) — a freshly `scaffold_product`'d product self-heals this at scaffold time (`ensure_framework_deps_for_product`); for an existing/drifted product, audit via `python mcp/noctusai/cli.py --check-framework-deps [--fix]` (auto-derives `@noctusai/lib` organ transitive deps from `seed/lib/frontend/src` — not just the hand-curated `FRAMEWORK_DEPS` list — since the 2026-07-20 `KanbanBoard`/`@dnd-kit/*` incident; `seed/framework/frontend/src` deps stay manual-only, see `organ_transitive_deps` module docstring). `--fix` borrows a real range from the seed's own `package.json`(s) first, the donor product second — never writes `"*"`; a dep with no range anywhere returns `status=fix_unresolved` and touches nothing, naming the dep. |
...
**Prevent.** One canonical checkout per machine. Sanctioned exceptions are explicit and read-only: per-agent worktrees (`KB § PATTERNS/architect/branching-and-merging.md`), template/sibling workspaces consuming noc read-only (`KB § PATTERNS/architect/seed-workspace.md`, with chmod -R a-w). Anything else is a second clone, and a second clone is a drift hazard.
