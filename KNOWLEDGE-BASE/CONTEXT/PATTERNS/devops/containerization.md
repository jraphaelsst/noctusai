...
| `[vite] Rollup failed to resolve "<dep>"` from `seed/.../*.ts` | a seed source import isn't in the product's `package.json` (Vite `dedupe` forces product `node_modules`) — add it; audit via `python mcp/noctusai/cli.py --check-framework-deps [--fix]` (auto-derives `@noctusai/lib` organ transitive deps from `seed/lib/frontend/src` — not just the hand-curated `FRAMEWORK_DEPS` list — since the 2026-07-20 `KanbanBoard`/`@dnd-kit/*` incident; `seed/framework/frontend/src` deps stay manual-only, see `organ_transitive_deps` module docstring) |
