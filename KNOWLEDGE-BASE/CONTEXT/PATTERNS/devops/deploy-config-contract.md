...
- **Boot side** — `create_product_app` folds the baseline into `require_prod_config`, **plus** the product's own `required_prod_config=[...]` opt-in (§6 table row) — both sources land in the SAME aggregated `_required_prod_keys` list before the single boot-time check. A deploy missing any key from either source now aborts loudly at startup, aggregated, instead of a deep lazy error (or, for a product-declared key with no seed-level enforcement at all, a *silent* one — the shape `ENCRYPTION_KEY` closed for social-wiring/p-studio's `credential_vault`).
- **Gate side** — `noctus.dev.predeploy_check` check `required_prod_env_present` folds the SAME two sources: `_load_baseline_required_env()` (the fleet-wide baseline) **plus** `load_product_required_prod_config(root, product)` (an AST-static read of the product's own `create_product_app(required_prod_config=[...])` call — never an import, so the gate never needs the product's full runtime env just to answer a question about a literal list). Both folded lists are diffed against the prod `.env` snapshot, catching a missing/empty key **before** the deploy. A non-literal `required_prod_config` argument (the reader genuinely cannot resolve it statically) SKIPS loudly (`ok=True`, message names the product) rather than silently checking zero keys. Failure classifies as `required_prod_env_missing` (B4 container env).
