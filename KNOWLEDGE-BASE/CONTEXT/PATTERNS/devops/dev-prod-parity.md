...
- **Reading config?** Know the layer: build-time (`VITE_*`, baked ⇒ rebuild) vs runtime (`getenv`, per-request ⇒ restart/recreate). Document which, so the deployer knows rebuild-vs-restart.
...
- **FE bundle** — confirm the baked bundle carries the prod URL (¬ `http://localhost:<port>`); a bundle-hash unchanged on a backend-only change is a good signal.
...
- **Risky cutover** — keep the working override in place during the switch; remove it only **after** the new path is live-verified. (The 2026-05-22 first CORS cutover failed registry-empty but never left prod broken — the override protected it until verified.)
- **Container freshness** — if the running container serves code this session changed, confirm runtime parity (`KB § PATTERNS/devops/containerization.md § 12b`). *Even a fresh container can be prod-wrong if the code assumed dev parity* — this rule is the layer above freshness.
...
- `KB § PATTERNS/backend/boundary-contract-tests.md` **B4** — container env propagation (`.env` ↔ compose ↔ stage chain ↔ container).
- `KB § PATTERNS/devops/containerization.md § 12b` — a stale running container ("tests green" ≠ "container reflects the fix").

---

## 6 · Slip history + the remediation that closed each (the N≥3 that born the rule, 2026-05-20 → 22)

Each was shipped by a prior agent who verified **dev**, not **prod**:

1. **`infra.tsx` localhost default** — defaulted the backend URL to `http://localhost:8000`; fine for core, misrouted every other product's FE → CORS → "Servidor indisponível" toasts. **Fix:** canonical default (`?? ""`, throw on unmapped) + propagate → [[seed-canonical-defaults]].
2. **Cross-product nav → localhost after the prod deploy** — no `PRODUCT_URL_*` override ⇒ fell to the DB `url_base` (seeded localhost **by design**) ⇒ prod SSO'd users to `http://localhost:8080`. **Fix:** set `PRODUCT_URL_PATTERN=https://{slug}.noctusai.com` + per-product overrides on the VPS `.env`; recreate `core` (runtime config, no rebuild). DB stays localhost by design.
3. **CORS empty in the slim prod container (the sharpest)** — `derive_cors_origins` built CORS from the `start.sh` PRODUCTS registry, but **the slim prod image ships NO `start.sh`** ⇒ registry empty ⇒ CORS collapsed to localhost-only ⇒ apex login down. Even a *canonical* default wouldn't have saved it: the **derivation source itself was absent in prod**. **Fix:** made `derive_cors_origins` deploy-aware — in `@registry:all` mode it resolves each registry slug via `resolve_product_url` **and** scans `os.environ` for `PRODUCT_URL_<SLUG>` keys **directly** (registry-free); immediate stopgap was an explicit `CORS_ORIGINS` env list, verified by preflight.

**CI sibling (same class):** test-dep superset drift — `test_audit_hook` imports `sqlalchemy`, `test_dimob_service` imports `defusedxml`; both lived in a product's per-product `requirements.txt` but not the **root superset** the CI backend jobs install ⇒ `ModuleNotFoundError` in CI only. **Fix:** added the deps to the **root** `requirements.txt` with an inline comment naming the importing test. The common thread: *dev/local/full-image had X; prod/slim-image/CI-superset did not; nobody checked B.*

---

## 7 · Detection status (codification honesty)

- **Wrong-value sub-class** (localhost default) → **Stage-4**: `check_seed_canonical_default` keeper ships, 0 baseline → [[seed-canonical-defaults]].
- **Derives-from-dev-only-artifact sub-class** (the registry-empty case) → **Stage-4**: `check_derives_from_dev_only_artifact` keeper ships ([[deploy-config-contract]] static leg), paired with the `prod_config_parity` pre-deploy gate (value-correctness) + the `create_product_app` boot guard (runtime presence) = the contract's three legs.
...
**Doc anchors.** Memory: `feedback_dev_prod_parity_verify_in_prod_shape` · `reference_cross_product_nav_url_resolution` (URL/CORS specifics) · `feedback_ci_layered_rehab` (the CI superset shape) · `feedback_deploy_config_contract` (executable form). CLAUDE.md: the "Finish the session — verify" §1 bullet + §2 Map pointer. Siblings: [[deploy-config-contract]] (executable form) · [[seed-canonical-defaults]] (value-correctness) · [[boundary-contract-tests]] B4 · `KB § PATTERNS/devops/containerization.md § 12b` (stale container). Guide: `KB § GUIDES/production-deploy.md` §6. Born 2026-05-22 (nav-remap → prod-CORS session); doc authored + reconciled 2026-05-23.
