...
2. **Lockfile↔package.json sync (the static prerequisite)** — the base Dockerfile uses `npm install` (forgiving: installs from `package.json`, mutates the lock) rather than `npm ci` (reproducible, but **fails hard** if the lock is out of sync with `package.json`). Reproducible builds want `npm ci` — but only safe once the lock is provably complete (every declared dep ∈ `package-lock.json`). This is now guarded per-product by `check_product_lockfile_dep_sync` (`KB § PATTERNS/devops/product-lockfile-and-slug-drift.md` — shipped 2026-07-22 after the class recurred 3× in one session: `@dnd-kit`, `recharts`/`@radix-ui-react-tabs`, and the sibling `ALL_SLUGS` shape). **Sync-drift remediation is NOT a bare `npm install`** — it re-resolves the whole tree (535 unrelated version bumps measured from doing this naively on one product). Use a scoped `npm install <dep>` or a surgical lockfile splice; see that doc's "The `npm install` trap" section.
