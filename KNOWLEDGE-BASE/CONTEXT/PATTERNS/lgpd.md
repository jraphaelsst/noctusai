...
- **Components** live in `seed/frontend/lib/src/design-system/ai/`: `<AIConsentToggles/>` (settings panel, grouped by product, "padrão" annotation when `!decision_recorded`, locked + "infraestrutura" pill when `!toggleable`), `<PendingConsentBadge/>` (compact nudge linking to `/settings/ai`, null-renders when nothing to nudge), plus the supporting `useConsents` + `useUpdateConsent` hooks.
- **Page hosting them** lives in `seed/frontend/framework/src/pages/ConsentSettingsPage.tsx` — title + LGPD-friendly intro paragraph (forward-only revocation explained inline) + `<AIConsentToggles/>`.
- **Route auto-injection** — `createProductApp` in `seed/frontend/framework/src/app.tsx` injects `<Route path="/settings/ai" element={<ConsentSettingsPage/>}/>` into both flat and role-based route trees BEFORE product-specific routes. Authenticated users of any role can manage their own consents.
- **Layout slot default** — `seed/frontend/framework/src/layout.tsx` resolves `enrichment.aiBadge !== undefined ? enrichment.aiBadge : <PendingConsentBadge/>`. Products pass `null` to explicitly opt out, or any React node to override (Wave 5's `<AIBadgeStack/>` composes multiple badges here).
