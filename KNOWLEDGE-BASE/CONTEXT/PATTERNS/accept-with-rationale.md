...
### Calendar credential seam — `CalendarCredentialResolver` Protocol over spec'd `Callable` — **ACCEPTED 2026-05-03**
...
- **Decision:** accept the Protocol-based seam as shipped; do not refactor back to a bare Callable.
- **Reason:** the Protocol's typed-union return lets `get_calendar_adapter(resolver, tenant_id)` dispatch on credential KIND (SA vs OAuth vs Fake fallback) inside the factory itself — consumers don't have to encode adapter-selection logic per product. A bare Callable would have either forced consumers to pick the adapter class explicitly OR required runtime type-sniffing on a duck-typed return. Same DI shape (consumer constructs and passes once at app-start); strictly stronger types. The rest of the seed (`noctusai_lib.domain.scheduling`, `noctusai_lib.integrations.google_maps`) uses Protocols for analogous seams — Protocol is the existing convention.
- **Scope:** `noctusai_lib.integrations.google_calendar.credentials` only — both real adapters consume `CalendarCredentialResolver`.
- **Revisit trigger:** effectively "won't revisit." If a consumer's storage shape is naturally a closure, the Protocol's single-method shape is one line to implement around it (`class _CallableResolver: def get_credentials(self, tenant_id): return my_callable(tenant_id)`) — no migration cost.
- **Recorded by:** `projects/google-calendar-real-adapters/` close-gate (2026-05-03).
