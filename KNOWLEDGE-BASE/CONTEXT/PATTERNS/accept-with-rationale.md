...
- **Subject:** `KB § PATTERNS/mcp-first-scripts.md` mandates new automation default to a `noctus.dev.*` MCP tool. 16 absorbable scripts were ported + deleted 2026-05-18 (`scripts-mcp-absorption`). 9 remain shell.
- **Decision `[A]`:** these 9 stay shell, NOT absorbed: `pre-commit` (`[carve:hook]` — thin dispatcher; logic IS in `noctus.dev.*`, the shell entry is the only carve-out part) · `install-hooks.sh` · `setup.sh` · `first-time-setup.sh` · `bootstrap-worktree.sh` · `bootstrap-seed-workspace.sh` · `build-init-local-db.sh` (`[carve:bootstrap]` ×6 — run *before* the venv the MCP lives in exists; they *create* it) · `build-base-images.sh` + `build-and-push.sh` (`[carve:docker]` ×2 — whole body is `docker build`/push plumbing of the seed base + product images; a Python wrapper would only shell out. `build-and-push.sh` relocated from `projects/production-deploy-migration/` → `scripts/infra/` 2026-05-24: a permanent CI surface MUST live outside `projects/` — it broke CI with exit 127 when that project was archived; durable-refs gate now prevents the class).
...
- **Scope:** the 9 files above, each carrying its `[carve:*]` row in `KB § PATTERNS/mcp-first-scripts.md` §3 (the manifest is the keeper-parsed single source of truth; this entry is its 1:1 rationale pairing).
