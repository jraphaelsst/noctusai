...
| Playwright E2E | ⚠️ 2/11 wired, **specs LIVE** (updated 2026-06-10; was "0 specs, dormant") | core (6 specs) + erp-imobiliario (8 specs) under `e2e/tests/*.spec.ts` — config pattern: `webServer` boots vite dev, backend mocked via `page.route` fixtures, chromium-only, role/label selectors (see this doc's selector-stability note). social-wiring is being wired as the 3rd adopter (mailchimp slices, in flight 2026-06-10). Remaining products: not wired. |
...
| Frontend Playwright specs in core + ERP | ✅ SHIPPED (core 6 + erp 8 specs as of 2026-06-10; social-wiring in flight) — row retained for the remaining unwired products | `<product>-playwright-golden-paths` |
