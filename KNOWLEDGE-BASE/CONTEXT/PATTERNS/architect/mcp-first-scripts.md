...
| `claude-guard-primary-write.py` | [carve:hook] | `hooks/claude-guard-primary-write.py`; PreToolUse adapter for the self-branching write gate. CANNOT be a `noctus.dev.*` tool: it runs in the harness's pre-tool path on EVERY Bash/Edit/Write call, where the MCP server is not reachable and a round-trip would be unaffordable (~60 ms budget; importing `compliance.py` alone costs 0.27 s). The logic it calls IS library code — `tools/noctus/dev/primary_write_guard.py`, stdlib-only, imported BY PATH — so the decision lives in the toolkit and only the protocol adapter is a script. See `KB § PATTERNS/common/self-branching-mode.md` §11 |
| `claude-guard-test-seams.py` | [carve:hook] | `hooks/claude-guard-test-seams.py`; PreToolUse adapter for the no-self-monkeypatch WRITE gate. Same structural carve-out as its sibling above and for the same reason — it runs in the harness pre-tool path on every Edit/Write, where the MCP server is not reachable. The logic IS library code (`tools/noctus/dev/test_seam_guard.py` + the shared `self_patch_predicate.py`, stdlib-only, imported BY PATH); only the protocol adapter is a script. The stdlib-only constraint is load-bearing, not stylistic: the first version imported `compliance.py` (pydantic at module scope), the hook failed OPEN under the system interpreter, and the guard silently never fired. See `KB § PATTERNS/compliance/testing.md` |
| `claude-guard-harness-signature.py` | [carve:hook] | `hooks/claude-guard-harness-signature.py`; PostToolUse adapter that flags a Bash call whose non-zero exit matches a known harness-failure signature (2026-09-20). Same structural carve-out as its two siblings above, one event earlier in the tool lifecycle: it runs on EVERY Bash call, where the MCP server is not reachable and a round-trip would not fit the per-call budget. The logic IS library code — `tools/noctus/dev/harness_signatures.py`, stdlib-only, imported BY PATH, SHARED with `noctus.dev.gate_sweep`'s own harness-suspect classifier — so only the protocol adapter is a script. See `KB § PATTERNS/common/methodology-execution-discipline.md` § 10 |
| `claude-guard-primary-write-post.py` | [carve:hook] | `hooks/claude-guard-primary-write-post.py`; PostToolUse half of the primary-checkout write guard (2026-09-23): diffs the `git status` baseline its PreToolUse sibling captured against the real state after a Bash call and surfaces genuinely new primary dirt — a safety net that never blocks. Same structural carve-out as its siblings above: it runs on EVERY Bash call, where the MCP server is not reachable and a round-trip would not fit the per-call budget. The logic IS library code — `tools/noctus/dev/primary_write_guard.py`, stdlib-only, imported BY PATH — so only the protocol adapter is a script. See `KB § PATTERNS/common/self-branching-mode.md` §11 |
...
- `KB § PATTERNS/common/accept-with-rationale.md` (carve-out rows pair 1:1)
- `KB § PATTERNS/architect/dev-toolkit-scaffolders.md` (`scaffold_mcp_tool` emits the default skeleton)
- `KB § PATTERNS/common/methodology-codification-pipeline.md` (this rule reached `s4` 2026-05-18 — keeper shipped same session as the doc)
