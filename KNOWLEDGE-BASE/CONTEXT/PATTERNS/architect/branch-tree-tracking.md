...
## 2 · The ledger — `branch-tree.ndjson` on the orphan `origin/ledgers` branch

Append-only ndjson. **Since 2026-09-24 it lives on the orphan `origin/ledgers` branch**, written by git plumbing through `_ledger_store` — a pointer write is **never a commit on `dev`** (owner decision; 465 `chore(branch-pointer)` dev commits since 2026-08-01 were this ledger, each one cancelling CI and racing the next). Globally accessible: every read is the **dual-read** — `origin/ledgers` ∪ the legacy `project-history/branch-tree.ndjson` dev copy (a peer on stale code may still push there until it restarts) — so every agent sees the same cross-branch picture. The dev copy, the drains and the ledger-drain keeper go at S4 (`project-history/roadmaps/ledgers-off-dev-2026-09.md`, trigger T1). Mechanics: `KB § PATTERNS/common/ledger-store.md`.
...
**🔴 2026-09-17 — the ON-DISK path itself must resolve to the PRIMARY checkout (fifth instance of `KB § PATTERNS/common/claim-vs-evidence-shared-state.md`'s family).** (Historical since 2026-09-24: `LEDGER_PATH` is now only the dual-read's dev-copy half and the Fake store's backing file; rows publish to `origin/ledgers`.) `branch_pointer.LEDGER_PATH` (and the since-deleted `MIRROR_PATH`) derive from `settings.LEDGER_ROOT` (`workspace.get_ledger_root()`), never `settings.REPO_ROOT` — the latter deliberately STOPS at a `.claude/worktrees/<slug>` boundary (correct for code-editing tools), so a ledger writer trusting it inherits the wrong guarantee if the MCP server happened to boot with cwd inside a worktree. This is defense-in-depth, not the primary safety net here: `append`/`update`'s commit+FF-push-to-`origin/dev` in the SAME call (§4 below) already makes a successful row durable regardless of which checkout hosted the commit — the constant only matters on a push FAILURE, where the row would otherwise sit uncommitted wherever `LEDGER_PATH` resolved.

### The mirror — `branch-tree.mirror.ndjson` (DELETED 2026-09-24)
Until 2026-09-24 a second, byte-identical `project-history/branch-tree.mirror.ndjson` was written on every append and `check_branch_tree_mirror` hard-blocked any parity drift. The owner decided to delete it when the ledger moved to `origin/ledgers`: one ledger, one writer (`_ledger_store`), nothing to keep in parity. The file is gone from `dev`, `branch_pointer` no longer writes it, and the keeper's parity leg is removed. A stale-code peer may still recreate it in the primary checkout until it restarts; the cache exemption and the session-end drain still list the path so that leftover is shipped instead of left dirty (both go at S4).
...
2. **Engineer, right BEFORE self-branching** — `task_branch action=start` publishes your claim (`status=on_going`) to **`origin/ledgers`** (a plumbing FF push — no dev commit, no CI run). Pass your `paths` and `brief` so the collision zone is real *before* you touch a file.
3. **Engineer, on every commit to the orchestrator branch** — append an updated pointer (refresh `commit`, `status`, append to `notes` mirroring the commit message); it publishes to `origin/ledgers` in the same call. Plus **mid-flight** whenever it fits (entering `blocked`, widening `paths`, a finding worth recording).
...
The **global read is always the dual-read** (`origin/ledgers` ∪ dev's legacy copy) so every agent at every branching level sees the same updated truth in real time; misinformation is designed out by the no-skip + publish-on-every-write rules.

### 🔴 Cache-sync discipline — pointer pushes MUST be lag-free
A pointer write happens **constantly** (before self-branch, every commit, mid-flight). It MUST NOT trigger the heavy cache-refresh hooks (noc-graph / embeddings) — that's the multi-minute lag the platform already hit on every commit. **Since 2026-09-24 this holds by construction:** a pointer publish is a push to `refs/heads/ledgers` only, and `scripts/hooks/pre-push` takes a ledgers-only fast path (no refresh, no keeper; delete/rewind still refused) — and no CI workflow triggers on `ledgers`. The legacy dev-copy exemptions below remain for stale-code peers until S4:
...
Writes go through `_ledger_store` (`KB § PATTERNS/common/ledger-store.md`): fetch `origin/ledgers` → hash-object → mktree → commit-tree → FF push, retried on the race — never a checkout, never the working tree or index, never a commit on `dev`. (Until 2026-09-24 this was the stage → commit → FF-push-to-dev idiom; it is retired.)

| action | signature | does |
|---|---|---|
| `append` | `branch, base, commit, role, agent, parent, paths, status, brief, notes?, worktree?, session?, project?, push_dev=True` | append a row; `push_dev=True` publishes it to `origin/ledgers` at once, `False` spools it for the next publish (the flag name predates the move) |
...
| `query` | `from_dev=True, status?, branch?, agent?, project?, paths_overlap?` | resolve latest-per-branch from the **dual-read** (`origin/ledgers` ∪ dev's copy); `paths_overlap=[…]` returns branches whose collision zone intersects (the pre-dispatch planner) |
| `list` | `from_dev=True, include_terminal=False` | the live map: all non-terminal pointers (add terminal with the flag) |

`query`/`list` default `from_dev=True` — read the global copy (`origin/ledgers` ∪ `origin/dev`'s legacy file), not the local branch's. Append/update default `push_dev=True` — the no-skip guarantee is the default, not an opt-in.
...
In `compliance.py` (+ keeper-pattern-cache mirror + `--check-branch-tree-mirror` cli flag), wired into the **pre-push** hook. It reads the dual-read (`_branch_tree_text`: the pushing tree's dev copy ∪ the local `origin/ledgers` ref — no fetch); the file-parity leg went with the mirror file on 2026-09-24. A push to dev is **blocked** unless, for the branch being pushed:
