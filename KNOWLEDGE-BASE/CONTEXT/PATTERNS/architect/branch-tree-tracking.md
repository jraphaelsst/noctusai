...
1. **`integrated-worktree-live`** (documented in the enum table above) — `_autoheal_branch_pointers` now looks up whether the branch's `.claude/worktrees/<slug>` directory still exists (via the SAME `_worktree_branches` parse the rest of the module already does) before flipping; if it does, it writes this non-terminal status instead of `shipped`. `NOC-REMEDIATE[integrated-worktree-live-reconcile]`: once the directory is LATER removed, this pointer is not automatically promoted back to `shipped` by this pass — cleanup is never blocked forever in practice (the PHANTOM path in `cleanup_stale_worktrees`/`mole` removes the git worktree registration once the directory is gone, independent of pointer status), but the ledger value itself needs an explicit re-heal; a future sweep enhancement can close that residual gap.
