...
**CI validates `dev` continuously (not only at the `main` boundary).** `test.yml` (the full Tests & Build matrix) and `seed-typecheck.yml` run on **every push to `dev`** ∧ PRs targeting `dev` ∨ `main` — so the `dev` tip is proven CI-green *as work lands*, and the "CI green" evidence a bless needs is already satisfied (confirm via the latest `dev` run + `noctus.dev.release stage='status'`). `build-and-push.yml` stays **`main`-only** by design (it ships GHCR deploy artifacts — those belong to the release line, not every integration push). This closes the gap where a `dev`-landed failure (a Bandit B324 in KE's `google_drive/fake.py`) surfaced only at bless-time, on a `dev→main` PR opened solely to trigger CI. Born 2026-05-24 — the failure that prompted the trigger move IS the argument for it ([[safety-nets-become-learnings]]).

**Bless targets the LIVE `dev` tip — re-verify it equals your CI-checked sha (TOCTOU).** `release stage='bless'` re-fetches and FFs `main` to whatever `origin/dev` is at *confirm* time — **NOT** a pinned sha (only `promote` takes `sha=`). If a concurrent session pushes to `dev` between your `status`/dry-run (showing sha X) and the `confirm` (now sha Y), you bless **Y** — potentially commits you never CI-verified onto sacred `main`. Mitigations, in order: (a) bless only when `dev` is **quiet** — no concurrent pushes ([[concurrent-agents-never-share-one-checkout]]); (b) **immediately before `confirm=True`, assert `origin/dev == <your-verified-sha>`** (the dry-run already prints `dev_sha` — diff it against the sha whose CI you confirmed); (c) tooling follow-up — add an `expect=<sha>` guard to `bless` that refuses if `origin/dev` moved. Bit 2026-05-24: verified CI on `b64d2966`, blessed `c72f7b3e` — 3 concurrent `svg_render` commits landed between dry-run and confirm; they happened to be CI-green so no harm, but that was **luck, not the gate**.
...
**Coordination via the patterns log (master-tree case).** When branches are part of a master-tree-parallel-batches project (`KB § PATTERNS/architect/master-tree-parallel-batches.md`), the master root's `live-patterns-log.md` is the coordination surface. Each batch's branches land in order by batch number; mid-batch parallelism happens within a batch but the cross-batch ordering is sequential.
...
**Worked example — same-line conflict on a KB doc.** Imagine two branches both edit `KB § PATTERNS/architect/project-execution.md § 2.7 Recurrence rule`:
...
**The shape.** When the user invokes a **master-tree project** (orchestrator project that derives N child projects, executable in parallel or serial — see `KB § PATTERNS/architect/master-tree-parallel-batches.md`), the branching shape is hierarchical:
...
3. **Cross-pollination respects the tree.** Per `KB § PATTERNS/architect/master-tree-parallel-batches.md`, children share findings via the master root's scratchpad. Hierarchical branching ensures children see updates to the scratchpad as the master commits them.
...
The existing collision protocol (`feedback_parallel_agent_collision_protocol.md` + KB § PATTERNS/architect/project-execution.md § 2.9) covers the **explicit revert** shape: parallel agent re-edits the same lines you edited; STOP at the second revert; wait. This subsection covers the **silent regression** shape that surfaced multiple times during `containerization-backlog-closure` Wave 1+2 merging.
...
- An **accepted divergence** (a real deviation from ideal, knowingly kept) → `KB § PATTERNS/common/accept-with-rationale.md`.
...
A **new collision pattern** caught during the 6-branch convergence (dogfooding §21.9). **C2 (same-file-additive) is concat-clean ONLY when the branch is based off the *current* integration tip.** A C2 branch forked off an **older base** still conflicts — not on its genuine addition, but because it carries a **stale copy of an *unrelated* line** the integration tip has since superseded (e.g. an old doc-symbology row vs the tip's caveman-extended row). git surfaces this as a same-region conflict even though the branch never meant to touch that line.

- **Resolution rule:** for the conflicted region, **keep-HEAD for every unrelated/stale line** (the tip is the superset) ∧ **keep-branch only for its genuine addition** (the lines the branch exists to add) ∧ **drop the branch's stale duplicate** of the superseded line. Verify-kb-sync / pytest is the post-resolution oracle, not eyeballing.
- **Prevention (the real fix):** §21.3 already says rebase the worktree onto the stabilized tip once the parallel agent is done — extend it: **a C2 branch that will merge late should be rebased onto the integration tip *before its engineer is dispatched* whenever the tip is stable**, converting C2-off-stale-base → true-C2-off-tip (concat-clean, zero stale-dup). Stale-dup conflict is the *symptom* that the base was older than it needed to be.
- Anti-pattern: resolving a stale-dup conflict by surgical marker-edits (error-prone — observed this session); always replace the **whole conflict block** in one edit with the resolved content.

**Three-way-synced 2026-05-18**: this section (§21, incl. §21.9 + §21.10 — the latter absorbed mid-convergence via §21.9, dogfooded) + memory entry `feedback_collision_class_branching.md` + CLAUDE.md §1 wave-based-dispatch bullet (amended clause + pointer — §21 extends the existing bullet, no new bullet). The global meta-rule it instances: `KB § 01-PHILOSOPHY.md § Always-hardening` + CLAUDE.md §1 `Always-hardening posture` bullet + memory `feedback_always_hardening_posture.md`.
