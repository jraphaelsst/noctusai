...
## Composes with
[[branching]] (the unified primitive) · [[branching-and-merging]] (§0 sacred-main + §2b promote ritual) · [[fe-be-contract-first-dispatch]] + `KB § GUIDES/production-deploy.md § 0.1` (the dev-validation gate) · [[self-branching-mode]] (never on `dev`) · `KB § PATTERNS/devops/prod-exposure-consent.md` (the per-product consent gate bless/promote never made).
