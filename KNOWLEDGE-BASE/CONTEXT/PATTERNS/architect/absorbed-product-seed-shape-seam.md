...
| `noctusai_lib.integrations.youtube.YoutubeClient` | new methods `get_channel_info_mine` + `list_owned_videos` + `get_video_full` + new `VideoFull`/`ChannelInfo` value objects (existing `Video`/`Channel` unchanged) | **Projection-mismatch axis.** Seed `Video` projected a strictly narrower field set than the chatbot UI consumed (missing `thumbnail_url`/`duration_string`/`privacy_status`/`like/comment_count`/`tags`/`category_id`). Forcing through seed = degrade-consumer (forbidden). **Method-pairing pattern**: lean `get_video` + rich `get_video_full` both ship; callers pick the projection; self-documents the trade-off. | Pilot-gate verified no-op. |
| `noctusai_lib.integrations.google_drive.DriveReader` | enriched `DriveSearchHit` (`parents`/`owners`/`icon_link`/`raw` + `.file_id`/`.modified_at`/`.is_folder` properties) + thicker `DriveFileContent` (`raw_mime` + lazy-`.text` via media seam + `.bytes_read`) + new `SyncDriveReader` facade + `translate_rendered_as` bi-directional vocabulary | **Projection-mismatch axis + async/sync split + vocabulary divergence.** Seed `DriveReader` was async-only with narrower projection than the chatbot UI; product surface was sync; legacy `csv`/`text`/`pdf-text` vs canonical `text/csv`/`text/plain`/`passthrough`. Dual-accept translation + sync facade + lazy media composition preserves chatbot tool descriptions LLM-trained on legacy vocab. PDF-text lifted to seed `media.extract_pdf_text` (N=2 dedup vs `OpenAIMediaResolver._pdf_text_layer`). | Pilot-gate verified no-op. |

**Recurrence threshold passed N=5 (across 4 distinct axes):** *write-payload-shape* (token_store) · *route-prefix* (oauth_router) · *security-control* (PKCE) · *projection-mismatch* (youtube ×1, drive ×1). Per the DRY recurrence rule (`KB § PATTERNS/architect/project-execution.md § 2.7`), N≥3 ⇒ MUST formalize; this doc IS the formalization and is now broad enough that future agents triage by axis first.

**Vocabulary translation appendix (Phase 6a-drive 2026-05-19):** when an absorbed product uses a different ontology than the seed for an enum-like classifier (`rendered_as`, `severity`, `priority`, …), prefer **bi-directional translation** (`translate_X(label, to=canonical|legacy)`) with both vocabularies passing through verbatim for unknowns — NOT silent rename of the seed canonical to match the legacy. Preserves any LLM tool-descriptions / external API contracts trained on legacy strings. Add at module level alongside the `RENDERED_AS_*` constants.
...
- **Stage 4 (s4) candidate:** a deterministic keeper that flags a seed-primitive Real-adapter's write-payload key set ⊄ the consumer's migration DDL columns (the false-green class). Routed to `phase_learnings`; not built yet (see `KB § PATTERNS/common/methodology-codification-pipeline.md`).
...
- DRY recurrence rule (why N≥3 ⇒ MUST formalize) → `KB § PATTERNS/architect/project-execution.md § 2.7`
- Seed Fake+Real factory shape (the seam expectations) → `KB § PATTERNS/backend/seed-fake-real-adapter.md`
