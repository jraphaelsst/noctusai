...
3. **Continue with non-colliding deliverables.** The collision blocks ONE seam, not the whole project. Ship what's independent + catalog the deferred work in `KB § PATTERNS/common/accept-with-rationale.md` so the design intent survives folder deletion + the wait window.
...
→ catalog the deferred work in `KB § PATTERNS/common/accept-with-rationale.md`. No collision-report project.
...
**Rule.** A deferral *or* an accept-with-rationale whose **resolution path is clear / recommended / resolvable-now within scope** is **resolved in-flight** and lands in the **SAME commit as the deliverable** (commit-on-ship, `KB § PATTERNS/architect/branching-and-merging.md §21.4`). It is **¬ parked for later**. Parking/deferring is the *exception*, reserved strictly for genuinely-unclear ∨ out-of-scope ∨ needs-a-user-decision ∨ hard-to-reverse work — and that exception is itself **announced loudly** (the LOUD training contract, `KB § 01-PHILOSOPHY.md § Always-hardening`).
...
**Three-way-synced 2026-05-18**: this §2.13 + `CLAUDE/projects.md` pointer + memory `feedback_in_flight_resolution.md` + MEMORY.md line. Companion: `KB § PATTERNS/common/defer-is-not-resolve.md` (R5), `§21.4` commit-on-ship, `KB § PATTERNS/common/accept-with-rationale.md`.
...
**Three-way-synced 2026-05-20**: this §2.14 + memory `feedback_recovery_brief_env_fingerprint.md` + MEMORY.md line. Companion: `KB § PATTERNS/common/methodology-codification-pipeline.md` (s2-s4 codification of the env-drift class), `noctus.dev.dispatch_preflight` (the deterministic predicate that backs the rule). Sibling rule for the doc-side dispatch slip: K-3 is structurally cured by the same preflight tool's `project_slug` argument (`projects/<slug>/PROJECT.md` auto-include).
...
The "three-layer discipline" in `KNOWLEDGE-BASE/CONTEXT/PATTERNS/compliance/testing.md` is the default for every phase — **not a project-level choice.** When a phase writes a new router, a new service function, or a new migration, the same phase writes the router tests, service unit tests, and any integration paths required. Skipping tests to "backfill later" violates the methodology; a phase that shipped without its tests is not `✅`, it's `⏳ (tests deferred)`.
...
**Durable-refs gate — a project may not be archived while a durable surface still points into it (DETERMINISTIC, not curatorial).** Distinct from learn-before-archive (which is judgment): this is the **N≥3 recurrence** where a permanent surface — a **CI workflow / `scripts/` entry / `.mcp.json` / compose / Makefile / KB doc** — references a path inside `projects/<slug>/` and **breaks the instant the project is archived** (`build-and-push.yml` → `bash: projects/production-deploy-migration/deploy/fleet/build-and-push.sh: No such file or directory`, exit 127, GHCR delivery red for days — 2026-05-24). `noctus.dev.archive(mode="project")` `git grep`s every tracked file outside `projects/` + `archive/` (excluding the sanctioned `project-history/` index) for `projects/<slug>/` and **REFUSES** with the offending lines listed, until each reference is relocated to a durable home (`scripts/infra/` for build/deploy orchestration, `seed/` for shared code, inlined into KB) **and its referrer updated** — THEN re-archive. `allow_durable_refs=True` overrides only a vetted, genuinely-non-breaking mention. This is the executable/config half of [[durable-docs-self-contained]] (`KB § 01-PHILOSOPHY.md § Durable docs are self-contained` — "Durable ≠ only docs"). Test: `mcp/noctusai/tests/test_archive.py::TestDurableRefsGate`.
...
**Branch-per-feature workflow** (per `KB § PATTERNS/architect/branching-and-merging.md § 11`): features get the same branching treatment as projects. Branch first, file feature, implement, commit on branch, push branch, orchestrator-merges to main.
...
- **Skipping the feature.md and just branching.** The branch-first-then-file-then-implement order (`KB § PATTERNS/architect/branching-and-merging.md § 13`) requires the feature.md to be filed before implementation starts. The .md is the brief; without it, the work is undescribed.
...
- `KNOWLEDGE-BASE/CONTEXT/PATTERNS/compliance/testing.md` — three-layer test discipline referenced by §9.
