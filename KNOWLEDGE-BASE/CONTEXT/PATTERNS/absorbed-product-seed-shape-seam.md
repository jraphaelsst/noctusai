...
**Recurrence threshold passed N=5 (across 4 distinct axes):** *write-payload-shape* (token_store) · *route-prefix* (oauth_router) · *security-control* (PKCE) · *projection-mismatch* (youtube ×1, drive ×1). Per the DRY recurrence rule (`KB § PATTERNS/project-execution.md § 2.7`), N≥3 ⇒ MUST formalize; this doc IS the formalization and is now broad enough that future agents triage by axis first.
