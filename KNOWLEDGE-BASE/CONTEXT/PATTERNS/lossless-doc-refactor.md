...
Author/refactor every doc per the caveman-aligned glossary by default — `KB § PATTERNS/doc-symbology.md` ([[feedback_symbol_first_authoring]]). Lossless-swap test gates each swap; prose expansion retained inline where a glossary-less reader would otherwise lose meaning. ¬ on error messages / first-paragraph / quoted-user / commits. New symbol needed ⇒ add to the glossary, never invent inline. **Enforcement teeth**: `check_doc_symbology_drift` (s4, live-parses the glossary) — platform baseline is **zero-drift**; a refactor that ends non-zero is not done. A swap that *adds* meaning fails the test ⇒ keep prose (calibrated zero-change is the rule working, ¬ a skipped deliverable).
...
→ companions: `KB § PATTERNS/doc-symbology.md` · `KB § PATTERNS/methodology-codification-pipeline.md` · `KB § 01-PHILOSOPHY.md § Context budget discipline`
