...
**Auditability is part of the LGPD posture too, not a separate concern (migration 135).** 092's original UNLINKED upload shape discarded the source PDF the moment transcription finished reading it — so a data-subject request, an office audit, or a simple "does this transcription actually say what the PDF says" question had no source to answer against, and the write-once trigger (111) correctly refused to let the transcription be fixed in place either. Retaining the source is itself an LGPD-relevant decision (a second copy of the same CPF-bearing PDF, in a new table `matricula_extracao_arquivos`), made deliberately symmetric with the already-logged linked shape: same bucket, same `superficie="imovel"` retention policy, same access-log table (`imovel_documento_acessos`, keyed by `extracao_id` — the exact optional column 111 added for "an extraction with no `imovel_documentos` row to log against"). **A re-transcription of a retained source SUPERSEDES, never rewrites** — a new row is inserted and the old one is marked `substituida_por` (the platform's existing collapse-pointer shape, `negociacoes_venda.substituida_por`, migration 054), so the write-once trigger's guarantee (a signed deed keeps resolving to the exact text it quoted) is extended to a two-more-column freeze, never fought. The 6 rows that predate this migration cannot be repaired — there is no source left to retain — so they are made LEGIBLE instead (`possui_marcacao_bruta`, a stored flag an operator's list view shows) and the contract generator (`card_hub.contrato_gerador.derivacao`) refuses to generate off text still carrying a raw `**`/`<u>` marker, independent of that flag.
