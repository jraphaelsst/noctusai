...
> `KB § PATTERNS/backend/llm-tool-audit.md` (the observation layer). Audit
...
Pydantic Input models (the platform pattern per `KB § PATTERNS/architect/mcp-tool-conventions.md § 2`)
...
- **Cost budget**: track `cost_estimate_cents` per conversation;
  ...
  LLM-usage tracking in `KB § PATTERNS/backend/llm-usage.md`.
...
- [ ] **Audit trail** wired (per `KB § PATTERNS/backend/llm-tool-audit.md`)
...
- **Observation layer:** `KB § PATTERNS/backend/llm-tool-audit.md`. The audit
  catches what slipped past these defenses; the defenses keep the
  audit table from filling with bad rows.
- **LGPD:** `KB § PATTERNS/security/lgpd.md` — overlaps with output sanitization
  (PII redaction in logs / messages / audit JSON).
- **LLM usage tracking:** `KB § PATTERNS/backend/llm-usage.md` — the cost
  budget rate limiter reads from this sink.
- **Webhook signature verification:** `KB § PATTERNS/security/webhook-signatures.md`
...
for `KB § PATTERNS/architect/seed-lib-layout.md § api`.
...
redactors → primitives layer per `KB § PATTERNS/architect/seed-lib-layout.md`).
...
- [ ] **Audit trail** (`KB § PATTERNS/backend/llm-tool-audit.md`) wired.
