...
- **Sibling of *defer ≠ resolve* (R5)** (`KB § PATTERNS/defer-is-not-resolve.md`) — both are *completeness ≠ deferral*. R1 is the absorption-docs instance; R5 is the general scope-shrink instance. Filing the consume-doc as a follow-up is exactly the de-scope R5 forbids.
...
**Memory:** `feedback_absorption_ships_consume_docs`. **CLAUDE.md:** §1 *Absorption ships consume-docs* pointer. **Companion:** `KB § GUIDES/absorb-seed-workspace.md` (the gate home), `KB § PATTERNS/defer-is-not-resolve.md` (R5 sibling), `KB § PATTERNS/methodology-codification-pipeline.md` (the s1→s4 model this doc is the s3 of).
