...
| Unrelated KB pattern docs (`PATTERNS/whatsapp-chatbot-seed.md` if not used here) | **Pointer integrity.** KB pages cross-reference each other (`PATTERNS/seed-fake-real-adapter.md` references `03-SEED-ARCHITECTURE.md`, etc.). Pruned pages leave dangling pointers that `noctus.dev.kb_sync` flags. |
