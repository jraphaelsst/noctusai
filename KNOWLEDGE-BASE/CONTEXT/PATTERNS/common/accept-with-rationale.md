...
- **Subject (historical):** the stale cross-worktree editable-finder eviction logic was formalized in `mcp/_kit/seed_pin.py` (`pin_in_tree_seed`, also called by `_kit.bootstrap.prepare_sys_path`; N=2 — META-2 + GOOG-2 hand-rolled it). The pre-formalization per-connector copies — `mcp/google_workspace/conftest.py::_pin_in_tree_noctusai_lib`, `mcp/google_workspace/server.py` inline meta_path loop, `mcp/meta/tests/test_smoke.py` inline `_is_noctus_editable_finder` — were initially **retained** under the prior `[A]` because removal was proposed at a project wrap-up under a known-unstable harness window (risk/reward then negative).
- **Resolution (triage rule: recurrence flips `[A]`→`[F]`):** the named-destination condition in the prior entry — "a `conftest`-delegates-to-`from _kit.seed_pin import pin_in_tree_seed` swap is safe ONLY with a green-suite re-verify per connector, in a healthy-harness session, as a pure dedup" — was met. Swap applied to all 3 sites (`mcp/google_workspace/conftest.py`, `mcp/google_workspace/server.py`, `mcp/meta/tests/test_smoke.py`); zero behaviour change (the helper is intent-equivalent + idempotent); `mcp/google_workspace/README.md` prose updated. Re-verify: `_kit` ∧ `meta` suites green (47 passed). `google/tests/` collection is pre-existingly red on the clean base (`ModuleNotFoundError: google.api_core` — `mcp/google_workspace/` shadows the site-packages `google.*` namespace package; unrelated to seed-pin, out-of-scope, surfaced to `gmail-seed-lift`); byte-identical before ∧ after this change ⇒ no regression.
