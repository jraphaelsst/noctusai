...
- **Conflict-zone merge ownership — LAST-FINISHER-MERGES.** When two branches share a collision zone, the *later finisher* owns reconciling them — not the orchestrator. On finishing, read the peer's pointer **status**: peer **done** → you merge their branch into yours, reconcile, deliver pre-merged (rationale in `notes`); peer **still `on_going`** → write a `MERGE-CONFLICT: merge feat/<you> …` signal into the **peer's** pointer `notes` + push — the peer catches it on finish (it MUST read its own latest pointer before delivering), resolves the merge, then delivers the combined work. Conflict zones thus reach the orchestrator already reconciled. Depth: [[branch-tree-tracking]] §3.
...
| B16 | dispatched a "filed / not-started" project that was already ~done | the project doc is derived + drifts vs the tree | ground project state against the tree (grep the success signal) before dispatch; doc-size ≠ work-size; the doc's OWN scope/churn estimate also drifts — re-size from the files (estimate-off-evidence), even to **confirm** a deliberate deferral (2nd dogfood: a filed "~8 test asserts" was ~12 + per-fixture reasoning) | §2 · [[project-execution]] |
| B17 | "branch them all" (N projects) but the methodology yields write-work on far fewer (even 0) | the instruction over-specifies the branch-count; only **WRITES** self-branch (reads / blocked-on-decision / user-deferred / N=1-wait → no-branch) | classify-first via §1 — the realized branch-count is set by **readiness**, not by the instruction; a 0-branch round is the methodology working, not a dodge. (2nd dogfood 2026-05-25: the 5 smallest-on-disk projects = 4 blocked/deferred/N=1 + 1 deferred-moot ⇒ 0 warranted branching; B14+B16 VALIDATED) | §1 · [[self-branching-mode]] |
...
<!-- NOC-REMEDIATE[codify]: worktree-sensitivity guard — flag working-tree-scanning tools (compliance/validate/scan_*/hound/kb_sync-counts/outline-corpus/graph.build) run on a busy shared checkout (peer-uncommitted files ⇒ phantom regressions); deferred design-first — predicate must distinguish "busy checkout" from a real regression without false-positives — 2026-05-25 -->
