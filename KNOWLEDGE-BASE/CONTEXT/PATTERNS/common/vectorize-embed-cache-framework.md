...
- **Transform via `noctusai_lib.integrations.llm.generate_embedding`** (`seed/lib/backend/noctusai_lib/integrations/llm/embeddings.py`). The seed's OpenAI-via-provider-registry pipeline owns key resolution + provider dispatch + cost capture.
