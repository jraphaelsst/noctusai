...
**🔴 2026-09-17 — `wire_env` now DEFAULTS TO TRUE on `task_branch action='start'`.** Four independent incidents hit the SAME false-red in one session — two frontend engineers reading `tsc --noEmit` noise as real drift, `predeploy_check` reading a missing local install as a blocked deploy, and the MCP toolkit's own pytest suite reading missing `ts-morph` as a detector regression — all because `node_modules/` is gitignored and a bare `task_branch action=start` never wired it in. A fresh worktree must come ready to run gates **by construction**, not by the caller remembering `wire_env=True`. Nothing below changes: pass `wire_env=False` to opt OUT (e.g. a doc-only slice where the symlink pass is pure overhead); a worktree forked some other way (a bare `git worktree add`) still needs the manual recipe or a follow-up `task_branch action=start` call. The `@noctusai/*` re-points are now **DERIVED** from each product's own `package.json` `file:` deps (`_derive_product_repoints` in `task_branch.py`), not a hand-maintained fleet-wide pair — see the module for the fallback when a `package.json` is absent/malformed. Every node-toolchain-dependent gate (`frontend_build` in `noctus.dev.predeploy_check`, the lying-loading-state Mode-B scan) also now self-diagnoses a missing `node_modules` via the shared `mcp/noctusai/node_env.node_deps_ready()` predicate BEFORE shelling into vite/node, so a worktree wired some OTHER way than `task_branch` (a bare `git worktree add`) still reads as "environment not provisioned" instead of a raw vendor error.

A fresh worktree is a clean git checkout — `node_modules/`, `.venv/`, and seed-version stamps are **gitignored, so they are ABSENT**. `noctus.dev.vite_build`/`noctus.dev.pytest` (MCP) run against the **primary** tree, not the worktree — so to verify *your worktree's* changes before integrate, wire the env in (all gitignored ⇒ never staged). The manual recipe below is the FALLBACK for a worktree that wasn't forked via `task_branch action='start'`:
...
**Known worktree-env caveats (codified breadcrumbs):** (a) the lib's *vitest render* tests dual-React-fail in a symlinked worktree — pre-existing, **not CI-gated** (lib CI gate is `tsc`); see [[reference_lib_frontend_vitest_render_harness_gap]]. (b) Seed-version-stamp critical false-positives in compliance scans (stamp gitignored/absent) — scope the scan or copy the stamp. (c) `noctus.dev.scan_wiring` may not be live as an MCP tool in a long-running session (no CLI flag yet); call the pure `analyze_*`/`scan_wiring()` functions directly. **Fallback when env-wiring is too fragile: author in the worktree, let the architect build-verify on the primary at integrate** (the documented verify-on-integrate path). **Automated, by default:** `noctus.dev.task_branch action=start` now auto-wires this recipe AFTER the worktree exists — no `wire_env=True` needed. It symlinks the PRIMARY tree's per-package `node_modules` in + re-points each product frontend's `@noctusai/*` deps (DERIVED from that product's own `package.json` — `_derive_product_repoints`, not a hand-maintained pair) at the WORKTREE's seed copies, reports `wired`/`skipped`, honors dry-run (reports `would_wire` without `confirm`), and is best-effort (missing primary `node_modules` — the reason NAMES the exact `npm install` to run — / a real `node_modules` already in the worktree are reported in `skipped`, never clobbered). All target paths are gitignored ⇒ never staged. Pass `wire_env=False` to opt out.
