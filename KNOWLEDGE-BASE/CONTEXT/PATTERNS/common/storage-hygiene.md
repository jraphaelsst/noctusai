...
⚠️ "Merged" per the above is necessary, not sufficient — a worktree forked seconds ago is trivially merged (0 commits ahead). The live-pointer + minimum-age guards below are the additional gate before "merged" becomes "safe to remove right now" (see the 2026-09-16 incident writeup).
...
Net: `force=True` bypasses dirty/stash/locked → still NO (unchanged); bypasses the pointer guard → still NO; bypasses the age guard → **YES** (documented, deliberate asymmetry).
