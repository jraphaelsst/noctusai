...
⚠️ "Merged" per the above is necessary, not sufficient — a worktree forked seconds ago is trivially merged (0 commits ahead). The live-pointer + recent-mtime + minimum-age guards below are the additional gate before "merged" becomes "safe to remove right now" (see the 2026-09-16/17 incident writeups).
...
**🔴 0-commits-ahead false positive + the removal guards (2026-09-16 incident).** `is_ancestor(branch, base)` — `git merge-base --is-ancestor` — is trivially TRUE the instant a worktree is forked: a fresh branch has ZERO commits ahead of `origin/dev`, so it IS its own ancestor by definition, before the engineer has committed anything. `community-m1-backend` / `community-m1-frontend` were ~6 minutes old, 0 commits ahead, two engineers actively working in them — and `noctus.dev.cleanup_stale_worktrees force=True` removed both, destroying the uncommitted work. "Merged" and "safe to remove right now" turned out to be two different questions.

Three guards close the gap, added to the shared `_worktree_staleness.py` helper (so both `cleanup_stale_worktrees` and `mole`'s worktree scope inherit them identically, checked AFTER the existing dirty/stash/lock gates — those already protect a genuinely dirty tree):

1. **Live branch-tree pointer guard (`pointer_blocks_removal`)** — a non-terminal (`on_going`/`blocked`/`deferred`/`integrated-worktree-live`) row for the branch in `project-history/branch-tree.ndjson` (KB § PATTERNS/architect/branch-tree-tracking.md) is an explicit peer claim. Resolved via `branch_pointer.query`'s own latest-per-branch logic (never re-parsed by hand). **NEVER bypassed by `force=True`** — force only ever meant "skip the confirmation ritual", never "override an explicit peer claim". Result buckets as `pointer_blocked` (`cleanup_stale_worktrees`) / category `POINTER_BLOCKED` (`mole`), each carrying the `status` + a `reason` string — a silent skip is as bad as a silent delete. 🔴 2026-09-17: an UNRESOLVABLE pointer (no row published for the branch, an unreadable ledger, or the query itself raising) is now ALSO treated as blocking (`status=None`) — absence of information is not permission to delete, the same conservative-refusal shape guard 2 already used for an unresolvable age.
2. **Minimum-age guard (`is_too_young`)** — age = `now - max(worktree dir creation time, last commit time on the branch)`; a worktree younger than `min_age_minutes` (default **60**, overridable per-call; documented range 30-120 — a full working session's grace without meaningfully delaying cleanup of the genuinely-abandoned, which are hours-to-days old) is refused. This is what would have caught the 2026-09-16 incident directly (~6 minutes old ≪ 60). `force=True` **MAY** bypass this one — an operator explicitly asking to force-sweep has made the age call themselves. Unresolvable age (dir already gone, branch unresolvable) is treated conservatively as too-young, never as a silent pass. Result buckets as `too_young` / category `TOO_YOUNG`, same reason-string contract as guard 1.
3. **Recent-mtime guard (`is_recently_active`, 2026-09-17 Leg 2)** — INDEPENDENT of the ledger entirely: a raw filesystem walk (no pointer read, no `git log` call) finds the most recent mtime among the worktree's files, pruning `.git`/`node_modules`/`__pycache__`/`dist`/`.pytest_cache`/`venv` (regenerable/vendored noise whose timestamps say nothing about human/agent activity). A worktree touched within `recent_mtime_minutes` (default **60**, deliberately paired with guard 2's default — an operator tuning one has an obvious analog for the other) is refused. **NEVER bypassed by `force=True`** — same rationale as guard 1: current activity outranks an operator's blanket flag, and this evidence survives even a missing, wrong, or never-published pointer. Result buckets as `recently_active` (`cleanup_stale_worktrees`) / category `RECENTLY_ACTIVE` (`mole`), same reason-string contract as the other guards. An unresolvable mtime (directory gone, or genuinely empty of readable files) is treated conservatively as recently-active, never as a silent pass.

Net: `force=True` bypasses dirty/stash/locked → still NO (unchanged); bypasses the pointer guard → still NO; bypasses the recent-mtime guard → still NO; bypasses the age guard → **YES** (documented, deliberate asymmetry — the ONLY guard force may override).

**🔴 The second-order hole (2026-09-17): the ledger is a CLAIM, the filesystem is EVIDENCE.** Guard 1 above trusts the ledger alone. `session_end_sweep`'s own auto-heal (`_autoheal_branch_pointers`, KB § PATTERNS/common/session-end-sweep.md) used to flip an integrated branch's pointer straight to the terminal `shipped` the instant `origin/dev` absorbed it — WITHOUT ever checking whether the branch's `.claude/worktrees/<slug>` directory (and the session using it) still existed. It flipped a peer session's live `ef-w8-models-worker` pointer to `shipped` mid-session; guard 1 then read a terminal status and waved a `force=True` sweep through. Two fixes close this, neither alone sufficient (full writeup: KB § PATTERNS/architect/branch-tree-tracking.md):
- The auto-heal now checks worktree-directory existence (via the same `_worktree_branches` parse the module already does) BEFORE flipping, and writes the distinct non-terminal `integrated-worktree-live` status instead of `shipped` when the directory is still there — guard 1 refuses it exactly like `on_going`.
- Guard 3 (recent-mtime) is the independent backstop: even if a pointer is wrong, stale, or was never published at all, a worktree touched moments ago is refused on filesystem evidence alone.

**🔴 The ledger-less-immortal-branch hole (2026-09-20): "unresolvable" is not the same as "unknowable".** Guard 1's 2026-09-17 fail-closed fix (unresolvable pointer ⇒ block) is correct in isolation, but it has a second-order effect nobody had measured: a branch that was simply **never published a pointer for at all** becomes IMMORTAL — permanently un-sweepable by `cleanup_stale_worktrees`, forever, regardless of `force`. Measured 2026-09-20: 37 stale worktrees on disk (6.4 GB), 36 of them carrying **zero** rows in `project-history/branch-tree.ndjson` (537 rows / 206 branches accumulated since 2026-09-13 — pointer publishing is real but not universal), every one of the 36 independently verified merged into `origin/dev` by hand (`git merge-base --is-ancestor <branch> dev`). Only branches that HAD been pointer-published were ever getting swept — the entire debt was the ledger-less set.

A second, POSITIVE liveness signal closes this — `wts.merged_into_base_confirms_dead(run, branch, base)`, consulted **ONLY** when `pointer_status_for_branch` resolves NOTHING at all (never when a live non-terminal pointer, or even a resolvable-but-terminal one, exists — those paths are unchanged). It requires BOTH: the branch is a true SHA ancestor of `base` (`is_ancestor`, not merely patch-id/cherry-pick equivalence — deliberately stricter than the `is_merged` gate checked earlier in the same function, since this signal has no ledger corroboration to lean on), AND the branch has genuinely diverged from `base` (`branch`'s tip SHA differs from `base`'s). The divergence half is what stops this from reintroducing the exact 2026-09-16 false positive above: a worktree freshly forked off the current tip has ZERO commits ahead, so `branch` and `base` are the literal same sha — trivially "ancestors" of each other, carrying zero evidence of deadness. A branch that diverged (did real work) and is NOW reachable from `base` by true ancestry, by contrast, is fully represented in `base`'s history already — there is no unique commit content a removal could lose.

Known, deliberate limitation: a branch whose landing IS `base`'s current tip (e.g. `task_branch action=integrate` fast-forward-PUSHES a branch's own commits onto dev — not a cherry-pick — so immediately after that FF, branch-tip == base-tip even though real work landed) is NOT confirmed by this signal in that narrow window; it stays `pointer_blocked` until either a pointer is published for it or `base` advances again (near-certain in an active repo — not a correctness bug, just a deferred sweep). Erring toward a missed sweep over a false "confirmed dead" is the correct trade for a signal that never force-bypasses guard 1's caller.

Every `stale` entry now carries its authorizing evidence in the `stale_signals` map (`ledger_pointer` | `merged_into_base` | `phantom_worktree_dir_gone` | `orphan_unregistered_dir`) — the verdict names its evidence, never a bare "removed". This is `cleanup_stale_worktrees`-only (not yet wired into `mole`'s worktree scope) — see `mcp/noctusai/tools/noctus/dev/_worktree_staleness.py::merged_into_base_confirms_dead`.
...
**🟢 2026-09-24 — the ledger moved to `origin/ledgers`, and merged-branch rows stopped.** The recovery ledger is now `worktree-salvage.ndjson` on the orphan `origin/ledgers` branch, appended through `_ledger_store` (git plumbing, never a dev commit; `KB § PATTERNS/common/ledger-store.md`). No caller commits it any more, so G3 below is historical. `_worktree_salvage.append_ledger` also **drops a record whose SHA is already on `origin/dev`** (an ancestor, or every commit patch-equivalent per `git cherry`). `origin/dev` itself recovers such a branch, so the row was noise. An unresolvable SHA is kept. In practice the automatic sweeps and `task_branch cleanup` (which removes only merged work) write no rows. `task_branch cleanup` no longer calls the recorder at all: that leg produced 373 `chore(salvage)` dev commits in seven weeks. `salvage_before_delete` (operator-invoked) still writes its row, because `delete_integrated_remote` requires it.

**🔴 2026-09-17 — the recovery ledger itself could land in the wrong tree (fifth instance of `KB § PATTERNS/common/claim-vs-evidence-shared-state.md`'s family).** `mole sweep` / `cleanup_stale_worktrees`'s `root` may legitimately BE a caller's own worktree (an explicit `worktree_path` — correct for scoping the worktree-list enumeration to that tree), but the salvage ledger must never land there: a worktree is exactly the ephemeral thing this mechanism protects against. `workspace.unwrap_worktree_root(root)` now corrects the `record_sweep(...)` call site ONLY when `root` is itself worktree-shaped (`.../<primary>/.claude/worktrees/<slug>`), passing any other explicit root through unchanged. Separately, `session_end_sweep`'s implicit default (neither `repo_root` nor `worktree_path` supplied) now resolves via `settings.LEDGER_ROOT` (`workspace.get_ledger_root()`) instead of `settings.REPO_ROOT` — the resolver `get_noctusai_home()` deliberately STOPS at a worktree boundary (correct for code-editing tools), so a ledger writer trusting that same value was inheriting the wrong guarantee. This ALSO resolves G3's "not an MCP-exposed seam" caveat below for the implicit-default case: the correct root is now the tool's OWN default, not a fresh-process-only workaround.

**🔴 The one path that still skips all three legs: a bare `git worktree remove` typed by hand** (NOT through a tool). The architect's post-merge cleanup MUST go through `task_branch cleanup` (or `mole sweep` / `cleanup_stale_worktrees`) — never a raw `git worktree remove` — or the learnings + recovery pointer are silently lost. This was the 2026-05-25 drift: an engineer worktree (`feat/sw-frontend-verify`) was removed by hand right after merge ⇒ no ledger entry, no learnings extracted until retroactively salvaged. The salvage ritual ORDER at teardown: **(1) extract learnings → KB/memory → (2) record recovery pointer → (3) mole worktree-sweep → (4) remove.** Sibling: `KB § PATTERNS/architect/project-execution.md` (learn-before-archive) · `01-PHILOSOPHY.md § Durable docs are self-contained` · `KB § PATTERNS/architect/branching-dispatch.md` (the architect's merge→cleanup step) · `KB § PATTERNS/common/self-branching-mode.md § cleanup`.
...
The cleanup belongs in the orchestrator playbook (operator side when split per `KB § PATTERNS/architect/two-session-architect-operator.md`; architect side in single-session mode). Cherry-pick that doesn't cleanup the source worktree is incomplete work.
...
- §19 of `KB § PATTERNS/architect/branching-and-merging.md` — worktree lifecycle methodology (worktree-scope predecessor)
...
- `KB § PATTERNS/architect/seed-absorption.md § noctus.hound.scan` — hound metaphor parent
