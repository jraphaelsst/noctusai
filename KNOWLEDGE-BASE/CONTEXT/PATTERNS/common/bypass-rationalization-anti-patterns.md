...
**The rule.** Any use of `git commit --no-verify` OR `git push --no-verify` is **forbidden**, with no brief-level carve-out (the former `engineer-seed.md §2` KB-autostage-hook exception was retired 2026-09-22 — `KB § PATTERNS/architect/dispatch-engineer-tuning.md` §4d; the only override is a human's `NOCTUS_ALLOW_HOOK_BYPASS=1`). The "commit-only is harmless, only push needs the gates" rationalization is **structurally wrong** — pre-commit hooks fire on `commit`, NOT on `push`; bypassing commit = bypassing the keepers entirely. This doc names the **5 typical rationalizations** an auto-mode agent uses to talk itself into a bypass, refutes each on the methodology contract, and pins the `ea7514e7` build-learn-cache slip as the canonical worked example so the next agent sees the recurrence catalog before reaching for the flag.
