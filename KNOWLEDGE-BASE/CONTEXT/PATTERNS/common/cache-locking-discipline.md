...
- **DON'T** inline `PRAGMA journal_mode=WAL` in a new cache's `_connect()`. Call `apply_locking_pragmas(conn)` — a raw WAL line without the busy_timeout sibling is the exact regression this discipline now guards against. The **`check_cache_uses_locking_helper` keeper (high severity)** blocks it: it flags any raw `journal_mode=WAL` under `mcp/noctusai/tools/**` outside the helper home (`cli.py --check-cache-uses-locking-helper`; zero violations after the 2026-05-30 centralization).
- **DON'T** open a raw `sqlite3.connect()` on a cache and forget the helper entirely. This is the SUBTLER failure — a connection that omits the pragmas (default `busy_timeout=0`, no WAL line) is invisible to a `journal_mode=WAL` grep, yet errors `database is locked` the instant a writer contends. A freshness-read helper (`get_source_sha()` / `get_bundle_sha()`) is the function MOST likely to run concurrently with the refresh writer, yet the one most easily written as a throwaway raw connect — that exact slip caused the 2026-05-30 code-embed lock. The keeper's **second leg (AST)** catches it: it walks every `FunctionDef` and flags (`symbol=cache-connect-without-locking-helper`) any function that contains a cache `sqlite3.connect()` but never calls `apply_locking_pragmas` — delegating to `_connect()` / `connect_cache()` is fine (those hold no raw connect). **Lesson: a keeper that asserts the presence of the right pragma must also assert the absence of the bare form, or it has a blind spot.**
...
2026-05-30 surfaced the writer-vs-writer gap WAL didn't cover: a pre-push embedding-refresh fan-out collided with a live MCP-server reader on the Tier-1-shared `kb-embeddings.sqlite` → `kb-embed: sqlite3.OperationalError: database is locked` (the refresh leg failed; the push itself succeeded, the cache self-healed on a manual retry). Fix-on-contact: extracted the single `apply_locking_pragmas` helper (WAL + busy_timeout), wired all 9 raw-WAL sites to it (retiring the N=9 duplication), + 5 regression tests, + the **`check_cache_uses_locking_helper` s4 keeper** (high) that blocks any future raw-WAL re-introduction. The pre-push refreshers still fan out concurrently; busy_timeout makes that safe.

2026-05-30 (same day, follow-up) surfaced the keeper's BLIND SPOT — and that the bug it was meant to prevent was still live. The code-embed `database is locked` recurred because `code_embeddings.get_source_sha()` opened a raw `sqlite3.connect()` with `busy_timeout=0`, bypassing the module's own `_connect()`. The s4 keeper missed it: it only grepped the `journal_mode=WAL` literal, so a connection that omitted the pragmas *entirely* was invisible. This was an **N=9 recurrence** of the same shape — the three `get_*_sha`/`get_bundle_sha` freshness readers (code/kb/agent-context), the corpus + memory refresh source_sha WRITERS, two `cache_deploy_mirror` reads, `component_list`, `code_baseline` — plus the two ledger DBs (`batch_speed_gains` / `phase_learnings`, dispatch-time concurrent writes) and the seven in-engine freshness reads in `compliance.py` itself. Fix-on-contact (`1a61db8d`): routed every site through the helper (readers → `_connect()`, writers → `connect_cache()`, cross-module → `apply_locking_pragmas`), and hardened the keeper with a **second AST leg** that flags a cache `sqlite3.connect()` whose enclosing function never calls `apply_locking_pragmas`. NOTE: this fix landed directly on `dev` (not a fix branch) due to a backgrounded `git commit` racing a `git checkout` during a harness-instability window — a process deviation surfaced honestly; the commit was fully gated (pre-commit green) and correct.
