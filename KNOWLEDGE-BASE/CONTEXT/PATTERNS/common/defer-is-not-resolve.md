...
- **Sibling of *absorption ships consume-docs* (R1)** (`KB § PATTERNS/common/absorption-ships-consume-docs.md`) — both are *completeness ≠ deferral*. R1: the consume-doc is in-scope for the absorbing project; R5: any in-scope work is in-scope for its project. Filing the R1 consume-doc as a follow-up is a textbook R5 violation.
...
- **Sharpens the recurrence/triage rule** (`KB § PATTERNS/architect/project-execution.md § 2.7`) — triage outcomes apply to *newly-surfaced* patterns; they do not license retroactively de-scoping the brief that surfaced them.
...
**Memory:** `feedback_defer_is_not_resolve`. **CLAUDE.md:** §1 *Defer ≠ resolve* pointer. **Companion:** `KB § PATTERNS/common/absorption-ships-consume-docs.md` (R1 sibling), `KB § PATTERNS/architect/project-execution.md § 2.7` (the triage rule this sharpens), `KB § PATTERNS/common/methodology-codification-pipeline.md` (why this stays s3).
