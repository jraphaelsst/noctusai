...
Supabase MCP per `KB § PATTERNS/backend/database-rls.md`:
...
questions (`KB § PATTERNS/security/lgpd.md`) or call `noctus.dev.lgpd_flag(...)`.
...
- **Not a confidence-threshold gate.** Low-confidence tool calls
  ...
  See `KB § PATTERNS/security/llm-bot-security.md § Confidence thresholds`.
...
Cross-reference: `KB § PATTERNS/security/llm-bot-security.md` for the defense
