...
The keeper detectors that watch over the platform follow the same convention: every detector ships colocated with a regression test. The detector `check_detector_has_regression_test` enforces this — a `check_*` function in `mcp/noctusai/tools/compliance.py` that has no matching `Test<CamelCase>` class in `mcp/noctusai/tests/` is itself a violation. See `KB § PATTERNS/compliance/testing.md § Regression-test-the-detector` for the pattern + worked examples.
