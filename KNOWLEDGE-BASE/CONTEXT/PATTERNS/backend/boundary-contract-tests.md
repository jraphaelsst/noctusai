...
> Companion to `KB § PATTERNS/compliance/testing.md`. Authored 2026-05-20 after a
...
|---|---|---|---|
| **B1** | **Build-injection** — env var → vite `define` → bundle literal | `infra.tsx` defaulted to `localhost:8000` (core's port) → every non-core product's bundle baked core's URL. `vite.config.factory.ts` `\|\| 8000` for unmapped ports — same shape. **SSO (2026-05-25):** `SSOCallback`'s `coreApiUrl` defaulted to a bare `localhost:8000`, but products bake only `VITE_CORE_URL` (not `VITE_CORE_API_URL`) → every product's `/sso` baked `localhost:8000` → "Failed to fetch" in prod (the browser at `social.noctusai.com` hit its own `localhost`). Cured at the seed: `app.tsx` resolves `coreApiUrl = VITE_CORE_API_URL ‖ VITE_CORE_URL ‖ dev-house-port` — **core is same-origin FE+API, so `VITE_CORE_URL` IS the API base**; every product bakes it (template Dockerfile `ARG VITE_CORE_URL` + `build-and-push.sh` + `build-and-push.yml`=`https://core.noctusai.com`) ⇒ SSO works for every product, new ones by construction. **Absorption (same day, N≥3 ⇒ formalize):** the root (hand-rolling `import.meta.env.VITE_CORE_* \|\| <literal>`) recurred in ~18 nav sites (`Login/Landing/AcceptInvite/LLMPreferences`, stale `localhost:5173` = dead pre-house port) + `erp/Configuracoes.tsx` (2nd core-XHR). The seed already shipped the canonical getters `env.CORE_URL`/`env.CORE_API_URL` — but **nobody consumed them**. **Rule: consume the getter, NEVER hand-roll the resolution.** Migrated seed `app.tsx`+`layout.tsx`+template pages (all-inherit) + pilots. **`getViteVar` dynamic access is prod-safe** — `createProductSupabase` reads `env.SUPABASE_*` via the same getter and Supabase auth works in prod (vite populates a runtime `import.meta.env` object dynamic bracket access resolves at build; `define` only rewrites `VITE_BACKEND_API_URL`+`VITE_PRODUCT_SCHEMA`). | Unit tests read the seed source via Node, not the built bundle. The `define`-injected literal is invisible until vite builds. SSO needs a real cross-origin browser fetch (prod hostname) — only an in-browser launch surfaces it. **The B1 "consume the getter" cure means a unit/build green is necessary but not sufficient: the getter must be the SOLE resolution site (grep for hand-rolled `VITE_CORE_*` defaults).** |
...
| **B4** Container env propagation | **Partial:** `prod_config_parity` (`noctus.dev.predeploy_check`, 2026-05-23) — pre-deploy audit of the prod env snapshot: every product resolves a non-localhost prod URL, no `PRODUCT_URL_*`/`CORS_ORIGINS` loopback value (the deploy-config-contract 3rd leg, `KB § PATTERNS/devops/deploy-config-contract.md § 5b`). Container-freshness still manual (`KB § PATTERNS/devops/containerization.md § 12b`); full `.env`↔compose↔stage-chain smoke is the remaining gap. | ⏳ Env-value subclass detected; full-chain `smoke-fleet-env-propagation` still destination when N=2. |
...
- `KB § PATTERNS/compliance/testing.md § Coverage gaps + ratchet plan` — boundary-
  contract tests now appear as a named row.
- `KB § PATTERNS/architect/seed-canonical-defaults.md` — the B1+B5 detector lives
  here in full detail.
- `KB § PATTERNS/common/methodology-codification-pipeline.md` — the s1→s4 path
