...
The base is **pure orchestration — no IO**. Per `KB § PATTERNS/backend/seed-fake-real-adapter.md`, the exemption test fires: a Fake here would exercise the same code as the Real. No Fake adapter needed.
...
- `KB § PATTERNS/backend/llm-usage.md` — token tracking + provider routing
- `KB § PATTERNS/common/accept-with-rationale.md § Per-product _render_bodies + _generate_narrative digest wrappers retained at N=4` — the wrappers are now methods of `XDigestService(BaseDigestService)` subclasses; the *boundary rationale* (per-product LLM prompts + Jinja context-dict keys) stands
