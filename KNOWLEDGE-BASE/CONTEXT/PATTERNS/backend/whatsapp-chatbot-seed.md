...
def processor(conversation_id: str, memory: list[dict]) -> None:
    ...
    reply = dispatcher.reply(
        ...
        # audit_writer=make_audit_writer(...),  # optional, see KB § PATTERNS/backend/llm-tool-audit.md
    )
    if reply:
...
- **Audit-row persistence.** `LLMDispatcher.reply(audit_writer=...)`
  ...
  `KB § PATTERNS/backend/llm-tool-audit.md`). Default is no-op so the
...
- `KB § PATTERNS/architect/seed-lib-layout.md` — placement rationale (`integrations/`
  for transports + adapters; `domain/chatbot/` for the framework).
- `KB § PATTERNS/security/webhook-signatures.md` — `verify_hmac_sha256_hex` is the
  helper the WhatsApp router uses.
- `KB § PATTERNS/backend/llm-tool-audit.md` — audit_writer wiring for the
  dispatcher.
- `KB § PATTERNS/security/llm-bot-security.md` — defense trio for the LLM
  surface (sanitize / arg-validate / rate-limit).
- `KB § PATTERNS/backend/scheduling-seed.md` — pairs naturally with the chatbot
