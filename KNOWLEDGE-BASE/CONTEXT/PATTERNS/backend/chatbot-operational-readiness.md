...
> Twilio, Slack, …). Pairs with `KB § PATTERNS/security/llm-bot-security.md`
> (defensive trio against prompt-injection / arg coercion / flooding)
> and `KB § PATTERNS/backend/llm-tool-audit.md` (the observation layer).
>
...
`KB § PATTERNS/architect/seed-lib-layout.md`).
...
`logging.basicConfig(...)`. The pattern at `KB § PATTERNS/backend/logging.md`
