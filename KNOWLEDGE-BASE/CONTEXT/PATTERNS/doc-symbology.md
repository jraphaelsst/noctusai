...
- **The dispatcher coordination file** (2026-05-18) — the two-session architect↔operator queue (Inbox+Outbox). Purest AI-to-AI surface (ephemeral, gitignored, never human-read). *(Relocation off repo-root + single-file consolidation is a filed follow-up — when it lands, this entry's path follows it.)*
