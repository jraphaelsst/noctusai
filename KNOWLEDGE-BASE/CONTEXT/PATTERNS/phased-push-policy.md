...
- The push is the literal last step of project close (see `KB § PATTERNS/project-execution.md § 2.10`) — committed ∧ green first, presented second, executed only on go.

## 6. Relationship to other rules

- **Cures R2** (`KB § PATTERNS/verify-seed-on-fork-base.md`) — keeping `origin/main` current removes the fork-base-mismatch class entirely.
- **Pairs with the worktree-base preamble** (`KB § PATTERNS/branching-and-merging.md § 16.7`) — R4 prevents the stale base; the preamble recovers from it when prevention has not yet propagated.
- **Amends *project-close push gate*** (`KB § PATTERNS/project-execution.md § 2.10`) — the project-close push is now per-increment + presented + user-gated, not a blanket close action.
...
**Memory:** `feedback_phased_push_policy`. **CLAUDE.md:** §1 *Phased-push policy* pointer. **Tooling:** `scripts/merge-debt-monitor.sh`. **Companion:** `KB § PATTERNS/verify-seed-on-fork-base.md` (the gap this cures), `KB § PATTERNS/project-execution.md § 2.10` (project-close gate), `KB § PATTERNS/branching-and-merging.md § 16.7`.
