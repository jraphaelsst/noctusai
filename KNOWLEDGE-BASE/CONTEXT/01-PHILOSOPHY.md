...
3. **Eight-way-sync the codification** in the same session (across all eight first-class methodology surfaces — `KB § PATTERNS/common/eight-way-sync.md`).
...
**Explanation-as-signal added 2026-05-21** (three-way-synced: this Standing-duty bullet + CLAUDE.md §1 `Always-hardening posture` clause + memory `feedback_explanation_as_signal.md` + MEMORY.md index line). User directive: *"[the CLI vs MCP explanation] sounds like we have a listener opportunity here as well. Let's use this mechanism to search for improvements as well, as the other improvements mechanisms we already have."*

**Surface-partition under concurrency added 2026-05-24** (three-way-synced: this clause + CLAUDE.md §1 `Always-hardening posture` surface-partition clause + memory `feedback_cross_cutting_improvements_serialize.md` + MEMORY.md index line). User surfaced the drift: *"we have a mechanism on our methodology that tells the running agent to implement improvements found in-flight, those could be hurting our methodology … can you evaluate?"* — correct: the improvement contract was amplifying shared-surface collisions; resolved by partitioning application (in-slice = running agent; cross-cutting = serialized through the integration owner).
