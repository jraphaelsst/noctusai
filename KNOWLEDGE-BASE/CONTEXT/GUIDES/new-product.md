...
**Now derived — no manual registration (verified 2026-09-14).** `propagate.py` `PRODUCTS` is `_load_products()`, parsed from the `start.sh` registry via `noctusai_lib.config.cors_registry.parse_products_registry`. `scaffold_product` writes that `start.sh` row, so a scaffolded product is inside the propagate set by construction. `_C_VITE` is a per-slug OVERRIDE map: an unlisted slug gets the seed build-arg block (`_C_VITE.get(slug, _C_VITE_SEED)`). Add a `_C_VITE` entry only when the product needs extra `VITE_*` args. Freshly scaffolded products DO read `stale` on the first `propagate target='both' check=True` (the scaffold leaves template placeholders), so run `propagate target='both'` once before the first commit. The rule and history below are kept for the *why*.

**Rule (historical, pre-derivation).** A new **frontend-bearing** product MUST be added to `propagate.py` `PRODUCTS` (+ `_C_VITE`) **at creation** — same slug+port as its `start.sh` block (item 5). Miss it and the product is silently outside propagation: its compose/Dockerfile never regenerate from seed, so every future seed-level container fix (a base-image change, a node-source swap, a mount, …) **skips that product** and it drifts. The fix never reaches it and nobody notices until a build breaks. This is precisely how `knowledge-extractor` ended up outside the 8-product set — and why the 2026-05-23 Debian-node Dockerfile fix would have missed any frontend product left unregistered.
...
- [ ] RLS policies on every table (see `../PATTERNS/backend/database-rls.md`).
...
- [ ] SSO works from Core. **Works by construction — do NOT hand-wire it.** The seed SSO callback (`seed/framework/frontend/src/app.tsx`) + the template `Login`/`Landing`/`AcceptInvite` pages already resolve core's URL through the canonical getter `env.CORE_URL` / `env.CORE_API_URL` (`@noctusai/lib`). **MANDATORY:** any product→core link you write (SSO XHR, "back to dashboard" nav) reads the getter — **NEVER** hand-roll `import.meta.env.VITE_CORE_* || "<literal>"` (the SSO "Failed to fetch" / stale-`5173`-nav recurrence; the `check_handrolled_core_url` keeper blocks it). The Dockerfile bakes `VITE_CORE_URL` ⇒ prod resolves the real core. Full system: `../PATTERNS/frontend/core-url-routing.md`.
- [ ] Frontend FRAMEWORK_DEP parity (the `[vite] Rollup failed to resolve "<dep>"` class). **Works by construction — no manual audit needed at scaffold time.** `scaffold_product` calls `ensure_framework_deps_for_product` before returning (see `framework_deps_ensured` in the scaffold result); it borrows a real version range from the seed's own `package.json`(s) first, never writes `"*"`. The standalone `noctus.dev.check_framework_deps` / `predeploy_check` audit remains the fleet-wide backstop for drift introduced LATER (a seed organ growing a new peer dep after this product was scaffolded). See `../PATTERNS/devops/containerization.md § 8 Troubleshooting`.
...
- [ ] `tests/conftest.py` calls `bind_consent_module_to_mock(mock_sb)` inside the `client` fixture (default in `templates/product-seed/` since 2026-04-27 — verify it survived your scaffold). Required even if your product doesn't register consent features today; idempotent if catalog is empty. See `../PATTERNS/compliance/testing.md § Consent-guard product conftest pattern` for the full rationale.
