...
The generalized loop (a foreign platform, vs a sibling workspace): **take a non-noc structure → analyze & diagnose deeply → learn & neutrally compare → salvage the valuable ideas behind the bad code, fixing the execution → re-confirm the value → bring the capability in-home as a seed organ → propagate to pilots to validate the procedure → propagate to the whole fleet (the absorbed product included, as the last consumer).** The standard 10 gates then carry the scaffold→reconcile→port→teardown→container legs; these refinements front-load a learning phase and re-aim the reconcile at the seed.

> Reference instance: the orbity diagnosis doc set (faithful procedures, neutral 16-capability comparison, bad-code salvage evaluation, seed-first uplift strategy) — staged as the transient source-bridge under `KB § ABSORPTIONS/orbity/` (see Refinement 5).

### Refinement 5 · Absorbed knowledge becomes NATIVE — de-reference the source

Once a capability is **fully absorbed** (built as a seed organ + documented as a native noc pattern/organ/guide), its knowledge is **noc's own** — documented natively, **de-referenced**, with **zero external-source framing**. We learned *from* the source; we are not wired to it, dependent on it, or inheriting it. *It was the source of the knowledge, but once it's ours, it's ours.*

- The foreign-source diagnostic set (`KB § ABSORPTIONS/<source>/`, full of "<Source> does X" + `<source-repo>/…` pointers) is the **TRANSIENT bridge** for the rebuild — it must **NOT** become permanent external pollution in noc's durable docs. As each capability lands natively, its durable value moves into the canonical noc doc **without any source framing** ("lesson from <source>" / external paths are dropped), the bridge is superseded, and at source-archival (Gate 8) the bridge is **removed entirely** — its value already de-referenced into native docs.
- **Native noc docs never carry external-source framing.** A pattern documenting (say) idempotent month-end financial closure is a noc finance pattern — not "what <Source> did." Provenance is at most a one-line git-history footnote, never the framing.
- **The canonical structure is the validated improving GROUND — it evolves by absorbing valuable learnings FROM new structures.** The old (noc seed / KB / patterns / organs) is canonical *because it is validated*, not because it is frozen. A new (foreign) structure is a **source of improvements to that ground**: its valuable learnings *improve* the canonical structure. The knowledge is **neither bent to fit the old shape, nor does it replace the old** — the canonical ground **grows to incorporate the learning, on its validated seam**. (Mechanically, the platform already auto-recognizes a new KB subtree — the general KB walker emits every `KNOWLEDGE-BASE/**/*.md` as a graph node, queryable via `noctus.graph.explain` + `kb_search` after `--refresh-kb-embeddings` — but that is just plumbing; the *deeper* rule is that the validated canonical structure itself improves from new learnings, and no permanent node-kind is enshrined for transient external material.)
