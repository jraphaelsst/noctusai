...
**② GHCR-pull — the designed CI target (not yet active; activate by pushing).** A build host / CI runs `build-and-push.sh` *with* push (needs `GHCR_USERNAME` + a `write:packages` `GHCR_TOKEN`), tagging + pushing `ghcr.io/jraphaelsst/noctus-<slug>:${NOCTUS_IMAGE_TAG:-latest}`; the VPS then `docker compose -f deploy/fleet/docker-compose.prod.yml pull && … up -d`. Once images actually live on GHCR, C2 uses `--source pull` (the default) — identical snapshot/health/rollback, just `compose pull` instead of a local build. Pin `NOCTUS_IMAGE_TAG=<git-sha>` for an immutable, rollback-able deploy. **Pros/cons of activating it: see the migration's PROJECT.md / `[[reference_production_deploy_runbook]]`.** Full runbook: `deploy/fleet/README.md`.
