...
**② GHCR-pull — the CI delivery layer (`.github/workflows/build-and-push.yml`; LIVE).** The workflow runs on push to `main` **or `prod`** (path-filtered to `products/**`/`seed/**`/the build script) + `workflow_dispatch` (with a `products` input for a fast targeted/test build); it runs `build-and-push.sh` and always pushes the immutable `ghcr.io/jraphaelsst/noctus-<slug>:<sha>`. GHCR auth = the built-in `GITHUB_TOKEN` (no PAT). **Repo secrets:** `VITE_SUPABASE_URL` + `VITE_SUPABASE_PUBLISHABLE_KEY` (baked into the FE bundle — the step is **fail-closed** if absent, so it never pushes a broken-FE image; set 2026-05-22). Packages are **public** (repo is public) → the VPS pulls anonymously, **no token**. Deploy with `noctus.dev.deploy_image <product> --source pull`. **This is the DELIVER layer; C2 (`deploy_image`) is the deploy/rollback-safety layer — they compose, neither supersedes the other.**
> 🔒 **PROD-PIN fix (2026-07-20) — `:latest` no longer moves on a bare `main` push.** `docker-compose.prod.yml` pins every product to `${NOCTUS_IMAGE_TAG:-latest}`, so whichever build last moved `:latest` is what the fleet actually runs. The workflow now moves `:latest` **ONLY on a `prod`-ref build** (the push that `noctus.dev.release stage='promote'` FF's, or an explicit `workflow_dispatch move_latest=true` override) — a routine `main` push tags the non-fleet-facing **`:edge`** instead (pullable for manual testing, never what a routine `docker compose pull` picks up). Backstop: `noctus.dev.deploy_image` REFUSES a `tag=latest source=pull` deploy whose baked `org.opencontainers.image.revision` isn't a verified ancestor of `origin/prod` (fail-closed on a missing/unresolvable label too) — see §2a C2 below. Building on `main` still does NOT auto-*roll out* to prod (`deploy_image`/`docker compose pull` is still a deliberate, separate action) — and now the `prod` promote-gate governs the VPS **code checkout** ∧ (once the promoted build lands) **which image `:latest` resolves to**, closing the earlier §2b gap where the two had drifted apart. The immutable `:<sha>` tags also give durable rollback/DR (pull an old sha) regardless of which build moved `:latest`. Pros/cons (minutes/storage; for one VPS build-on-VPS is cheaper): `[[reference_production_deploy_runbook]]`. Full runbook: `deploy/fleet/README.md`.
...
- **P6 · catalog-scope guard** ✅ *(2026-09-17 — `noctus.dev.deploy_image` + `noctus.dev.migrate_product`)* — the incident: a real prod deploy ran `deploy_image product='erp-imobiliario' confirm=True` and then `migrate_product` wrote to `erp` — `erp-imobiliario` was `ativo=false, deploy_scope='dev'` in the catalog, and `deploy_image` reported `status='deployed'`, healthy, swap-verified anyway, because none of its OTHER gates ask "should this be touched at all?", only "did the touch land cleanly?". The mistake surfaced only afterwards, via `deploy_verify`'s `skipped_inactive` classification — a rule `deploy_verify`/`predeploy_check` already honoured that the deploy/migrate ACTIONS did not. Both tools now REFUSE — `status='refused_catalog_scope'`, `exit_code=1`, nothing touched — before doing anything else, unless the product is `ativo=true AND deploy_scope='live'` (or `core`), reusing `deploy_verify`'s own catalog resolution (`mcp/noctusai/tools/noctus/dev/_catalog_scope_guard.py`) rather than a second hand-rolled read. Fail-closed on an unreadable catalog (`in_scope=False`, never a guess). Escape hatch: `allow_inactive=True` (a deliberate, supervised reactivation only) — the `catalog_scope` verdict rides on the result either way, never silently. See `KB § PATTERNS/architect/product-working-scope.md` § 4b.
...
- **C2 · atomic image rollback** ✅ *(2026-05-22 — `noctus.dev.deploy_image`; live-validated on social-wiring incl. a real rollback)* — a product-image redeploy is atomic + **fail-safe**: `docker commit` the running container → `:previous` (a reliable rollback target — tagging the container's image id is NOT reliable on the containerd manifest-list store) → **VERIFY the snapshot resolves, else REFUSE to deploy** (no confirmed rollback target ⇒ container untouched) → acquire the new image (`--source local` for the live build-on-VPS model §2 ①, or `--source pull` once GHCR is active §2 ②) → **PROD-PIN ancestry guard** (2026-07-20, `tag=latest source=pull` only) → `up -d --force-recreate` → **active** `/api/health` probe (port from the container healthcheck; fast detection + startup grace) → on failure, retag `:previous` → `:<tag>` + `up -d --force-recreate` + re-probe + **VERIFY restored** (loud `rollback_failed` + a manual-recovery command if not — never a false `rolled_back`). **The ancestry guard** reads the pulled image's `org.opencontainers.image.revision` OCI label (baked by `build-and-push.sh`) and REFUSES to recreate the container — fail-closed, container untouched — unless that commit is a verified ancestor of `origin/prod` on the VPS checkout; a missing label or an unresolvable `origin/prod` also refuses (unverifiable ≠ safe). This is the deploy-time backstop for the PROD-PIN HOLE (§2 ② above): even if `:latest` somehow moved outside a prod-ref build, a pull-deploy of it is refused unless promoted. Pass `skip_ancestry_check=True` to override deliberately (a pinned `tag=<sha>` deploy and `source='local'` never need it — see the tool docstring). Run `noctus.dev.deploy_image <product>` (CLI `--deploy-image [--deploy-image-source local] [--deploy-image-confirm]`); DRY-RUN by default. By construction it only uses a safe docker allowlist (inspect/image/tag/ps/exec/commit + compose pull/up) + a safe read-only git allowlist (fetch/rev-parse/merge-base, for the ancestry guard only) — never rmi/prune/down/reset/checkout/push (a colocated test asserts it). `deploy_pull`'s rebuild-decision points here when a pull touches product runtime. **Hardened by a live failure 2026-05-22** (an earlier version falsely reported `rolled_back` while prod stayed broken → a real social-wiring outage; the snapshot-verify + rollback-verify + commit-snapshot fixes were proven by re-validation).
...
> **⚠️ CORRECTION (2026-07-20) — the `prod` gate governs the VPS *code checkout*, NOT the images the fleet actually executes.** An earlier version of this doc claimed "code becomes production *exclusively* by a deliberate human FF of `prod`" — that is true for the *checked-out files* (`docker-compose.prod.yml`, `deploy/tunnel/ingress.yml`, scripts, `.env` structure) but **NOT** for the product **images**. Every `main` push builds + pushes `ghcr.io/.../noctus-<slug>:latest` (§2 ②) — independent of `prod`. `docker-compose.prod.yml` pins every service to `${NOCTUS_IMAGE_TAG:-latest}`, so **any** `docker compose pull` + recreate on the VPS (a routine restart, `deploy_image --source pull`, a manual `up -d`) picks up whatever is currently tagged `:latest` — the CONTENTS of `main`, not necessarily anything that was ever promoted to `prod`. The `prod` promote-gate is real and still worth having (it FF-gates the deploy-config surfaces + gives a deliberate go/no-go moment), but it is **NOT** "the only ref that governs what code runs in production," and it is **NOT** a review point for whether a *product should be publicly exposed at all* — that decision is governed by an entirely separate mechanism: `check_prod_exposure_consent` / `KB § PATTERNS/devops/prod-exposure-consent.md`, gated on a product's first arrival on `deploy/fleet/docker-compose.prod.yml` / `deploy/tunnel/ingress.yml` / `ALL_SLUGS` in `scripts/infra/build-and-push.sh` — because THAT edit, not the prod-branch FF, is what makes a product reachable at `<slug>.noctusai.com` running live code with no later gate reviewing the decision again.

> ✅ **FIX (2026-07-20, same day) — the PROD-PIN HOLE this correction describes is now closed by construction.** `.github/workflows/build-and-push.yml` no longer moves `:latest` on a bare `main` push — it moves `:latest` ONLY on a `prod`-ref build (a routine `main` push now tags the non-fleet-facing `:edge`, §2 ②), and `noctus.dev.deploy_image` additionally REFUSES a `tag=latest source=pull` deploy whose baked commit isn't a verified ancestor of `origin/prod` (§2a C2). So the `prod` promote-gate is **once again load-bearing for the images**, not just the checkout — a `docker compose pull` on the VPS can no longer silently promote un-promoted `main` content, and even a manual `--move-latest` override on a non-prod build would still be refused at deploy time by the ancestry guard. **What is still true, unchanged, and NOT what this fix addresses:** the `prod`-branch FF is still NOT a per-product exposure review — `check_prod_exposure_consent` / `KB § PATTERNS/devops/prod-exposure-consent.md` remains the separate, only mechanism that gates "should this product be public at all."
...
> ✅ **COMPLETE for `noctusai.com` 2026-05-25.** All 8 live hosts (apex, core, erp, social, seed, n8n, waha, legacy) are proxied CNAME → tunnel and verified 200 end-to-end; `noctus-caddy` (container + `caddy_data`/`caddy_config` volumes + deploy-local `caddy/` dir) was retired and VPS ports 80/443 freed. The runbook below stays as the repeatable A→B procedure for future domains.
...
**After ALL hosts are on the tunnel + verified** (✅ done 2026-05-25): `noctus-caddy` was stopped + removed along with its `caddy_data`/`caddy_config` volumes (LE certs no longer needed — CF terminates TLS) and the deploy-local `caddy/` dir; VPS ports 80/443 are now unbound. Caddy is fully retired — the dead `caddy/Caddyfile` + `caddy/compose.caddy.yml` artifacts were removed from the archived migration tree too (do NOT relocate `caddy/`; it's gone). **Still pending: relocate the *live* deploy configs** (`tunnel/config.yml`, `docker-compose.prod.yml`) out of `projects/<deploy-project>/` to a durable home — the config went MISSING under `projects/` (container ran a stale inode), the exact `[[durable-config-not-anchored-to-projects]]` fragility (tracked: `projects/prod-deploy-compose-durable-relocate`). The tunnel is a single egress dependency (watch `cloudflared` logs for `:7844` flap).
...
| **🔴 `DROP SCHEMA` on an API-exposed Supabase schema = fleet REST outage** | PostgREST caches a schema built by introspecting **every schema in its exposed-schemas config** (dashboard → Settings → API → Exposed schemas). Drop one that's still in that list and the next schema-cache rebuild (any DDL fires `NOTIFY pgrst,'reload schema'`) errors `schema "<x>" does not exist` → the cache can't build → **ALL** REST endpoints serve `PGRST002 / 503`, fleet-wide, stuck retrying ~every 32s. **Heuristic: if the `rls_disabled_in_public` advisor ever flagged a schema's tables, that schema is EXPOSED** ⇒ unexpose it FIRST, reload, THEN drop. **The exposed list lives in the `authenticator` role's `pgrst.db_schemas` setting** (= what the dashboard Settings→API→Exposed-schemas reads/writes, so a SQL change ≡ the dashboard change). Read it: `SELECT setconfig FROM pg_db_role_setting s JOIN pg_roles r ON r.oid=s.setrole WHERE r.rolname='authenticator'`. **Safe drop order (verified clean 2026-05-31):** `ALTER ROLE authenticator SET pgrst.db_schemas='…(minus the schema)'` → `NOTIFY pgrst,'reload config'` → verify REST 200 + the schema now returns 406 (unexposed) → `DROP SCHEMA … CASCADE` → `NOTIFY pgrst,'reload schema'` → verify REST stays 200. **Blast radius hides at the container level** — only a product that queries Supabase REST *at boot* (e.g. erp-imobiliario's `recover_stuck_processando` lifespan startup, no retry → crash-loops) surfaces it; others stay healthcheck-green while their user traffic 503s. **Diagnose:** `get_logs service=postgres` shows `ERROR: schema "<x>" does not exist` from `application_name=postgrest`; curl `/rest/v1/<public-table>` with the anon key reproduces 503 fleet-wide. **Recover (instant):** `CREATE SCHEMA IF NOT EXISTS <x>; NOTIFY pgrst,'reload schema';` → cache builds → REST + boot-querying products recover in seconds. **C2 image-rollback CANNOT save this** — new + old images fail identically on the external PostgREST dep. Bit 2026-05-31 (dropped the orphan `automation_workflow` schema during a deploy). |
| **🔴 a NEW product's schema was never ADDED to `pgrst.db_schemas` = PGRST106 with `/api/health` staying 200** | The reciprocal of the row above: a brand-new product's schema simply never made it into the `authenticator` role's exposed list in the first place (nobody ran the `ALTER ROLE` step during scaffolding). Its own startup-hook PostgREST probe fails `PGRST106 "schema … does not exist" ` while the plain `/api/health` liveness check — which never touches PostgREST — stays 200, so every gate that shells `curl /api/health` reports green. A product with no boot-time REST probe 5xx's silently on its first real request instead. **Mechanism:** `noctus.dev.ensure_schema_exposure` (`action='check'`, read-only — reports every catalog-live product whose declared schema is unexposed; `action='apply' confirm=True` runs the SAME `ALTER ROLE` + `NOTIFY` sequence as the row above, but APPEND-ONLY — union with the current list, never a removal). **Gate:** the `schema_exposure` leg in `noctus.dev.predeploy_check` — UNLIKE every other leg in that tool, a `not_configured`/`unavailable` result here is a FAILURE, not a skip, because "couldn't verify" must never read as "it's fine" for the one check that exists to catch this class before a product's first deploy. Bit 2026-09-16 (agents/academia-de-reciclagem cutover). |
| **🔴 a secret `${VAR}` compose-INTERPOLATES from `deploy/fleet/.env.fleet`, not the root `.env` — an empty default resolves silently** | `docker-compose.prod.yml` mapping `ANTHROPIC_API_KEY: ${JULIA_ANTHROPIC_API_KEY:-}` for `agents` looks like ordinary runtime config, but `${VAR}` is compose-PARSE-time interpolation from a file literally named `.env` in the compose project directory (`deploy/fleet/` — the `.env.fleet` symlink, § "🔴 `deploy/fleet/.env`" above), a DIFFERENT mechanism from `env_file: ../../.env` (which feeds the already-started container from the ROOT `.env`) — and `environment:` wins over `env_file:` regardless. `JULIA_ANTHROPIC_API_KEY` was documented only as a root-`.env` key, so it silently interpolated to `''`; every Julia turn failed at the CLI-spawn step while the container stayed healthy. **Mechanism:** `deploy/fleet/env.fleet.keys` — a tracked, names-only manifest of every `${VAR}` either prod compose file interpolates. **Gate:** `check_prod_compose_env_manifest_sync` (pre-commit, staged-file-triggered) cross-checks the manifest against both compose files AND flags any secret-shaped name (`*_KEY`/`*_TOKEN`/`*_SECRET`/`*_PASSWORD`) with no enforced non-empty default (escape hatch: an `env-fleet-empty-ok` comment for a REVIEWED, intentional degrade — e.g. `dev-team`'s `ANTHROPIC_API_KEY`, which runs healthy-but-switch-not-flipped when unset). The one thing no repo-only check can see — whether the VPS's actual `.env.fleet` carries a VALUE — is the optional `env_fleet_manifest` leg in `noctus.dev.predeploy_check`, fed a snapshot via `env_fleet_path` / `NOCTUS_ENV_FLEET_FILE`. Bit 2026-09-16 (same cutover as the row above). |

## 7 · Artifacts — the durable home (graduated 2026-05-25)

The prod deploy composes **live at the repo-root `deploy/` tree** — the durable home, NOT under a `projects/` slug (a project path vanishes on archive and breaks every tool/CI that points at it — the `[[durable-config-not-anchored-to-projects]]` class; it bit live prod once, see `projects/prod-deploy-compose-durable-relocate`). Layout:
- `deploy/fleet/docker-compose.prod.yml` — the 8-product prod fleet (image-pull, slim runtime; `env_file: ../../.env`). **This is `DEFAULT_COMPOSE`** for `noctus.dev.deploy_image` + `noctus.vps.*`.
- `deploy/fleet/compose.infra.prod.yml` — redis (+ waha profile).
- `deploy/services/compose.services.yml` — n8n/waha/postgres (volume-preserving PaaS decommission, §5).
- `deploy/tunnel/` — the cloudflared named-tunnel `compose.tunnel.yml` + `config.yml.template` + `ingress.yml` (the live rendered `config.yml` is deploy-local/gitignored on the VPS).
- Build/push orchestration is `scripts/infra/build-and-push.sh` (NOT under `deploy/` — it's a `[carve:docker]` script).

> The standalone `legacy/` (one-permutas) deploy stays in the archived migration tree for now — its build context couples to an archived `reference/one-permutas` source, so relocating it cleanly is a separate effort (tracked in `projects/prod-deploy-compose-durable-relocate`). It's not referenced by any tool.
