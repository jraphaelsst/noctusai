...
| 2026-08-18 | **Gate 0.2 and 0.3 SETTLED, from the generated spec.** `/v1/configuracao/callbacks` is present (get, put) and `/v1/configuracion/callbacks` is absent; `/v1/callbacks/geracao/eventos` is present (post) and `/v1/callbacks/generacion/evento` is absent. The Spanish spellings in the Notion prose are documentation artefacts. Kept, not deleted — the spec is generated from the running BR code, so absence is strong evidence, not proof of no alias. | `imovelweb.diagnostics.fetch_swagger`, both hosts |
| 2026-08-18 | **Every endpoint we had written down exists**: 15/15 baseline rows confirmed against the spec, `in_baseline_not_in_spec` empty. The diff also found two endpoints the adapter CALLS that the baseline had not listed (`/v1/imobiliarias/{cod}/mensagens`, `/v1/imobiliarias/{cod}/anuncios/{cod}/mensagens`) — now added. | same run |
| 2026-08-18 | **The vendor's edge 403s `Python-urllib/*` by name.** The public spec answered 200 to curl and 403 to urllib from the same machine seconds apart, so the connector now sends an identifying User-Agent and reports a 401/403 on this endpoint as a client rejection rather than "not served". Without it the tool blamed our network for a WAF decision. | curl vs urllib, same URL |
| 2026-08-18 | **40 spec endpoints are not in our baseline, and almost all are the listing-publication surface** (`anuncios`, `lancamentos`, `multimidia`, `tipopropriedade`, `locais`, `imobiliarias/{}/usuarios`). Not an omission — that surface overlaps `products/erp-imobiliario`'s outbound XML feed, and reaching it needs Read-and-Write credentials. It is scope to decide on, not scope we missed. | same run |
