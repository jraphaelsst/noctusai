-- Fix trigger_rollup_on_diaria to calculate proper meta_pretendida for aggregated metas
-- based on metas_config when creating them
...
BEGIN
  ...
  LOOP
    ...
    IF v_agg_id IS NULL THEN
      -- Calculate initial meta_pretendida based on config and type
      IF v_tipo_agg = 'semanal' THEN
        -- Weekly: monthly / ~4 weeks
        v_semanas_mes := semanas_no_mes(v_data_ref);
        v_initial_pretendida := CEIL(v_config_mensal::numeric / v_semanas_mes);
      ELSIF v_tipo_agg = 'mensal' THEN
        -- Monthly: use config directly
        v_initial_pretendida := v_config_mensal;
      ELSIF v_tipo_agg = 'anual' THEN
        -- Annual: monthly * 12
        v_initial_pretendida := v_config_mensal * 12;
      END IF;
