 ...
 *
 * MOVED from `products/social-wiring/frontend/src/components/card/TooltipIconButton.tsx` into the
    ...
    return (
      <TooltipCaption label={label} side={side}>
        <Button
          ...
          aria-label={label}
          data-testid={testId}
          // `max-sm:` = the mobile-first 40px touch floor (R0). A caller's
          // `h-7 w-7` still sizes it at desktop; `min-*` wins below `sm`.
          className={cn("h-8 w-8 shrink-0 max-sm:min-h-10 max-sm:min-w-10", className)}
          {...rest}
        >
