...
Uses FakeWahaClient (no httpx) and asyncio.run (not get_event_loop). Ported
from `products/social-wiring/backend/tests/services/test_whatsapp_identity.py`
as part of the 2026-09-17 seed promotion (`community` module 3 seed slice) —
social-wiring's own copy is now a re-export shim and keeps its original test
file (unchanged import path, still green against the shim).
"""
