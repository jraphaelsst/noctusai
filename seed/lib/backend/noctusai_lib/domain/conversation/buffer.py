...
(Phase 4 — `domain/conversation/worker.py`) drains via
`due_conversations` + `mark_conversation_claimed`.

Documented debounce race: `due_conversations` returns IDs whose due
timestamp has passed; the worker reads + claims (`zrem`) but a new
inbound between read + claim could push the due timestamp forward —
the worker may then dispatch on slightly stale memory. Sibling
accepted this; we preserve the behavior + leave a follow-up project
slot open (see project §7 Q5).
"""
...
class ConversationBufferService:
    ...
    def due_conversations(self, now: float | None = None, limit: int = 25) -> list[str]:
        """IDs whose debounce timer has fired (score ≤ now). Worker
        consumes + claims via `mark_conversation_claimed`."""
