-- ============================================================================
-- 007_anon_grant_lockdown.sql — AdConnect (adconnect)
--
-- Closes the schema-wide `anon` exposure inherited from `001_adconnect.sql`
...
--   ALTER DEFAULT PRIVILEGES IN SCHEMA adconnect GRANT ALL ON TABLES TO anon, ...;
--
...
-- `products/adconnect/backend/migrations/*.sql` for `TO anon`,
...
-- flags, no PII. Re-granted explicitly below. Every other RLS policy in
-- this schema is `TO authenticated` or `TO service_role`, so REVOKE-only
...
REVOKE ALL ON ALL TABLES IN SCHEMA adconnect FROM anon;
ALTER DEFAULT PRIVILEGES IN SCHEMA adconnect REVOKE ALL ON TABLES FROM anon;

-- `todos_veem_producao` (001_adconnect.sql) is a deliberate PUBLIC-role
...
GRANT SELECT ON adconnect.status_pagina TO anon;
