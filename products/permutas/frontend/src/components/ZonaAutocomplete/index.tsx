...
type Props = {
  value: string
  onChange: (id: string, zona?: Zona) => void
  onAddClick?: () => void
  required?: boolean
  showAddButton?: boolean
}

const ZonaAutocomplete = ({ value, onChange, onAddClick, required, showAddButton = true }: Props) => {
  const api = useAxios()
  const [query, setQuery] = useState('')
  const [suggestions, setSuggestions] = useState<Zona[]>([])
  ...
  useEffect(() => {
    if (value && !selectedZona) {
      api.get(`/zona/${value}/`)
        .then(res => {
          setSelectedZona(res.data)
        })
        .catch(() => {})
    }
  }, [value])
  ...
  useEffect(() => {
    const searchTimeout = setTimeout(async () => {
      ...
      try {
        ...
        const res = await api.get(`/zona/${searchParam}`)
        const data = res.data
  ...
  if (selectedZona) {
    return (
      <Container ref={containerRef}>
        <SelectedBadge>
          <SelectedText>{selectedZona.nome}</SelectedText>
          <ClearButton type="button" onClick={handleClear}>×</ClearButton>
        </SelectedBadge>
        {showAddButton && onAddClick && (
          <AddButton type="button" onClick={onAddClick} title="Adicionar nova zona">+</AddButton>
        )}
        ...
        <SuggestionsList $show={showSuggestions}>
          {loading ? (
            ...
            suggestions.map((zona, index) => (
              ...
              >
                <SuggestionName>{zona.nome}</SuggestionName>
                {zona.descricao && (
                  <SuggestionDetails>{zona.descricao}</SuggestionDetails>
                )}
              </SuggestionItem>
            ))
          ) : (
            <NoResults>Nenhuma zona encontrada</NoResults>
          )}
        </SuggestionsList>
      </InputWrapper>
      {showAddButton && onAddClick && (
...
export default ZonaAutocomplete
