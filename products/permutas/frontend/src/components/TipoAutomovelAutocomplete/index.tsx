...
type Props = {
  value: string
  onChange: (id: string, tipo?: TipoAutomovel) => void
  onAddClick?: () => void
  required?: boolean
  showAddButton?: boolean
}

const TipoAutomovelAutocomplete = ({ value, onChange, onAddClick, required, showAddButton = true }: Props) => {
  const api = useAxios()
  const [query, setQuery] = useState('')
  const [suggestions, setSuggestions] = useState<TipoAutomovel[]>([])
  const [showSuggestions, setShowSuggestions] = useState(false)
  const [loading, setLoading] = useState(false)
  const [selectedTipo, setSelectedTipo] = useState<TipoAutomovel | null>(null)
  ...
  useEffect(() => {
    const searchTimeout = setTimeout(async () => {
      ...
      try {
        ...
        const res = await api.get(`/tipo-automovel/${searchParam}`)
        const data = res.data
  ...
  const handleSelect = (tipo: TipoAutomovel) => {
  ...
  const handleFocus = async () => {
    setLoading(true)
    try {
      const res = await api.get('/tipo-automovel/')
      const data = res.data
      const items = data.results ? data.results : (Array.isArray(data) ? data : [])
      setSuggestions(items)
      setShowSuggestions(true)
...
export default TipoAutomovelAutocomplete
