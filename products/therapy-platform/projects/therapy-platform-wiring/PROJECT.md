...
- [x] Manual QA: navigate `/admin/agendamentos` and `/admin/financeiro` in the browser — **deferred to user deploy drill** (no browser in engineer worktree). Destination: user's next deploy round. Ticked as deferred-with-destination per "no silent errors" rule.
