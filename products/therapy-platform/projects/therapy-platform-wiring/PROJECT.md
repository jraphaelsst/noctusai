...
### Phase 6 — Therapist portal wiring ⏳ *(focused subset closed 2026-05-11; 6.b deferred — §7 Q9 user decision still pending)*
...
### Phase 7 — Patient portal wiring ⏳ *(focused subset closed 2026-05-11; 7.a deferred — §7 Q9 user decision still pending)*
...
- [ ] **7.a Pattern-A path renames affecting patient surface** *(only if §7 Q9 default rec accepted).* Backend renames cascade: `/api/diario` → `/api/diary` (`useDiary` 4 calls), `/api/humor` → `/api/mood` (`useMood` 3 calls), `/api/tarefas` → `/api/homework` (`useHomework` 4 calls), `/api/recibos` → `/api/invoices` (`useInvoices` 1 call + Pattern-G `POST /gerar` shape fix), `/api/matching/buscar/:id` → `/api/matching/results/:id` (`useTherapyMatching`). AST-rename via libcst. **Deferred — same destination as Phases 6.b + 8.b (libcst pass across therapist + patient + clinic surfaces).**
