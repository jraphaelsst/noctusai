...
- **Created:** 2026-04-20
- **Last updated:** 2026-05-03 (Phase 0 ✅ + §7 Q9-Q14 decided + Phase 1 ✅ + scaffolding-debt sub-tasks pre-delivered)
- **Status:** ⏳ **Phase 0 ✅ + Phase 1 ✅ → settings.py Depends-pattern refactor + notificacoes test audit landed alongside Phase 1 → awaiting "continue" before Phase 2.** Identity resolver, `make_require_role` factory, settings.py 11-endpoint refactor, notificacoes audit (false-alarm, closed) — all shipped same session. **All tests green:** therapy-platform backend 1143/1143, seed-lib backend 448/448, frontend `vite build` clean. Per the project's pause-after-each-phase cadence, awaiting user signal before Phase 2 known-regressions sweep.
...
**Pattern F — `require_role` recurrence inside this product (N=2 local + 1 seed = N=3).** `noctusai_lib.api.auth.require_role` already exists in seed-lib. Product has TWO local re-implementations:
...
Per recurrence rule + absorption-search standing duty: **must formalize** = the seed already has the helper. Phase 1 absorbs both into `noctusai_lib.api.auth.require_role`. No design Q.
...
|---|---|---|
| `app/dependencies.py:55` `require_role()` (~product-wide use) | `noctusai_lib.api.auth.require_role` | Phase 1 (with identity resolver absorption) |
| `app/routers/settings.py:42` `_require_role(user, *roles)` (settings-local) | `noctusai_lib.api.auth.require_role` | Phase 1 |
...
- [x] **Cross-cutting absorption check** vs. `seed/lib/backend/noctusai_lib/{primitives,config,testing,integrations,domain,api,security}/`. Findings in §5.4.6 — N=2 local `require_role` re-implementations duplicate `noctusai_lib.api.auth.require_role` (Phase 1 absorbs both); single-key `_fetch_user_identity` is the Phase-1 bulk-resolver-of-1 case.
...
- §7 Q14 — Pattern F `require_role` consolidation. *No design Q* — Phase 1 absorbs into seed-lib, retire 2 local copies.
...
- [ ] **7.e LGPD walkthrough.** Patient is the most LGPD-sensitive surface — every personal-data endpoint (mood, diary, anamnese, longitudinal, payments, invoices) gets an `noctusai_lgpd_flag` if the new shape touches PII not previously flagged. Per `feedback_lgpd_first`.
...
14. ~~**Pattern F — `require_role` consolidation.**~~ *(Phase 1.)* — **APPLIED at Phase 1 (no design Q):** Phase 1 absorbs `app/dependencies.py:55` `require_role()` and `app/routers/settings.py:42` `_require_role()` into `noctusai_lib.api.auth.require_role`. N=3 recurrence (1 seed + 2 local) → formalize per absorption-search rule. No user signal needed.
