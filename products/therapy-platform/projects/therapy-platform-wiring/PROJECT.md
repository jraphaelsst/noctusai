...
- [x] **8.b Clinic-orphan backend audit — Pattern-A `rooms` rename portion closed (§7 Q9 batch).** Batched with 6.b + 7.a via §7 Q9 renames (Engineer KKK started + KKK-2 finished). Clinic-surface route renamed: `/api/salas` → `/api/rooms` (rooms.py prefix + 4 inner `@router` paths already used EN `/bookings`; KKK-2 updated test docstrings + `/reservas` → `/bookings` URL string literals across 17 rooms tests). **No frontend caller** for `/api/rooms` was found in the audit grep — clinic-side rooms management page does not yet exist, so the rename ships with zero consumer breakage. The non-Pattern-A portion of 8.b (clinics.py settings + branding orphan audit, therapist-config endpoints) is split into 8.b-orphan-audit follow-up (kept in `**Improvements:**` block below); only the Pattern-A path-rename slice is closed here per the §7 Q9 batch scope. **Other 8.x sub-tasks (8.a + 8.c + 8.d + 8.e + 8.f) remain open** — full Phase 8 close future. **Improvements:** see shared 7.a / 6.b / 8.b block below.
