-- 016_status_pagina_dev_visibility.sql — make 'desenvolvimento' pages
...
-- dev-visibility, migration 026/032): therapy.status_pagina had exactly ONE
...
-- desenvolvimento branch (isDevOrOwner) was therefore dead code for therapy.
--
...
--   public.current_org_id() (already defined for therapy in 001_therapy.sql).
...
--
-- Forward-only + idempotent. Apply to the noctusai Supabase (therapy schema).
...
DROP POLICY IF EXISTS "dev_veem_desenvolvimento" ON therapy.status_pagina;
CREATE POLICY "dev_veem_desenvolvimento" ON therapy.status_pagina
