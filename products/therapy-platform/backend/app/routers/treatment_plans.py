...
async def list_plans_for_patient(
    ...
    Treatment plans are clinical content — therapist-of-patient + patient only
    per Q2 of `compliance-audit-reconciliation` 2026-04-22 (clinic_admin/
    platform_admin dropped from clinical content platform-wide).
    """
    user, token = await get_current_user(authorization)
    role = get_user_role(user)
