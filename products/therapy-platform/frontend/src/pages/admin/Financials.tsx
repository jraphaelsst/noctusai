...
// ── Admin-specific hooks (co-located since they are admin-only) ────
...
function useAdminTransactions(filters: Record<string, string>, page: number, pageSize: number) {
  const { user } = useAuthStore();
...
function useAdminPayouts(page: number, pageSize: number) {
  const { user } = useAuthStore();
...
function useDeleteCommissionOverride() {
  const qc = useQueryClient();
  return useMutation({
