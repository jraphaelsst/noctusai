...
import { useAvailabilitySlots } from '@/hooks/useAvailability';
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Button variant="outline" onClick={() => navigate('/therapist/agenda/disponibilidade')}>
...
// ── Appointment detail (shared across therapist/patient) ─────────
...
function AppointmentDetailContent({
  ...
  const participantLabel = viewerRole === 'terapeuta' ? 'Paciente' : 'Terapeuta';
  const participantName =
    viewerRole === 'terapeuta'
      ? appointment.patient_name ?? 'Paciente'
      : appointment.therapist_name ?? 'Terapeuta';

  // Check if cancellation would be late (< 24h)
    ...
    <div className="space-y-4">
      ...
      {isCancellable && isLateCancellation && (
        <p className="text-xs text-red-600">
          Atencao: cancelamento com menos de 24h pode gerar cobranca integral.
        </p>
      )}
    </div>
  );
...
export { AppointmentDetailContent };
