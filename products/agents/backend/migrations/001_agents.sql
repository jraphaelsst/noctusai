-- ============================================================
-- Schema lock — pin name resolution to agents, public
-- WHY:
...
GRANT USAGE ON SCHEMA agents TO anon, authenticated, service_role;
GRANT ALL ON ALL TABLES IN SCHEMA agents TO anon, authenticated, service_role;
GRANT ALL ON ALL SEQUENCES IN SCHEMA agents TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA agents GRANT ALL ON TABLES TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA agents GRANT ALL ON SEQUENCES TO anon, authenticated, service_role;
...
CREATE TABLE agents.status_pagina (
...
ALTER TABLE agents.status_pagina ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "todos_veem_producao" ON agents.status_pagina
...
CREATE TABLE agents.invitations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE agents.invitations ENABLE ROW LEVEL SECURITY;
...
CREATE INDEX idx_agents_invitations_org ON agents.invitations(org_id);
CREATE INDEX idx_agents_invitations_token ON agents.invitations(token);
