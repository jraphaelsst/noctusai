...
-- Every product scaffolded FROM this agents copies 001_seed.sql (which ships
...
-- authenticated product-client read (agents/lib/frontend/src/page-status.ts
-- usePageStatus). This migration makes the agents canonical shape correct
...
--
-- Forward-only + idempotent. products/agents/ and templates/product-agents/
-- MUST stay in sync (pre-commit hook mirrors products/agents → templates on
-- staged change) — this file has a byte-identical sibling in
-- templates/product-agents/backend/migrations/ with agents in place
-- of the literal `agents` schema.
...
DROP POLICY IF EXISTS "dev_veem_desenvolvimento" ON agents.status_pagina;
CREATE POLICY "dev_veem_desenvolvimento" ON agents.status_pagina
...
-- DEV_ROLES const (agents/lib/frontend/src/roles.ts). There is no shared
