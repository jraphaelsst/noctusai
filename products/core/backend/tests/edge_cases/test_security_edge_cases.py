...
class TestOrganizationIsolation:
    ...
    def test_notifications_scoped_to_user(self, client):
        ...
        Endpoint path retargeted from `/api/notifications` (core-owned, deleted
        in `core-seed-wiring-v2` Phase 3) to `/api/notificacoes` (framework-
        bundled, same `public.notifications` table, Portuguese field mapping).
        Test intent preserved: endpoint returns 200 when user has notifications.
        """
...
class TestDataIntegrityEdgeCases:
    ...
    def test_empty_org_no_crash(self, client):
        ...
        Endpoint `/api/notifications` retargeted to `/api/notificacoes` in
        `core-seed-wiring-v2` Phase 3 — core now consumes the framework-
        bundled notifications router.
        """
        endpoints = [
            "/api/licenses",
            "/api/notificacoes",
