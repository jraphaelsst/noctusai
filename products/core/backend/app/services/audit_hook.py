"""Tool-call audit-writer factory — wires seed `make_audit_writer` to
core's `ToolCallAudit` ORM class.

Per `KB § PATTERNS/llm-tool-audit.md`: the LLM dispatcher (in
`audit_digest_service`) calls `audit_writer(record)` after the
`digest_narrative` chat-completion. The writer persists one row to
`public.tool_call_audits` via SQLAlchemy.

Best-effort: the seed `make_audit_writer` rolls back + logs on DB error;
the digest pipeline is never broken by an audit-side failure. See
`noctusai_lib.domain.ai.tool_audit.make_audit_writer` for the guarantee.

**Lazy SQLAlchemy session.** Core's primary data path is the Supabase
admin client (per `KB § PATTERNS/backend.md`). SQLAlchemy is only needed
for the typed-table-class shape `make_audit_writer` requires. We
construct the engine + sessionmaker lazily — the first time
`get_audit_writer()` is called — so:
