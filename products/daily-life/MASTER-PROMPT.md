...
7. **Bootstrap auto-hydrate (`2a1ad89`)** — `scripts/bootstrap/bootstrap-worktree.sh` Step `[4/4]` (∧ `scripts/setup.sh` Step 4/6) auto-enumerates every `products/*/backend/requirements.txt` ∧ pip-installs into the shared venv. Adding a new dep to `products/daily-life/backend/requirements.txt` suffices — fresh worktrees pick it up at hydrate time. No manual `pip install` round-trips.
