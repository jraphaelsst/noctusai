/**
 * Daily Life Product — Unified Layout
 *
 * Follows the NoctusAI product layout pattern:
 * - Single Layout.tsx per product
 * - Nav data defined as static constants
 * - SharedHeader + SharedSidebar via AppShell from design system
 * - useTheme + useActivityRefresh from shared hooks
 * - logoutBehavior="redirect" (SSO — redirects to Core)
 * - Page status filtering via usePageStatus + filterNavByPageStatus
 */
...
import {
  LayoutDashboard, Users, ChevronLeft, CalendarCheck, Home,
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const userRole = ssoCtx.isProductAdmin
    ? "Administrador"
    : ROLE_LABELS[ssoCtx.org.role] || "Daily Life";
