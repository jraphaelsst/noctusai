-- 008_status_pagina_dev_visibility.sql — make 'desenvolvimento' pages
...
-- dev-visibility, migration 026/032): daily_life.status_pagina had exactly ONE
...
-- desenvolvimento branch (isDevOrOwner) was therefore dead code for daily_life.
--
...
--   public.current_org_id() (already defined for daily_life in 001_daily_life.sql).
...
--
-- Forward-only + idempotent. Apply to the noctusai Supabase (daily_life schema).
...
DROP POLICY IF EXISTS "dev_veem_desenvolvimento" ON daily_life.status_pagina;
CREATE POLICY "dev_veem_desenvolvimento" ON daily_life.status_pagina
