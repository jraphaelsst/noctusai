"""
Team Management Router — Invitations and member management for Daily Life.
...
Daily Life supports the full 7-role hierarchy. Corporate customers manage
teams via this router; individual users may still belong to a one-person org.
"""
import logging
...
DAILY_LIFE_ROLES = ("admin", "member")

DAILY_LIFE_ROLE_LABELS = {
    "admin": "Administrador",
    "member": "Membro",
}

import os
DAILY_LIFE_FRONTEND_URL = os.environ.get("DAILY_LIFE_FRONTEND_URL", "http://localhost:8110")
...
def _require_daily_life_admin(user) -> None:
    role = get_user_role(user)
    if role == "platform_admin":
        return

    core = get_core_client()
    check = core.table("noctus_users").select("org_role").eq("id", str(user.id)).execute()
    if check.data:
...
def _get_org_name(org_id: str) -> str:
    ...
    if result.data:
        return result.data[0].get("name", "Organizacao")
    return "Organizacao"
...
async def convidar_membro(body: InviteBody, authorization: Optional[str] = Header(None)):
    ...
    user, token = await get_current_user(authorization)
    _require_daily_life_admin(user)
    org_id = _get_org_id(user)
    db = get_admin_client()

    core = get_core_client()
    existing = core.table("noctus_users").select("id").eq("org_id", org_id).eq("email", body.email).execute()
    if existing.data:
        raise HTTPException(status_code=409, detail="Este email ja e membro da organizacao")

    invite = create_invitation(db, INVITATIONS_TABLE, org_id, body.email, body.role, str(user.id))

    org_name = _get_org_name(org_id)
    inviter_name = _get_user_display_name(user)
    role_label = DAILY_LIFE_ROLE_LABELS.get(body.role, body.role.capitalize())

    send_product_invitation_email(
        to=body.email,
        product_name="Daily Life",
        org_name=org_name,
        role_label=role_label,
        invite_token=invite["token"],
        invited_by=inviter_name,
        base_url=DAILY_LIFE_FRONTEND_URL,
    )

    logger.info("Invitation sent: email=%s org=%s role=%s invited_by=%s", body.email, org_id, body.role, str(user.id))
...
async def validar_convite(request: Request, token: str):
    ...
    return success_response({
        "email": invite["email"],
        "role": DAILY_LIFE_ROLE_LABELS.get(invite["role"], invite["role"]),
        "org_name": org_name,
    })
...
@router.post("/accept")
@limiter.limit("10/minute")
async def aceitar_convite(request: Request, body: AcceptInviteBody):
    ...
    nome_formatado = " ".join(word.capitalize() for word in body.nome.strip().split())
    ...
    try:
        auth_result = db.auth.admin.create_user({
            ...
            "email_confirm": True,
            "user_metadata": {"nome": nome_formatado, "org_id": org_id},
        })
    ...
    logger.info("Invitation accepted: user=%s email=%s org=%s role=%s", new_user_id, email, org_id, role)
...
async def remover_membro(user_id: str, authorization: Optional[str] = Header(None)):
    ...
    member = core.table("noctus_users").select("id, nome, email, org_id").eq("id", user_id).eq("org_id", org_id).execute()
    if not member.data:
