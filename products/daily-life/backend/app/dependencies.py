...
def get_user_role(user) -> str:
    """Resolve the user's role for Daily Life Product access.

    Uses shared ``resolve_sso_role()`` first (org owner/admin or NoctusAI
    admin -> 'platform_admin'). Non-admin users get their org_role or
    default to 'member'.
    """
    ...
    return metadata.get("org_role", metadata.get("role", "member"))
