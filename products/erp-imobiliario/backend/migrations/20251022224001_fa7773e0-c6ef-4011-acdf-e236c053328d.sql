-- Recreate rollup trigger to execute ONLY when a daily meta is created manually
...
BEGIN
  ...
  -- CRITICAL: Only execute rollup if meta was created manually
  ...
  LOOP
    ...
    IF v_agg_id IS NULL THEN
      -- Create aggregated meta if it doesn't exist
