"""Mount-smoke tests for the seed `ai_feedback` standard router on ERP.

Per PROJECT §6 Phase 7 / PF lessons §d.4: 5-test pattern — (a) route
exists, (b) auth gate, (c) happy-path 200, (d) wrong-user isolation
(seed router scopes by `user_id` per `ai_feedback_router.py:97`),
(e) shape.

`ai_feedback` has two endpoints — `POST` to upsert and `GET` to read
the caller's row for a given `output_ref`. The smoke uses `GET` (read
side) which is safe to exercise without setting up upsert seams; the
POST path is exercised by other tests + the seed test suite.
...
class TestAiFeedbackStandardRouterMountSmoke:
    ...
    def test_route_exists(self, client):
        ...
        client._mock_supabase.set_table_data("ai_feedback", [])
        resp = client.get(
            "/api/ai/feedback",
    ...
    def test_isolates_other_users_feedback(self, client):
        ...
        `.eq("user_id", str(user.id))` (ai_feedback_router.py:97).
        Default `mock_user` is `test-user-123`; seed feedback for
        another user against the same `output_ref` and assert it's
        filtered out."""
        client._mock_supabase.set_table_data("ai_feedback", [
            {
                "user_id": "other-user-456",
                "output_ref": "ai_output:abc-123",
                "rating": -1, "notes": "their text",
                "prompt_version": "v1",
            },
        ])
        resp = client.get(
        ...
        assert resp.status_code == 200
        body = resp.json()
        # `other-user-456`'s row must NOT leak through; data should
        # be null (no feedback for `test-user-123` on this ref).
        assert body["data"] is None

    def test_response_shape_contract(self, client):
        """(e) Shape: response carries `data` (the row dict or None) —
        the contract `create_ai_feedback_router` builds
        (ai_feedback_router.py:106)."""
        client._mock_supabase.set_table_data("ai_feedback", [])
        resp = client.get(
            "/api/ai/feedback",
