...
CREATE TABLE academia_de_reciclagem.examples (
...
ALTER TABLE academia_de_reciclagem.examples ENABLE ROW LEVEL SECURITY;

CREATE POLICY "examples_select_own_org" ON academia_de_reciclagem.examples
    FOR SELECT TO authenticated
    USING (org_id = current_org_id());

CREATE POLICY "examples_insert_own_org" ON academia_de_reciclagem.examples
    FOR INSERT TO authenticated
    WITH CHECK (org_id = current_org_id());

CREATE POLICY "examples_update_own_org" ON academia_de_reciclagem.examples
    FOR UPDATE TO authenticated
    USING (org_id = current_org_id())
    WITH CHECK (org_id = current_org_id());

CREATE POLICY "examples_delete_own_org" ON academia_de_reciclagem.examples
    FOR DELETE TO authenticated
    USING (org_id = current_org_id());

CREATE INDEX idx_academia_de_reciclagem_examples_org ON academia_de_reciclagem.examples(org_id);
CREATE INDEX idx_academia_de_reciclagem_examples_ativo ON academia_de_reciclagem.examples(org_id, ativo);
