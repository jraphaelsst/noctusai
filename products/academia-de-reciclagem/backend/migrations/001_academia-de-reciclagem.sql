...
CREATE SCHEMA IF NOT EXISTS academia_de_reciclagem;

-- Grant usage to authenticated users (required for PostgREST)
GRANT USAGE ON SCHEMA academia_de_reciclagem TO anon, authenticated, service_role;
GRANT ALL ON ALL TABLES IN SCHEMA academia_de_reciclagem TO anon, authenticated, service_role;
GRANT ALL ON ALL SEQUENCES IN SCHEMA academia_de_reciclagem TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA academia_de_reciclagem GRANT ALL ON TABLES TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA academia_de_reciclagem GRANT ALL ON SEQUENCES TO anon, authenticated, service_role;
...
CREATE TABLE academia_de_reciclagem.status_pagina (
...
ALTER TABLE academia_de_reciclagem.status_pagina ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "todos_veem_producao" ON academia_de_reciclagem.status_pagina
...
CREATE TABLE academia_de_reciclagem.invitations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE academia_de_reciclagem.invitations ENABLE ROW LEVEL SECURITY;
...
CREATE INDEX idx_academia_de_reciclagem_invitations_org ON academia_de_reciclagem.invitations(org_id);
CREATE INDEX idx_academia_de_reciclagem_invitations_token ON academia_de_reciclagem.invitations(token);
