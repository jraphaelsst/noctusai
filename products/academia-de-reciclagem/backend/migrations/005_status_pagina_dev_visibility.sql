...
-- Every product scaffolded FROM this academia_de_reciclagem copies 001_seed.sql (which ships
...
-- authenticated product-client read (academia_de_reciclagem/lib/frontend/src/page-status.ts
-- usePageStatus). This migration makes the academia_de_reciclagem canonical shape correct
...
--
-- Forward-only + idempotent. products/academia_de_reciclagem/ and templates/product-academia_de_reciclagem/
-- MUST stay in sync (pre-commit hook mirrors products/academia_de_reciclagem → templates on
-- staged change) — this file has a byte-identical sibling in
-- templates/product-academia_de_reciclagem/backend/migrations/ with academia_de_reciclagem in place
-- of the literal `academia_de_reciclagem` schema.
...
DROP POLICY IF EXISTS "dev_veem_desenvolvimento" ON academia_de_reciclagem.status_pagina;
CREATE POLICY "dev_veem_desenvolvimento" ON academia_de_reciclagem.status_pagina
...
-- DEV_ROLES const (academia_de_reciclagem/lib/frontend/src/roles.ts). There is no shared
