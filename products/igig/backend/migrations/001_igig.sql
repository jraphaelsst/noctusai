-- ============================================================
-- Schema lock — pin name resolution to igig, public
-- WHY:
...
CREATE SCHEMA IF NOT EXISTS igig;

-- Grant usage to authenticated users (required for PostgREST)
GRANT USAGE ON SCHEMA igig TO anon, authenticated, service_role;
GRANT ALL ON ALL TABLES IN SCHEMA igig TO anon, authenticated, service_role;
GRANT ALL ON ALL SEQUENCES IN SCHEMA igig TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA igig GRANT ALL ON TABLES TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA igig GRANT ALL ON SEQUENCES TO anon, authenticated, service_role;
...
CREATE TABLE igig.status_pagina (
...
ALTER TABLE igig.status_pagina ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "todos_veem_producao" ON igig.status_pagina
...
ALTER TABLE igig.invitations ENABLE ROW LEVEL SECURITY;
...
CREATE INDEX idx_igig_invitations_org ON igig.invitations(org_id);
CREATE INDEX idx_igig_invitations_token ON igig.invitations(token);
