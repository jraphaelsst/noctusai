...
```
products/igig/backend/app/
  main.py              → create_product_app("IgIg", "igig", settings)
  config.py            → SeedSettings(ProductSettings) — no extra fields
  database.py          → create_database_module(settings, "igig")
...
```
products/igig/frontend/src/
  App.tsx              → createProductApp() + createProductLayout()
  vite.config.ts       → createViteConfig({ port: 8170 }) — 3 lines
...
Schema: `igig` — only `status_pagina` (feature flags) and `invitations` (team invites). Zero domain tables.
