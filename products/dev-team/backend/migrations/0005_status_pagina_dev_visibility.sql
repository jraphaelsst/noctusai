-- 0005_status_pagina_dev_visibility.sql — make 'desenvolvimento' pages
-- visible to dev / owner / admin, closing a gate that hid them from EVERYONE.
--
-- THE DEFECT: dev_team.status_pagina had exactly ONE policy —
...
-- branch (isDevOrOwner) was therefore dead code for dev-team.
--
-- WHY THIS LANDS NOW, FOR AN INATIVO PRODUCT: dev-team is STATUS:INATIVO, so
...
--
-- Forward-only + idempotent. Apply to the noctusai Supabase (dev_team schema).

SET search_path = dev_team, public;
...
DROP POLICY IF EXISTS "dev_veem_desenvolvimento" ON dev_team.status_pagina;
CREATE POLICY "dev_veem_desenvolvimento" ON dev_team.status_pagina
