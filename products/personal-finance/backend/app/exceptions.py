...
class NotFoundError(AppException):
    def __init__(self, resource: str, resource_id: Optional[str] = None):
        ...
        super().__init__(code="NOT_FOUND", message=f"{resource} nao encontrado", status_code=404, details=details)
...
class UnauthorizedError(AppException):
    def __init__(self, message: str = "Autenticacao necessaria"):
...
async def app_exception_handler(request: Request, exc: AppException) -> JSONResponse:
    ...
    return JSONResponse(status_code=exc.status_code, content=format_error_response(exc.code, exc.message, exc.details))
...
async def validation_exception_handler(request: Request, exc: ValidationError) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=422,
        content=format_error_response("VALIDATION_ERROR", "Erro de validacao nos dados enviados", {"errors": errors}),
    )
...
async def postgrest_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    if pg_code == "PGRST116":
        return JSONResponse(status_code=404, content=format_error_response("NOT_FOUND", "Recurso nao encontrado"))

    # 23505: unique constraint violation → 409
    if pg_code == "23505":
        logger.warning(f"PostgREST unique violation: {message}")
        return JSONResponse(status_code=409, content=format_error_response("CONFLICT", "Registro duplicado — este recurso ja existe"))

    # 23503: foreign key violation → 400
    if pg_code == "23503":
        logger.warning(f"PostgREST FK violation: {message}")
        return JSONResponse(status_code=400, content=format_error_response("BAD_REQUEST", "Referencia invalida — recurso relacionado nao encontrado"))

    # 23502: not-null violation → 400
    if pg_code == "23502":
        logger.warning(f"PostgREST not-null violation: {message}")
        return JSONResponse(status_code=400, content=format_error_response("BAD_REQUEST", "Campo obrigatorio nao preenchido"))

    # All other PostgREST errors — log and surface the message
    logger.exception(f"PostgREST error [{pg_code}]: {message}")
    return JSONResponse(status_code=500, content=format_error_response("INTERNAL_ERROR", message or "Erro interno do servidor"))


async def generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    logger.exception(f"Unhandled exception: {exc}")
    return JSONResponse(status_code=500, content=format_error_response("INTERNAL_ERROR", "Erro interno do servidor"))
