"""
Tests for Team Management router — /api/team (Personal Finance)
...
from noctusai_shared.testing import MockSupabaseClient, MockSupabaseResponse
...
def _mock_core_client(mock_sb):
    """Patch get_core_client at the router module level (PF imports at top)."""
...
class TestInviteSuccess:
    def test_invite_success(self, client):
        # _require_pf_admin queries noctus_users for org_role (1st call),
        # then existing member check also queries noctus_users (2nd call).
        # Use sequential responses to handle both.
    ...
    def test_invite_duplicate_email(self, client):
        ...
        client._mock_supabase.set_sequential_responses("noctus_users", [
            MockSupabaseResponse(data=[{"id": USER_ID, "org_role": "owner"}]),  # admin check
            MockSupabaseResponse(data=[{"id": "existing-1", "email": "dup@test.com"}]),  # existing member
        ])
...
class TestRemoveMember:
    def test_remove_member(self, client):
        # admin check queries noctus_users (1st), then member check (2nd),
        # then org owner check (organizations), then delete (noctus_users again)
        client._mock_supabase.set_sequential_responses("noctus_users", [
            MockSupabaseResponse(data=[{"id": USER_ID, "org_role": "owner"}]),  # admin check
