...
products/seed/backend/app/
  main.py              → create_product_app("Seed Product", "seed", settings)
  config.py            → SeedSettings(ProductSettings) — no extra fields
  database.py          → create_database_module(settings, "seed")
...
products/seed/frontend/src/
  ...
  vite.config.ts       → createViteConfig({ port: 8100 }) — 3 lines
