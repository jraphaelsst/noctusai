...
CREATE TABLE community.examples (
...
ALTER TABLE community.examples ENABLE ROW LEVEL SECURITY;

CREATE POLICY "examples_select_own_org" ON community.examples
    FOR SELECT TO authenticated
    USING (org_id = current_org_id());

CREATE POLICY "examples_insert_own_org" ON community.examples
    FOR INSERT TO authenticated
    WITH CHECK (org_id = current_org_id());

CREATE POLICY "examples_update_own_org" ON community.examples
    FOR UPDATE TO authenticated
    USING (org_id = current_org_id())
    WITH CHECK (org_id = current_org_id());

CREATE POLICY "examples_delete_own_org" ON community.examples
    FOR DELETE TO authenticated
    USING (org_id = current_org_id());

CREATE INDEX idx_community_examples_org ON community.examples(org_id);
CREATE INDEX idx_community_examples_ativo ON community.examples(org_id, ativo);
