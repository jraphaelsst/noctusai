...
class TestPatchCliente:
    ...
    def test_patch_nacionalidade_is_accepted_and_stamps_manual_origin(
        ...
        resp = client.patch(
            f"/api/clientes/{a1}",
            json={"nacionalidade": "italiana"},
            headers=_auth(),
        )
        assert resp.status_code == 200, resp.text

        payload = scoped.table("clientes").updated_payloads[-1]
        assert payload["nacionalidade"] == "italiana"
        assert payload["nacionalidade_origem"] == "manual"
        assert payload["nacionalidade_documento_id"] is None

    def test_patch_nacionalidade_origem_is_not_accepted_from_the_body(
        self, client, scoped
    ):
        """Provenance is the server's to stamp — same discipline every
        other extracted field enforces (`extra='forbid'` catches an unknown
        ...
        resp = client.patch(
            f"/api/clientes/{a1}",
            json={"nacionalidade": "italiana", "nacionalidade_origem": "cnh"},
            headers=_auth(),
        )
        assert resp.status_code == 422, resp.text
    ...
    def test_patch_rg_different_from_cpf_is_accepted(self, client, scoped):
        ...
        resp = client.patch(
            f"/api/clientes/{a1}",
            json={"cpf": "412.954.238-98", "rg": "52.179.965-X"},
            headers=_auth(),
        )
        assert resp.status_code == 200, resp.text
        assert resp.json()["cpf"] == "412.954.238-98"
