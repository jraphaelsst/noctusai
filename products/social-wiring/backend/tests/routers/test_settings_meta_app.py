"""Tests for the Meta App config tab — PUT/GET /api/settings/meta-app*
(Wave 2, `wave2-contract.md` Slice 2A item 5).
...
class TestSecretNeverEchoed:
    ...
    def test_status_response_never_contains_the_secret(
        ...
        fake_store_override.put("meta_app_secret", "SUPER-SECRET-VALUE")
...
class TestPartialUpdate:
    def test_app_id_only_leaves_secret_untouched(
        ...
        fake_store_override.put("meta_app_secret", "ORIGINAL-SECRET")
        resp = admin_client.put(
            "/api/settings/meta-app",
        ...
        assert resp.status_code == 200, resp.text
        assert fake_store_override.get("meta_app_id") == "only-id-update"
        assert fake_store_override.get("meta_app_secret") == "ORIGINAL-SECRET"
    ...
    def test_no_fields_400(self, admin_client, fake_store_override):
        resp = admin_client.put(
            "/api/settings/meta-app", json={}, headers=_auth_header()
        )
