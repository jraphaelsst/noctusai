"""Fixtures for the ``scheduling`` module router tests (Wave 2.3,
deferred-then-added at W2-INTEGRATE: the ``sched_*`` tables now live in
``migrations/001_social-wiring.sql`` so ``MockSupabaseClient`` schema
validation passes for the scheduling-schema chains).
...
produces once the ``MODULES`` append lands. Mirrors the proven
``tests/modules/email_marketing/conftest.py`` seed-patch shape
(class-level ``DatabaseModule`` patch + per-fixture consent re-bind) so
the scheduling router tests are isolated the same way.
"""
...
def client():
    ...
    with patch(
        ...
        app = create_product_app(
            name="Social Wiring (scheduling test harness)",
