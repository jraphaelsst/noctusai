...
class TestPerClienteScope:
    """Migration 019 — scope = (org_id, marca_id)."""

    def test_org_default_and_cliente_are_distinct_rows(self, store_and_db):
        """An org-default (marca_id NULL) and a per-cliente connection coexist."""
        store, _ = store_and_db
        org_id = uuid4()
        marca_id = uuid4()
        ...
        scoped = store.get_connection(org_id, marca_id=marca_id)
        assert default is not None and default.marca_id is None
        assert default.server_prefix == "us6"
        assert default.audience_id == "aud-org"
        assert scoped is not None and scoped.marca_id == marca_id
    ...
    def test_downstream_get_connection_resolves_org_default(self, store_and_db):
        """(e) The no-marca_id path (deps/require_mailchimp_client) resolves the
        ...
        store.upsert_connection(
            org_id=org_id, api_key="cli-eu1", server_prefix="eu1", marca_id=marca_id
        )
        # Mirrors deps.require_mailchimp_client: get_connection(org_id, decrypt=True).
        record = store.get_connection(org_id, decrypt=True)
        assert record is not None
        assert record.marca_id is None
        assert record.api_key == "org-us6"
        assert record.server_prefix == "us6"

    def test_upsert_idempotent_per_scope(self, store_and_db):
        """(d) Re-upserting the same (org, marca_id) replaces the row in place."""
        store, _ = store_and_db
        org_id = uuid4()
        marca_id = uuid4()
        first = store.upsert_connection(
            org_id=org_id, api_key="k1-us6", server_prefix="us6", marca_id=marca_id
        )
        second = store.upsert_connection(
            org_id=org_id, api_key="k2-eu1", server_prefix="eu1", marca_id=marca_id
        )
        assert first.id == second.id  # same row, not a duplicate
        got = store.get_connection(org_id, marca_id=marca_id, decrypt=True)
    ...
    def test_scoped_get_missing_returns_none(self, store_and_db):
        ...
        assert store.get_connection(org_id, marca_id=uuid4()) is None
    ...
    def test_set_audience_scoped(self, store_and_db):
        ...
        store.upsert_connection(
            org_id=org_id, api_key="cli-us6", server_prefix="us6", marca_id=marca_id
        )
        updated = store.set_audience(org_id, "aud-cli-new", marca_id=marca_id)
    ...
    def test_delete_scoped_leaves_org_default(self, store_and_db):
        ...
        assert store.delete_connection(org_id, marca_id=marca_id) is True
        assert store.get_connection(org_id, marca_id=marca_id) is None
