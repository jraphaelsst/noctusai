...
def _fetch_filtered_rows(client: Any, org_id: UUID, filters: LeadFilters) -> list[dict]:
    ...
    The loop itself belongs to the seed's shared pager
    (``noctusai_lib.integrations.persistence.paging.iter_paged_rows``),
    which owns the no-progress guard and the page ceiling — this function
    only supplies the per-page query.
    """
...
def fetch_filtered(client: Any, org_id: UUID, filters: LeadFilters) -> list[dict]:
    ...
    🔴 Integration note (2026-08-18): two branches changed this function
    at once — one moved the paging loop onto the seed's shared pager, the
    other added the ``.in_()`` batching above. They are orthogonal and
    BOTH are kept: taking either side alone would have silently dropped
    the other's fix (the seed consolidation, or the URL-overflow 500 that
    ``/clientes/revisao`` hit in prod).
    """
