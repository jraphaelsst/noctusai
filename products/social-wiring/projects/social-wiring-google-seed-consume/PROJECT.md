...
- **Status:** Phase 0 ✅ · 1 ✅ (`b7dd6204`) · 2 ✅ (`469b5c54`) · 3 ✅ (`345ab867`+`0ab03aca`) · 4 ✅ (`d61a453e`) · 5 ✅ (`989b81d9`) · 6a ✅ (`9fe25d4d` yt + `81a52330` dr — projection enrichment landed despite concurrent-checkout slip; recovery + auto-improve codified) → **Phase 6b dispatching SERIALLY** (youtube chunked-upload-retry seed lift; cardinal rule: no more parallel dispatch on this branch) (seed `[F]` token_store table-shape seam — Wave-1 engineer surfaced a tree-verified seed gap; Phase 1 re-scoped to consume it + 40-site metadata-map migration). Throughput mode ("resolve it all"). Isolated worktree `../noctusai-wt-sw-google` on `feat/sw-google-seed-consume` (main tree parallel-agent-contended — do NOT execute there).
...
### Phase 6a — Projection enrichment `[F]` (youtube + drive parallel-then-corrected) ✅ (`9fe25d4d` yt + `81a52330` dr, 2026-05-19)
...
### Phase 6b — Seed youtube chunked-upload-retry lift `[F]`
- [ ] Lift the `_resumable_upload_with_chunk_retry` algorithm (per-chunk retry budget resets on chunk success; quota-exceeded short-circuit) into seed `RealYoutubeClient.upload_video`. Restores the reliability hardening removed fix-on-contact in Phase 4. Pilot-gated.

### Phase 6c — social-wiring-youtube-modularize
- [ ] Fold thin youtube surface (videos/upload/dashboard routers + youtube_service + video_cache_service + upload_service + download_cache + schemas/video,upload) into `modules/youtube/` package, resolving the app-level-vs-`modules/` asymmetry from Phase 0. Comes AFTER 6b (consumes the final seed surface).

### Phase 6 — Cleanup, doc reconcile, verify
- [ ] Reconcile `MASTER-PROMPT.md` / `README.md` (doc⊥code → doc=true; remove the drift marker).
- [ ] Full verify: `pytest` (social-wiring + seed + pilots erp·therapy·core + mcp/google + mcp/noctusai) · frontend `tsc --noEmit` + `vite build` · keeper `--review` · `noctus.hound.scan` · KB sync · three-way-sync any residual methodology learnings.
- [ ] Project close via `noctus.dev.archive`.
