...
- **Status:** Phase 0 ✅ · 1 ✅ (`b7dd6204` token_store seam) · 2 ✅ (`469b5c54` vault consume, fork removed) · 3 ⏳ (`345ab867` OAuth 3a+3b shipped — `/api/{youtube,calendar}/oauth/callback` preserved, 383+73 architect-verified; **3c PKCE seam dispatching** per Option-1 user decision + new `KB § PATTERNS/absorbed-product-seed-shape-seam.md` methodology pattern) (seed `[F]` token_store table-shape seam — Wave-1 engineer surfaced a tree-verified seed gap; Phase 1 re-scoped to consume it + 40-site metadata-map migration). Throughput mode ("resolve it all"). Isolated worktree `../noctusai-wt-sw-google` on `feat/sw-google-seed-consume` (main tree parallel-agent-contended — do NOT execute there).
...
### Phase 3 — OAuth seam(s) `[F]` + lifecycle consume ⏳ (345ab867: 3a+3b shipped; PKCE seam pending)
...
- [x] **3b CONSUME:** calendar/drive/youtube hand-rolled OAuth → seed `GoogleProvider` (lifecycle: authorize/exchange/refresh/revoke). External callback URLs **preserved unchanged** by construction: `/api/youtube/oauth/callback` + `/api/calendar/oauth/callback`. Architect re-ran: social-wiring 383 / seed oauth+token_store 73, 0 fail. [A] deviations: consumed `GoogleProvider` lifecycle primitive (not full `oauth_router` — product routes own rich post-exchange behavior, brief explicitly said "only lifecycle to seed"); `_get_service` Credentials-refresh stays Phase 5 (API-layer scope).
- [ ] **3c `[F]` SEED:** add **PKCE** (`code_challenge`/`code_verifier` round-trip, `use_pkce=False` default → back-compat) to `noctusai_lib.security.oauth.GoogleProvider`. Restore social-wiring's YouTube `Flow.code_verifier` round-trip via the new seam (re-activate the Redis verifier persistence that's currently dead-but-harmless in `settings_router.py`). The canonical "absorbed-product seed-shape-seam" pattern (N=3 instance — see `KB § PATTERNS/absorbed-product-seed-shape-seam.md` for the methodology body; no duplication here). Pilot-gate.
