...
Per `KB § PATTERNS/llm-tool-audit.md`: the LLM dispatcher (in the worker)
calls `audit_writer(record)` after every tool dispatch. The writer
persists one row to `media_scheduling.tool_call_audits` via SQLAlchemy.
...
Supabase admin client (per PROJECT.md §2). SQLAlchemy is only needed for
the typed-table-class shape `make_audit_writer` requires. We construct
the engine + sessionmaker lazily — the first time `get_audit_writer()`
is called — so:
