...
- **Ship-consent riders — bless never carries unapproved work.** Before bless run `release stage=manifest`; approval is per PROJECT via `noctus.dev.ship_consent` (the USER types the phrase — never author it for another agent's project); unapproved riders ⇒ bless cuts `release/<stamp>`, then `stage=backmerge`. → `KB § PATTERNS/devops/ship-consent-riders.md`
