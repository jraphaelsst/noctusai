...
owns_kb:
  ...
  - CONTEXT/PATTERNS/devops/ship-consent-riders.md
  - CONTEXT/PATTERNS/devops/predeploy-leg-verify-or-block.md
  - CONTEXT/PATTERNS/devops/environment.md
  - CONTEXT/PATTERNS/devops/prod-cache-container.md
  - CONTEXT/PATTERNS/devops/cache-deploy-mirror.md
  - CONTEXT/PATTERNS/devops/ssh-deploy-key-restrictions.md
  - CONTEXT/PATTERNS/devops/product-lockfile-and-slug-drift.md
  - CONTEXT/PATTERNS/devops/tunnel-ingress-source-of-truth.md
  ...
  - CONTEXT/PATTERNS/common/cache-as-agent-tool.md
  - CONTEXT/PATTERNS/common/vectorize-embed-cache-framework.md
...
> **Inherits CLAUDE.md §1 universal rules** (auto-loaded). This file is the SPECIALIST L1 index per `KB § PATTERNS/common/agent-context-architecture.md`. **Apply the `engineer-seed` standing protocol** (stay-in-worktree · on-disk verification · commit-own-branch-only · file-disjoint · AST-first for `.py` / `.ts` · scoped verification · short-form return).
...
- **Ship on the owner's ask — no approval phrase (2026-09-24).** Default `release stage=bless` (`mode=ff`) FFs main to the newest CI-qualifying-green commit `main..dev` (not necessarily the tip — R1); the rider manifest is informational. Never ask the owner for a per-project phrase. A new product's first prod exposure still needs `prod_consent`. `mode=cut`/`refuse` are opt-ins for shipping only approved work. → `KB § PATTERNS/devops/ship-consent-riders.md`
- **A `predeploy_check` leg must verify or block — never silently `ok=True` on a state it couldn't check.** `check_predeploy_leg_verify_or_block` (high) AST-derives `DEFAULT_CHECKS` + flags a literal `return True` with no preceding failure branch and no explicit "skip" marker in its message; the legitimate "nothing to verify" case (no frontend dir, no prod-env snapshot) is allowed only when it says so. → `KB § PATTERNS/devops/predeploy-leg-verify-or-block.md`
...
- **Cache-portable architecture (Tier-1 + Tier-2).** `<git-common-dir>/noctusai/cache/*.sqlite` (Tier-1, shared by ALL worktrees of this repo — one physical SQLite per content-addressed cache; the single-slot mirrors keeper-patterns / agent-context / auto-improvement / absorptions are per-tree at `<git-dir>/noctusai/cache/`, Tier 1a) + prod pgvector `noctus-cache-pg:5432` (Tier-2, machine-portable). Sync: `cache_deploy_mirror` (local → remote) + `cache_pull` (remote → local, auto-on-empty default ON; opt-out `NOCTUS_DISABLE_AUTO_CACHE_PULL=1`). One-time legacy `.claude/cache/` migration (copy, not move, sentinel-gated). 11 cache-path sites consolidated through `cache_backend.cache_path()` + `cache_dir()`. Concurrent-refresh guard: `acquire_refresh_lock()` (fcntl flock, non-blocking) wraps every CLI refresh handler — second writer skips cleanly, no SQLite write storms. → `KB § PATTERNS/common/cache-portable-architecture.md`
- **Cache-deploy-mirror — verify the row count, and read BOTH embedding siblings.** `cache_deploy_mirror` ships local SQLite (Tier-1) → prod pgvector (Tier-2), vectors verbatim (no re-embed). It now REFUSES `ok:true` on a shortfall — `rows_written` must equal `source_total` or the per-cache transaction rolls back with a named delta. 🔴 A chunk's embedding lives in ONE of two siblings (`*_vec` / `*_embeddings_json`) depending on whether the environment that last refreshed it had the UNPINNED `sqlite-vec` installed, so reading one sibling silently ships a fraction of the corpus; `noctus.dev.repair_embedding_cache_drift` is the lossless zero-re-embed repair for already-split caches. → `KB § PATTERNS/devops/cache-deploy-mirror.md`
- **Cache-as-agent-tool — the caches ARE the search engines.** When researching anything (design / methodology / code / structure / history / who-owns-what), REACH FOR A CACHE FIRST. Two flavors: semantic (vector — `kb_search` / `code_search` / `memory_search` / `corpus_search`) + structural (graph — `noctus.graph.*` over 23k+ nodes / 38k+ edges). `grep` / `Read` are CONFIRMATION tools, not DISCOVERY tools. → `KB § PATTERNS/common/cache-as-agent-tool.md`
- **Vectorize → embed → cache (the unified pipeline, META-RULE).** Any vectorize/embed/cache slice follows the SAME 3-leg shape: (1) caching-architecture decision FIRST (extend an existing cache by `chunk_kind` before spawning a sibling; N≥3 same-shape ⇒ extend), (2) embed via `noctusai_lib.integrations.llm.generate_embedding` ONLY (never inline OpenAI; cost-log to `vector-costs.ndjson`; deterministic `source_sha` gates idempotency), (3) cache persistence with the 3-leg mirror contract (eager + lazy + freshness keeper) + WAL + `acquire_refresh_lock`. 6+ existing instantiations (4 vector caches + organ chunk + build-learn re-embed). → `KB § PATTERNS/common/vectorize-embed-cache-framework.md`
