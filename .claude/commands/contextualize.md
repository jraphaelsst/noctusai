...
- "Hot drift surfaces" — the graph emits an `auto_improvement_event` node only at ≥3 events on the SAME target, so `noctus.graph.query "" kinds=["auto_improvement_event"]` is frequently empty. For the orientation-useful list of OPEN drift, query `noctus.dev.auto_improvement_query` and surface the not-yet-codified entries (`s1` / `s2-memory`). **Heal-on-contact FIRST:** run `noctus.dev.auto_improvement_reconcile dry_run=True` — any entry it reports under `reconciled` is ALREADY DONE (its `resolve_when` predicate passes against the live tree); EXCLUDE those from "hot drift" (else you re-recommend landed work). If `reconciled` is non-empty, run `dry_run=False` + commit the ndjson (a small `chore(ledger)` commit) so the surface stays truthful next session.
