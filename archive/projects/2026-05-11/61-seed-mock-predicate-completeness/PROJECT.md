...
- **2026-05-11** — Phase 1 landed by Engineer Q (SEED-MOCK-PREDICATE). Two structural fixes to `seed/lib/backend/noctusai_lib/testing/mocks.py`:
  1. **`_eval_is` — PostgREST IS-NULL / IS-TRUE / IS-FALSE + soft compat layer.** Replaced literal-Python `is` with proper dispatch on the wire-format sentinels (`"null"` / `"true"` / `"false"`). Architect-decided **Option A soft compat**: each branch ORs PostgREST-correct semantics with literal-string equality so Engineer T's `"deleted_at": "null"` shim seeds keep matching. Non-sentinel values fall through to literal equality (preserves `.is_(col, None)` callers via `None == None → True`).
  2. **`_FilterMixin.not_` — flag-based negation.** Replaced `@property → return self` no-op with a `@property` that sets `_next_negate=True` and returns self for chaining. The flag is consumed at the next `_record(...)` call (which now stores 4-tuples `(op, col, value, negated)`); `_row_matches_predicates` inverts the evaluator result when `negated=True`. Flag resets after every consume so `.not_.eq(a, x).eq(b, y)` only negates the first eq. Backward-compat: 3-tuple predicates are still accepted in the matcher.
  3. **Documented divergences.** `.not_.in_(col, [...])` uses Python's natural negation (`not (cell in list)`) which DOES include rows where cell is None (real PostgREST excludes them). `.not_.match({a:1, b:2})` applies the negate flag to each decomposed eq independently — yields `(a!=1 AND b!=2)`, stricter than PostgREST's `NOT (a=1 AND b=2)`. Both documented in `_row_matches_predicates` + `match()` docstrings. No production call site uses `.not_.match(...)`; `.not_.in_(...)` is used by therapy/erp services and is exercised by the new regression tests.
  - **Regression tests (13 new in `seed/lib/backend/tests/testing/test_mocks.py`):** TestEvalIsNull positive + negative (4 tests), TestEvalIsTrue + IsFalse positive + negative (4 tests), TestEvalIsLiteralStringCompat (shim compat), TestEvalIsWithPythonNoneValue (backward compat), TestNotEq, TestNotIn, TestNotIsNull (IS-NOT-NULL semantics), TestNotChainReset, TestNotDouble (real-world `erp/campo.py:243` pattern), TestNotEqThroughDeleteFilter (UPDATE/DELETE path coverage).
  - **Verification:** 33/33 green in `seed/lib/backend/tests/testing/test_mocks.py` (13 new + 20 pre-existing). Engineer T's 221 therapy regression tests (`tests/edge_cases` + `tests/routers/test_observations_router.py` + `tests/routers/test_messaging_router.py`) all pass via the compat layer. Personal-finance backend: 23 failures both pre- and post-change (pre-existing latent bugs unrelated to seed mocks). ERP-imobiliario backend: 60 post-change vs 61 pre-change failures — Phase 1 **fixed** `test_list_active_members` (which depended on `.is_("deleted_at", "null")` IS-NULL semantics) and introduced zero new failures. Mailing backend: 6 failures both pre- and post-change (baseline).
  - **Phase 3 ready to file** — `THE-MOCK-SHIM-CLEANUP` sweeps the 4 therapy test files (`tests/routers/test_messaging_router.py`, `tests/routers/test_observations_router.py`, `tests/edge_cases/test_security.py` ×2) to replace `"deleted_at": "null"` with `"deleted_at": None` (natural shape). The soft compat layer makes this a leisurely cleanup with zero correctness pressure.
