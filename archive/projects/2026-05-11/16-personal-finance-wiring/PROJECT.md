...
- **Created:** 2026-05-03
- **Last updated:** 2026-05-10 (Phase 5 ✅ — scheduler/yfinance/AI-indicator divergent batch)
- **Status:** Phases 0, 1, 2, 3, 4, 5 ✅. Phase 5 shipped: Recorrentes scheduler banner + `<StaleBadge>` component + ts-morph-tested decision function. Seed-side `scheduler` standard router deferred (out-of-scope per standalone mode) — proposal filed. Phases 6-7 pending.
...
> **Standalone-mode note (inherited from Phase 1).** Seed/cross-product files remain out of dispatch scope. PF-5's seed-side `scheduler` standard-router gift is **deferred-with-destination** (logged in `proposals/phase-5-scheduler-standard-router.md`); PF ships the visible-in-UI banner sub-task using the existing `proxima_data` field on each Recorrente. Sub-tasks downstream of the deferred seed change are documented + skipped, never silently absorbed.

- [x] **PF-5: Scheduler artifacts in UI — partial / product-side.** Seed-side `scheduler` standard router DEFERRED (out-of-scope; phase-end proposal filed). Product-side **banner shipped**: `RecorrenteRow` (`products/personal-finance/frontend/src/pages/Recorrentes.tsx`) now renders an "Auto" badge + "Próxima execução automática: {proxima_data}" line on every `is_automatico=true` row. "Última execução" indicator deferred — no `ultima_execucao` column exists on the `recorrentes` table; surfacing it requires the seed-side scheduler router OR a new `proxima_data → ultima_execucao` migration, both out of scope.
- [x] **PF-6: yfinance degraded-mode UX — component shipped, consumers pending.** `<StaleBadge>` + pure `computeStaleness(cotacao, kind, now)` decision function (`products/personal-finance/frontend/src/components/StaleBadge.tsx`) — threshold `15min`/`30min`/`20min` for stocks/funds/other per §7 Q5; decision tree dry-run → no-timestamp → old → fresh. **No consumer wired**: surveyed `Watchlist.tsx`, `WatchlistDetalhes.tsx`, `Carteira.tsx`, `CarteiraDetalhes.tsx` — none fetch live cotacao (`Cotacoes.tsx` page does not exist; persisted `preco_atual` is read off the `ativos` table, not `useCotacao`). The badge ships ready-to-wire when a real-time-quote surface lands.
- [x] **PF-7: AIIndicator runtime smoke — documented absence, no wiring.** Phase 0 already classified PF-7 as `OK` for `ref_type="transacao"` on `Transacoes.tsx`. Re-checked: backend `app/routers/ai.py` only emits `_persist_indicator(db, "transacao", ...)` from `categorize_transaction_endpoint` (line 95) and `recurring_flag_endpoint` (line 192). No `ref_type="ativo"` producer exists. Per PROJECT instruction "if absent, document the absence and skip" — Carteira detail wiring **skipped intentionally**.
- [x] Backend tests: yfinance fallback path **already covered** (4 tests at `products/personal-finance/backend/tests/services/test_cotacoes_service.py:18,45,81,107` — `test_returns_mock_when_yfinance_unavailable`, `test_falls_back_to_mock_on_yfinance_error`, etc.). Scheduler-artifact router tests DEFERRED with the seed gift.
- [x] Frontend tests: 11 `StaleBadge.test.tsx` tests (decision-tree + render) + 3 `Recorrentes.test.tsx` row-level banner tests. Both files added under their respective `__tests__` directories.
- [x] Manual browser QA: **deferred to integration-pass** (no browser in worktree; logged as deferred-with-destination per "no silent errors" rule — destination: project's §7 integration round at user-driven QA pass).
- [x] Keeper review + **Improvements** + phase proposal — see below.

**Improvements:**
- **Worktree env recurrence — N=2 confirmed.** Fresh worktree needed `npm install` in `products/personal-finance/frontend/`, `seed/framework/frontend/`, AND `seed/lib/frontend/` for vitest to resolve `jsdom` + `react/jsx-dev-runtime` + `@supabase/supabase-js`. Phase 3 logged the first two; Phase 5 adds `seed/lib/frontend/`. **N=2 trigger fires** — three-way recurrence: triage at decision time → recommend formalize by extending `KB § PATTERNS/seed-workspace.md § Worktree bootstrap` with an explicit "install seed/lib/frontend deps too" note + a `bootstrap-worktree.sh` script that runs the three `npm install` commands. Out-of-scope for this dispatch (touches KB + scripts); logged here for orchestrator absorption.
- **Page-component testability — break the seed-import chain via vi.mock.** Recorrentes page imports `@/hooks/useRecorrentes` which transitively pulls `@noctusai/seed/infra` → `react-router-dom` → seed/lib/frontend deps. To unit-test the row, the test file mocks all 6 hooks + Modal + ConfirmDialog at module scope before importing the SUT. **Pattern is N=1 here** (StaleBadge is dependency-free); if next page-level test repeats the same 8-mock preamble, the recurrence rule fires → extract `mockPageHooks()` test helper in `products/personal-finance/frontend/src/__test_helpers__/`. Captured for awareness, not action.
- **`<StaleBadge>` is a degraded-mode UX primitive, not PF-specific.** ERP also has stock/asset surfaces (asset management product). When ERP's equivalent ships, **N=2 triggers the absorption rule** — move to `seed/lib/frontend/src/components/StaleBadge.tsx` (parameterized with `fonteValues=['dry-run', 'mock', ...]` + custom freshness map). Currently N=1 (PF-only); accept-with-rationale until the ERP consumer surfaces. Filed in `findings.md` Category 4 for cross-product visibility.
- **`computeStaleness` is a pure function — test seam via `now: () => number` injection.** No global `Date.now` patching, no `vi.useFakeTimers`, no monkey-patching. Test fixture anchors `NOW_ISO = "2026-05-10T12:00:00.000Z"` and threads `fixedNow` through. Pattern matches "no workarounds — and no monkey-patching" rule cleanly. Lifting candidate: any future time-sensitive seed primitive (e.g. a "stale cache" indicator on the team-roster surface) should adopt the same shape.
- **Scope discipline — the "wire to N pages" sub-task did not exist in reality.** PROJECT.md PF-6 said "useCotacoes hook + Watchlist + Cotacoes + WatchlistDetalhes pages: render `<StaleBadge>` …" — but `Cotacoes.tsx` doesn't exist as a page, and Watchlist/WatchlistDetalhes/Carteira/CarteiraDetalhes don't actually call `useCotacao(ticker)` (they render persisted `preco_atual` from the `ativos` table). Phase 0 didn't surface this. **Methodology lesson**: Phase 0 audit should verify consumer existence, not just hook signature shape. Captured in findings.md Category 2 (slips).
- **Replication-to-seed symmetry — language hit at PF-5 scope-line.** PROJECT.md PF-5 says "ship `noctusai_seed.standard_routers["scheduler"]` as a cross-product seed gift" — the right per-product code count for this is ZERO. Standalone-mode constraint correctly defers the seed change to a cross-product follow-up. Filed in `proposals/phase-5-scheduler-standard-router.md`.
...
- [x] **6.a Auth surface walkthrough.** PF `Login.tsx` + `ForgotPassword.tsx` consume the seed design-system (`LoginForm`, `ForgotPasswordPage`) wired to `supabase` from `@noctusai/seed/infra`. There is NO per-product `SSOCallback.tsx` file — the seed `createProductApp` factory mounts `<SSOCallback>` at `/sso` automatically when `supabase` is present (verified at `seed/framework/frontend/src/app.tsx:253-264`); PF inherits this. `resolve_sso_role` + `get_sso_context` (at `seed/lib/backend/noctusai_lib/api/auth.py:118,149`) are pure dict-extraction helpers over `user.user_metadata` — org type-neutral by construction (no `is_personal` branching needed; `ensure_pf_personal_org` writes `is_personal=True` to `public.organizations`, which `user_metadata.org_id` references regardless of type). Component tests filed: `Login.test.tsx` (2 tests), `ForgotPassword.test.tsx` (1 test). **Manual browser QA DEFERRED with destination**: real Supabase login round-trip, SSO redirect, logout, forgot-password email — destination: user/orchestrator at Phase 7 manual QA pass.
- [x] **6.b Invitations surface (Equipe.tsx + AcceptInvite.tsx).** Migration `005_invitations.sql` ships the `invitations` table with 8 columns (`id, org_id, email, role, invited_by, token, status, expires_at, created_at`) — matches the seed `create_invitation` + `validate_invitation` writers. `AcceptInvite.tsx` consumes seed `AcceptInvitePage` wired to `acceptEndpoint="/api/team/accept"` (the page also probes `/api/team/accept/validate` per the component's source) — pinned by `AcceptInvite.test.tsx`. Equipe page's 5 direct-fetch callsites against the seed `team` standard router verified at PROJECT §5.2.3 and pinned by `Equipe.test.tsx`. **Seed-level contract drift surfaced and DEFERRED with destination**: the seed `_create_team_router` POST `/accept` handler invokes `accept_invitation(db=..., schema=..., token=..., user_id=..., email=..., password=..., name=...)` (`seed/framework/backend/noctusai_seed/routers.py:161-175`) but the actual `accept_invitation` function signature is `accept_invitation(db, table: str, invitation_id: str)` (`seed/lib/backend/noctusai_lib/domain/invitations.py:168`) — these don't match. Same drift on `validate_invitation` (router passes `schema=`, function takes `table:`). The flow is broken on the runtime path. PF has N=1 consumer (depends on seed; standalone-mode dispatch can't touch seed). **Destination: `proposals/phase-6-seed-team-router-accept-drift.md`** (filed). **Manual browser QA DEFERRED with destination**: invite, accept, revoke, remove — destination: user/orchestrator at Phase 7 manual QA pass (which will exercise the seed drift, blocking until the seed real-adapter project ships).
- [x] **6.c Landing + NotFound.** Both confirmed static — no `useQuery`, no `api.*`, no `fetch(`. `Landing.tsx` is marketing-copy + 3 `<Link to="/login">` CTAs; `NotFound.tsx` is 404 banner + 2 `useNavigate` actions. Component tests filed: `Landing.test.tsx` (2 tests — brand text + at least one `/login` link), `NotFound.test.tsx` (3 tests — 404 headline, back-nav, home-nav).
- [x] **6.d Public Q-equipe answer**: `Equipe.tsx` direct-fetch (Pattern D) RETAINED — no extraction to `useTeam` hook. Catalogued as accept-with-rationale: see `Equipe direct-fetch retained` entry in `KB § PATTERNS/accept-with-rationale.md` (filed this phase). Revisit trigger: a second product needs the same team-roster surface OR Equipe grows beyond 5 callsites.
- [x] Tests + manual browser QA per public route. Added 12 new tests across 6 new files: `Login` (2) + `ForgotPassword` (1) + `AcceptInvite` (1) + `NotFound` (3) + `Equipe` (3) + `Landing` (2). Total vitest count: 30 tests across 9 files (was 18/3). Manual browser QA DEFERRED per 6.a + 6.b destinations.
- [x] Keeper review + **Improvements** + phase proposal. Keeper PF: **0 issues, 0 proposals**. Improvements logged below. Phase-end proposal filed: `proposals/phase-6-seed-team-router-accept-drift.md` (high severity, seed-level — blocks invite flow runtime).

**Improvements:**
- **Seed-level contract drift in `team` standard router — broken accept_invitation invocation.** The seed router's POST `/accept` handler passes 7 kwargs to `accept_invitation` (db, schema, token, user_id, email, password, name) but the actual function takes 3 positional args (db, table, invitation_id). Same drift on `validate_invitation`. The flow is broken on the runtime path — any product invoking `/api/team/accept` will get TypeError. Migration `005_invitations.sql` shipped before this drift was caught (no integration test exists). **N=1 today** (PF is the first product whose Phase 6 audit caught it; other products inheriting `team` standard router likely have the same broken state but no live consumer surfaced it yet) — promotes to N=2+ when ERP/therapy Phase 6 audits run. **Cannot fix in standalone dispatch** (seed/framework/backend + seed/lib/backend out of scope). Filed: `proposals/phase-6-seed-team-router-accept-drift.md`. Triage: **formalize** at follow-up project — fix the seed router to call `validate_invitation(db, table, token)` (per actual sig) + add a separate "create auth user from invite" code path (the routers' present 7-kwarg call to `accept_invitation` implies a richer flow that doesn't exist in the domain helper) + ship an integration test that exercises the full validate → accept → mark-accepted → create-noctus-user round-trip.
- **No SSOCallback.tsx file in PF — by design, inherited from seed factory.** PROJECT §6.a phrasing said "PF pages Login.tsx, ForgotPassword.tsx, SSOCallback.tsx consume Core SSO" — but PF has no SSOCallback.tsx file. The seed `createProductApp` factory auto-mounts `<SSOCallback>` at `/sso` when supabase is configured (`seed/framework/frontend/src/app.tsx:253`). This is the right shape (zero per-product code for cross-cutting concern; PR-to-seed symmetry rule). PROJECT phrasing should be amended in any future Phase 6 brief to say "PF Login + ForgotPassword pages; SSO callback is seed-factory-mounted at /sso". Captured for orchestrator absorption (no code action needed; PF is correct already).
- **AcceptInvitePage's GET path is `/accept/validate` — already covered by the seed router.** Per `AcceptInvitePage.tsx:81-83`, the component probes `${apiBaseUrl}${acceptEndpoint}/validate?token=...` on mount — which resolves to `/api/team/accept/validate?token=...` matching the seed router's GET handler exactly. Wiring is correct.
- **Test-suite mocking pattern recurrence — vi.importActual on barrel modules trips UMD resolution.** Three of the four new tests (Login, ForgotPassword, AcceptInvite, Equipe) initially used `vi.importActual<>("@noctusai/lib/design-system")` to spread the actual module + override one export; this triggered a `MODULE_NOT_FOUND react-router-dom UMD` failure because the barrel transitively re-exports react-router-bound components and vitest's importActual hits a stray UMD path under `~/node_modules`. Fix: stub only the named export consumed by the SUT — drop importActual entirely. **N=4 in this session** (4 test files affected, one fix pattern). Triage: candidate for `__test_helpers__/stubDesignSystem.ts` if next product Phase 6 audit hits the same — accept-as-pattern for now (one-line cost per test file).
- **vi.hoisted is required when stubbing modules with closure-captured spies.** Equipe.test.tsx initially declared `const apiGet = vi.fn(...)` at module top scope, then referenced it inside `vi.mock("@noctusai/seed/infra", () => ({ api: { get: apiGet, ... } }))`. vi.mock hoists ABOVE the const declarations → ReferenceError. Fix: wrap the spy bundle in `vi.hoisted(() => ({ ... }))`. Pattern is documented in vitest docs but worth a note in `KB § PATTERNS/testing.md` if recurrence triggers.
- **Browser-QA deferral discipline — every public surface ticked with destination.** Per "no silent errors" rule, every manual-QA-only assertion ticks with a named destination (user/orchestrator at Phase 7 manual QA pass). Three deferrals filed: 6.a Supabase login round-trip, 6.b invite flow round-trip (blocked by seed drift above), 6.d admin gating UX. Captured for `findings.md` Category 2 (slips → discipline reinforced).

---

### Phase 7 — End-to-end verification ✅

- [x] `cd products/personal-finance/frontend && npx vite build` — clean. (✓ built in 4.78s; 555.80 kB main chunk advisory only, pre-existing).
- [x] `cd products/personal-finance/backend && python -m pytest tests/ -q` — **595 passed + 10 skipped** in 4.64s (Δ +11 from 584 baseline; pre-Phase-7: 590; Phase 7 adds 5 smoke tests).
- [x] `cd seed/lib/backend && python -m pytest tests/` — **1111 passed** in 6.53s.
- [x] `cd mcp/noctusai && python -m pytest tests/` — **1051 passed + 9 failed** in 2:53; all 9 failures pre-existing (touch ERP migrations, scaffold tests, dev_team — zero overlap with PF Phase 7 changes; verified by re-running a single failure unchanged).
- [x] `python mcp/noctusai/cli.py --review --product personal-finance` — **0 issues, 0 proposals**.
- [x] **Add 3 missing standard-router smoke tests** per §5.2.8: filed at `tests/routers/test_standard_routers_smoke.py` — 5 test functions across 3 classes (`TestAiOutputsStandardRouter`, `TestAiFeedbackStandardRouter`, `TestHealthStandardRouter`); each happy-path + auth-negative pair; status-code-assertion-rule strictly followed. Note: PROJECT brief specified `?ref_type=&ref_id=` for `/api/ai/feedback` but the current shipped router uses `output_ref` only (single query param); test pins the actual contract.
- [x] **Verify the 6 backend orphans** (§5.2.4): full frontend grep confirms **none of the 6 GET orphans are now consumed**. Disposition:
  - `POST /api/cotacoes/batch` — KEEP (no consumer; future-watchlist optimization)
  - `GET /api/ativos/por-carteira/{id}` — KEEP (CarteiraDetalhes does not call it; could wire in a future ativo-list pass)
  - `GET /api/operacoes/por-ativo/{id}` — KEEP (no CarteiraDetalhes ativo expansion shipped Phase 4)
  - `GET /api/operacoes/{id}` — **DELETABLE** (frontend only uses list `/api/operacoes` + DELETE; no detail page; mark for delete-on-touch)
  - `POST /api/carteiras/{id}/alocacao` — KEEP (no allocation editor UI; scaffold)
  - `GET /api/recorrentes/{id}` — KEEP (only list + `/proximas` consumed)
  - `GET /api/orcamentos/{id}/itens` — **DELETABLE** (only `/{id}/progresso` consumed; itens redundant)
  - `POST /api/ai/monthly-narrative/send` — INTENTIONAL CRON-ONLY ORPHAN (documented in MASTER-PROMPT).
  Per §5.2.7 cheap-to-keep policy, the 2 deletables are not removed this phase (zero side-effects; mark for delete-on-touch in future Recorrentes / Operacoes routing work).
- [DEFERRED] Manual browser QA of golden paths per surface — destination: user / orchestrator. No browser in worktree (no DISPLAY, no service stack). Smoke tests + keeper review + 595-passed suite cover the contract layer; browser QA exercises the full SSO + invite + scheduler stack which requires live Supabase + APScheduler + yfinance.
- [x] Update `products/personal-finance/MASTER-PROMPT.md` — added Standard Routers Mounted section + Contract Notes (created_by rename, fonte field, scheduler-deferred, backend orphans). Test count flipped 584 → 595.
- [x] Update `KB § CONTEXT/02-LANDSCAPE.md` if PF surface counts changed materially — auto-regenerated by `scripts/update-kb-counts.py`: PF test files 44 → 45, test fns 459 → 464; total 4,547 → 4,552. Counts block is hook-managed (do not hand-edit between markers).
- [x] Run `python scripts/update-kb-counts.py` and `bash scripts/verify-kb-sync.sh` — both green; sync OK; all CLAUDE.md pointers resolve.
- [x] **Lessons-learned harvest** — written at `personal-finance-wiring-lessons.md` (project root); 5 sections (what worked / what to redo / seed-lib reuse hit rate / test pattern recurrences / cross-product lift candidates). Parent `products-wiring-rollout` to fold into ERP plan.
- [x] Final **Improvements** block + phase proposal + Change Log entry before ✅. Phase proposal: `proposals/phase-7-disk-space-preflight.md`.

**Improvements:**
- **Disk-space pre-flight in `scripts/bootstrap-worktree.sh`.** Phase 7 hit "No space left on device" (124 MiB free; `~/.npm` cached 6.8 GiB; `npm cache clean --force` recovered ≈ 7 GiB instantly). The bootstrap is the right defense location — engineers' typical worktree consumes 2-3 GiB across product + seed/framework + seed/lib + pip venv. Filed at `proposals/phase-7-disk-space-preflight.md`. Triage: **accept-with-rationale at N=1**; promotes to formalize at N=2.
- **`/api/ai/feedback` contract drift between PROJECT.md and shipped router.** Phase 7 brief still cited the early `?ref_type=&ref_id=` shape; the current shipped router keys on a single `output_ref` query param. Smoke test pins the actual contract. **Triage: accept** — single-occurrence project-doc drift; not a code issue. Captured in Phase 7 changelog + lessons-learned (test pattern recurrences §d).
- **Standard-router mount-smoke coverage gap is structural across products.** PF had zero per-product mount-smoke for `ai_outputs` / `ai_feedback` / `health` (seed-tested but the per-product mount contract was unverified). Trivial to add (5 tests). **For ERP Phase 7: dispatch the same pattern as a Phase-0 deliverable, not a Phase-7 deferred TODO.** Captured in `personal-finance-wiring-lessons.md § (b) what to redo`.
- **MCP-toolkit pre-existing failures need a dedicated triage project.** 9 MCP failures untouched by PF Phase 7 (ERP migration `003_schema_separation.sql` SET search_path advisories + dev_team registration + scaffold slug placeholders + ts outline corpus baseline drift). They're below the radar of any single product-wiring project but they accumulate. **Triage: file as cross-product `mcp-toolkit-pretest-failures-triage` follow-up** when next PF / ERP engineer surfaces the same noise. N=1 today (PF Phase 7); promotes to formalize at N=2 (next engineer noticing).
- **Consumer-existence audit gap caught at Phase 5 should land at Phase 0.** `useCotacao` has zero page consumers; Phase 0 verified the hook→endpoint contract but not the call-site existence. **For ERP Phase 0: extend §5.2.3 with a `consumer_count` column.** Captured in lessons-learned § (b).
- **Backend orphans (2 deletable) NOT removed this phase.** `GET /api/operacoes/{id}` and `GET /api/orcamentos/{id}/itens` have zero frontend consumers but are cheap-to-keep per §5.2.7 policy. **Triage: accept-with-rationale**; delete-on-touch trigger if/when Operacoes / Orcamentos routing work re-opens those files.
...
| 2026-05-10 | **Phase 5 ✅** (PF engineer dispatched in isolated worktree `worktree-agent-a5ba6bf959f2eb5f6`). **DIVERGENT batch — scheduler + yfinance + AI-indicator wiring.** Three themes triaged against actual scope. **PF-5 (scheduler) — partial**: seed-side `scheduler` standard-router gift DEFERRED-with-destination (out of dispatch scope per standalone-mode rule; cross-product file `proposals/phase-5-scheduler-standard-router.md` filed with proposed seed surface + adopter list). Product-side **banner shipped**: `RecorrenteRow` in `pages/Recorrentes.tsx` renders "Auto" badge + "Próxima execução automática: {proxima_data}" line on every `is_automatico=true` row, sourcing existing data — no migration needed. "Última execução" indicator deferred (no `ultima_execucao` column; needs seed-side scheduler router OR new migration). **PF-6 (yfinance UX) — component shipped, consumers documented absent**: `<StaleBadge>` + pure `computeStaleness(cotacao, kind, now)` decision function added to `frontend/src/components/StaleBadge.tsx`. Threshold per §7 Q5: 15min stocks / 30min funds / 20min other. Decision tree dry-run → no-timestamp → old → fresh, returning `null` for fresh (silent absence = correct UX). **No consumer wired**: surveyed Watchlist/WatchlistDetalhes/Carteira/CarteiraDetalhes; none call `useCotacao(ticker)` directly (they render persisted `preco_atual` from the `ativos` table; `Cotacoes.tsx` page does not exist). Badge ships ready-to-wire when a real-time-quote consumer surfaces. **PF-7 (AIIndicator) — documented absence, no wiring**: re-verified backend `app/routers/ai.py` emits only `ref_type="transacao"` indicators (categorize + recurring-flag endpoints). No `ref_type="ativo"` producer exists. Per PROJECT instruction "if absent, document and skip" — Carteira detail wiring skipped intentionally. **Tests added**: 11 `StaleBadge.test.tsx` (decision-tree + render across dry-run/no-timestamp/old/fresh × stocks/funds/other) + 3 `Recorrentes.test.tsx` row-level (is_automatico true+data / false / true+missing-data). yfinance backend fallback path already covered by 4 pre-existing tests in `test_cotacoes_service.py`. **Verification**: `npx vitest run` → 18 tests pass (3 files: useAI 4 + StaleBadge 11 + Recorrentes 3); `npx vite build` → clean (✓ built in 21.85s); `pytest products/personal-finance/backend/tests/ -q` → 587 passed + 10 skipped + 3 baseline-only failures (unchanged from Phase 4 close); keeper review → 0 issues. **Methodology slips surfaced**: (a) Phase 0 audit verified hook→endpoint contract but didn't verify CONSUMER existence — `useCotacao` hook has zero page consumers; (b) worktree env three-install dance N=2 (Phase 3 + 5) → recommend `scripts/bootstrap-worktree.sh` (out-of-scope for this dispatch); (c) page-component test preamble heavy (8 mocks for Recorrentes row test) — extract `mockPageHooks()` helper if recurrence triggers. **Cross-product follow-ups surfaced**: (1) `<StaleBadge>` seed-lift candidate when ERP asset-management consumer surfaces (currently N=1 PF-only); (2) `scheduler` standard router (N=3 likely with PF + mailing + therapy adopters); (3) `ultima_execucao` column on `recorrentes` (cross-product if mailing/therapy schedulers want history too). Findings.md content returned as text in engineer's final report per §17.6.1. Manual browser QA deferred to project §7 integration round. Worktree base verification (§16.7): worktree branched from main fresh, no divergent commits to FF; orchestrator handles merge. | Claude Opus 4.7 (Phase 5 PF engineer, worktree-agent-a5ba6bf959f2eb5f6) |
...
| 2026-05-11 | **Phase 7 ✅ — project closing** (Engineer FFF, worktree `worktree-agent-ab5bb6977d27f6901`). **End-to-end verification + lessons harvest.** Verification matrix: PF frontend `npx vite build` clean (4.78s); PF backend `pytest` → **595 passed + 10 skipped** in 4.64s (Δ +11 from 584 baseline — Phase 7 adds 5 mount-smoke tests; Phases 1-6 contributed +6); seed-lib `pytest` → **1111 passed** in 6.53s; MCP `pytest` → **1051 passed + 9 failed** in 2:53 (all 9 failures pre-existing and untouched by PF Phase 7 — touch ERP migrations, scaffold tests, dev_team registration; verified by single-test re-run); keeper review `cli.py --review --product personal-finance` → **0 issues, 0 proposals**. **Smoke tests added**: `tests/routers/test_standard_routers_smoke.py` (5 fns across 3 classes pinning `/api/ai/outputs`, `/api/ai/feedback`, `/api/health` mount contracts); status-code-assertion rule strictly followed; `/api/ai/feedback` test uses the shipped `output_ref` shape (PROJECT brief cited an older `?ref_type=&ref_id=` contract — note added in Improvements). **Orphan triage** (§5.2.4): full frontend grep confirms 6 of 7 surveyed GET orphans have zero consumers; `GET /api/operacoes/{id}` and `GET /api/orcamentos/{id}/itens` flagged DELETABLE but per §5.2.7 cheap-to-keep policy NOT removed this phase — delete-on-touch trigger documented. `POST /api/ai/monthly-narrative/send` confirmed intentional cron-only orphan (now in MASTER-PROMPT). **MASTER-PROMPT update**: added Standard Routers Mounted section + Contract Notes (created_by rename, fonte field, scheduler-deferred, backend orphans); test count flipped 584→595. **KB sync**: `scripts/update-kb-counts.py` auto-regenerated `02-LANDSCAPE.md § Inventory` (PF 44→45 test files, 459→464 test fns, total 4,547→4,552); `scripts/verify-kb-sync.sh` → all CLAUDE.md pointers resolve, all KB docs indexed. **Lessons-learned harvest**: `personal-finance-wiring-lessons.md` filed at project root (5 sections: what worked / what to redo / seed-lib reuse hit rate / test pattern recurrences / cross-product lift candidates). **Phase-end proposal**: `proposals/phase-7-disk-space-preflight.md` — Phase 7 hit "No space left on device" mid-verification (124 MiB free; `~/.npm` cached 6.8 GiB; `npm cache clean --force` recovered ≈ 7 GiB); proposal extends `scripts/bootstrap-worktree.sh` with a leading `df -h /` probe. **Manual browser QA DEFERRED with destination** — no browser in worktree; deferred to user / orchestrator deploy drill. **Methodology slips captured**: (a) consumer-existence audit gap caught at Phase 5 should land at Phase 0 (`useCotacao` has zero page consumers); (b) standard-router mount-smoke is structurally missing across products (PF discovered Phase 7; for ERP make it Phase 0 deliverable); (c) `/api/ai/feedback` PROJECT-spec drift (brief vs shipped router contract); (d) disk-space pre-flight gap. **Findings.md content returned as text** per §17.6.1 (harness blocked engineer Write despite explicit authorization). | Claude Opus 4.7 (Phase 7 Engineer FFF, worktree-agent-ab5bb6977d27f6901) |
