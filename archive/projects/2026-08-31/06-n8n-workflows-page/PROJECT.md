...
- **R4 — the client tag is set in the page's Configurações subtab, NOT on the connect card.** [F] Fixed (tech-lead spec error, caught by S5). The original spec put a tag field on the card writing `channel_info` — but `channel_info` **has no REST write path at all** (neither `IntegrationAccountCreate` nor `IntegrationAccountService.create_account/update_account` accept it; migration 008 shipped the column and the youtube backfill, and nothing else). Extending the peer's service was the wrong fix: the card's job is narrower (connect = `base_url` + `api_key`), and the tag belongs to Configurações per the user's own framing. `PUT /api/n8n/settings` writes `channel_info` from the n8n module's own service ⇒ no peer-zone edit, and no fork (card→`metadata.n8n_tag` vs page→`channel_info.tag` would have been two homes for one fact).
- **R5 — pre-reshape rows report incomplete by DERIVATION, not by backfill.** [A] Accept. `status` is a stored column defaulting to `'validated'`, so a row written before the reshape *lies*. Rather than a decrypt-inspect-update backfill script (credentials are Fernet BYTEA — invisible to SQL), `GET /api/n8n/settings` derives the state on read: no `base_url` in the decrypted credential ⇒ report `error`. No migration, and it self-heals the instant the operator reconnects. Assigned to S3.
...
4. **Carried forward, NOT part of this project** (found by S5, evidence-backed — do not let them evaporate):
   - **The provider registry is dead data on the FE.** `integration_providers.py` declares `manual_key_fields` per provider and the router docstring claims "FE renders them as a form", but **zero components consume it** — `useIntegrationProviders()` (`useIntegrationAccounts.ts:107`) has exactly one consumer in the tree: its own test file. Every manual connect form (n8n, Mailchimp) is hand-authored inline in `ClienteModal.tsx`. Two independent declarations of one field-set ⇒ a live fork risk for the next provider added. Resolve as either a registry-driven connect-form organ, or retire the registry as descriptive-only.
   - **`channel_info` is a half-shipped column** (see R4) — real on one side, write-path-less on the other. Blocked while `feat/seed-auth-router-promotion` is live in that tree.
   - **`n8n` has no `PROVIDER_CARD_CONFIG` entry** (`seed/lib/frontend/src/design-system/integrations/providerCardConfig.ts`) — falls back to generic card rendering. Works today; means no tailored secondary-fields view for `base_url`/tag.
...
- **2026-07-31** — Live page returned **404, not 502**, once the previous fix made the failure legible. Root cause was **not** in the n8n slices at all: `useActiveAccountStore` held ONE provider-agnostic `activeAccountId`, persisted to localStorage and read raw by every provider-scoped hook. Navigating Meta → `/n8n` put the Meta account id (`25efa048…`, `provider='meta'`) on `GET /api/n8n/workflows`; `resolve_n8n_account` correctly rejected it as "right org, wrong provider". `ConnectedAccountSwitcher`'s reconcile effect existed for this exact case but is post-hoc — it runs only after the accounts query resolves, and the data hooks fire on the same render, so it lost the race. Fixed at the state layer (`feat/provider-scoped-active-account`): the selection is keyed by provider, so the leak is unrepresentable rather than reconciled. The flat field was **removed** rather than deprecated, which turned every one of the 26 read sites into a compile error — YouTube, Meta and WhatsApp were leaking the same way and were fixed in the same pass. Two things worth carrying forward: (1) the bug lived in shared state the n8n slices merely *consumed*, so no amount of review on those slices would have found it — the switcher's own docstring had even documented the limitation ("would need per-provider store slices, out of scope here") without anyone reading it as a live defect; (2) the resolver's 404 had the same diagnosability gap the 502 fix had just closed one layer down — the reason reached the browser and nothing reached the log, so identifying it cost a manual DB cross-reference. All three rejection branches now log, with tests; the 502 log the previous commit added by hand also got the regression test it shipped without.
