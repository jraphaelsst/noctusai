...
- **➡ Follow-up SPLIT OUT (2026-05-23) → `projects/deploy-config-pilot-consume/`** — pilots (erp/therapy/social) opt in by passing `required_prod_config=[...]` their actual prod-required keys. Deferred-with-destination (the seam is ready + back-compat; adoption is a per-product step, not a blocker) — NOT silent. The adoption wave now has its own project doc; it's **blocked on a user decision** (which keys are prod-required per pilot). Non-pilots extend in a later gated wave.
